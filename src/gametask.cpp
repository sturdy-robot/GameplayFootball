...
void GameTask::PutPhase() {
  ...
  if (match) {
    ...
    while (playerStartIndex < playersToProcess.size()) {
      ...
      for (unsigned int p = 0; p < playersPerThread; p++) {
        if (playerStartIndex + p >= playersToProcess.size())
          break;
        playersToProcessInThread.push_back(
            playersToProcess.at(playerStartIndex + p));
        // printf("adding player %i\n", playerStartIndex + p);
        //  unthreaded version: playersToProcess.at(playerStartIndex +
        //  p)->UpdateFullbodyModel();
      }
    ...
    match->UploadGoalNetting(); // won't this block the whole process thing too?
                                // (opengl busy == wait, while mutex locked ==
                                // no process)
  }
  ...
  if (match) {
    ...
    while (playerStartIndex < playersToProcess.size()) {
      ...
      // working on: maybe we need to use the gfx system get pointer somewhere
      // here? too tired to analyse this now :p
    }
