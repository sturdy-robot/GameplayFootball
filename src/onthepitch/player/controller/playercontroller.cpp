...
void PlayerController::Process() {
  ...
  if (match->GetLastTouchPlayer() == CastPlayer() &&
    ...
    reactionTime_ms = 0;
  // instant knowledge 'cheat' (aka. teamplay; players know what their teammates
  // will do. maybe make dynamic based on some teamplay stat in the future)
  // if (match->GetLastTouchTeamID() == team->GetID() &&
  // team->GetLastTouchType() != e_TouchType_Accidental) reactionTime_ms *=
  // 0.8f;
...
void PlayerController::AddDefensiveComponent(Vector3 &desiredPosition,
  ...
  if (match->IsInPlay() && !match->IsInSetPiece() &&
    ...
    if (opponentID != -1) {
      ...
      float possessionPlayerShootThreshold =
          24.0f; // distance at which opp must be marked tightly
      float genericOpponentShootThreshold =
          8.0f;                 // distance at which opp must be marked tightly
      float minDistance = 0.4f; // distance 'in front' of opp - how far from opp
                                // we defend as minimum
      float bufferDistance = 4.0f; // we want to be at least this distance
                                   // closer to shooting point than opp

      // calculate some basic vars
      Player *opp =
          match->GetTeam(abs(team->GetID() - 1))->GetPlayer(opponentID);
      PlayerImage oppImage = match->GetMentalImage(GetReactionTime_ms())
                                 ->GetPlayerImage(opp->GetID());
      ...
      // if shootingPoint.coords[0] exceeds offside trap line, alter
      // oppToThresholdDistance in such a way, that it results in the
      // shootingPoint being at least offsideTrapX distance away from goal
      // (unless player is closer already)
      if (shootingPoint.coords[0] * team->GetSide() >
        ...
        Line offsideLine;
        offsideLine.SetVertex(
            0,
            Vector3(team->GetController()->GetOffsideTrapX(), -pitchHalfH, 0));
        offsideLine.SetVertex(
            1,
            Vector3(team->GetController()->GetOffsideTrapX(), pitchHalfH, 0));
      ...
      // now we want to keep the distance to shootingPoint smaller than the
      // opponent's.
      Vector3 meToThreshold =
          shootingPoint -
          desiredPosition; // CastPlayer()->GetPosition() +
                           // CastPlayer()->GetMovement() * 0.14f;
      float meToThresholdDistance = meToThreshold.GetLength();

      float slackedDistance =
          meToThresholdDistance - (oppToThresholdDistance - bufferDistance);
      if (slackedDistance > 0.0f) {
        defendPosition = desiredPosition + meToThreshold.GetNormalized(0) *
                                               clamp(slackedDistance, 0.0f,
                                                     meToThresholdDistance);
      }

      // if we are too late, don't run straight towards the shootingPoint, but
      // instead, more towards our goal
      Vector3 actualPos =
          CastPlayer()->GetPosition() + CastPlayer()->GetMovement() * 0.14f;
      ...
      if (actualSlackedDistance > 0.0f) {
        defendPosition += (goalPos - defendPosition).GetNormalized(0) *
                          actualSlackedDistance * 0.7f;
      }
...
Vector3 PlayerController::GetDefendPosition(Player *opp, float distance) {
  ...
  // find the position on the opp -> goal line where we want to go to intercept.
  // this point is the same distance away from opp as it is from us. to find
  // this point: 1) create a line AB between us and op 2) create another line CD
  // perpendicular to AB, that intersects the middle point of AB 3) find the
  // intersection point of CD with the original opp to goal line. if this
  // intersection point is between opp and goal, it's our target. (if not, we're
  // too late, just run to goal :p)
  ...
  Vector3 midPoint =
      meToOppLine.GetVertex(0) * 0.5f + meToOppLine.GetVertex(1) * 0.5f;
  // 2
  Line midToOppPerpendicularLine;
  midToOppPerpendicularLine.SetVertex(0, midPoint);
  midToOppPerpendicularLine.SetVertex(
      1,
      midPoint + (meToOppLine.GetVertex(0) - midPoint).GetRotated2D(0.5 * pi));
  // 3
  float u;
  Vector3 intersect =
      oppToGoalLine.GetIntersectionPoint(midToOppPerpendicularLine, u);

  u = clamp(u, 0.0f, 1.0f);
  Vector3 target =
      oppToGoalLine.GetVertex(0) +
      (oppToGoalLine.GetVertex(1) - oppToGoalLine.GetVertex(0)) * u;
  ...
  */
  ...
  target += (oppToGoal.GetNormalized(0) * sprintVelocity * 0.1f) +
            (oppImage.movement * 0.14f) +
...
float PlayerController::OppBetweenBallAndMeDot() {
  ...
  Vector3 oppToBall = _mentalImage->GetBallPrediction(100).Get2D() -
                      (opp->GetPosition() + opp->GetMovement() * 0.1f);
  float dot =
      MeToOpp.GetNormalized(0).GetDotProduct(oppToBall.GetNormalized(0));
...
float PlayerController::CouldWinABallDuelLikeliness() {
  // uses the OppBetweenBallAndMeDot to check for correct angle, but also checks
  // ball distance from opp
  float dot = OppBetweenBallAndMeDot() * 0.5f + 0.5f;

  /* taking into account the relative distance to ball doesn't really seem to
  work all too well Vector3 meToBall =
  _mentalImage->GetBallPrediction(0).Get2D() - player->GetPosition();

  Player *opp = match->GetTeam(abs(team->GetID() -
  1))->GetDesignatedTeamPossessionPlayer(); Vector3 oppToBall =
  _mentalImage->GetBallPrediction(0).Get2D() - opp->GetPosition();
  ...
  return (1.0f - dot) * NormalizedClamp(oppBallDistFactor - meBallDistFactor,
  -1.0f, 0.0f); // 'same distance' is considered best now
  */
...
void PlayerController::_Preprocess() {
  _oppPlayer = match->GetPlayer(
      match->GetTeam(abs(team->GetID() - 1))->GetBestPossessionPlayerID());
  _timeNeeded_ms =
      CastPlayer()
          ->GetTimeNeededToGetToBall_ms(); // needed for synced version - if we
                                           // would ask on the spot and compare
                                           // to opp, opp may already have
                                           // calculated a new one while ours
...
void PlayerController::_KeeperDeflectCommand(PlayerCommandQueue &commandQueue,
  ...
  if (match->GetLastTouchTeamID() == team->GetID() &&
      match->GetLastTouchPlayer() != CastPlayer() &&
      match->GetLastTouchTeamID(e_TouchType_Intentional_Kicked) ==
          team->GetID())
    return;
  ...
  // can't use hands outside of keeper's 16 yard box (todo: make precise,
  // probably in humanoid.cpp's getbestcheatableanim)
  if (fabs(match->GetBall()->Predict(160).coords[1]) > 20.05f)
    return;
  if (match->GetBall()->Predict(160).coords[0] * -team->GetSide() >
      -pitchHalfW + 16.4)
    return;
...
void PlayerController::_BallControlCommand(PlayerCommandQueue &commandQueue,
  ...
  if (!CastPlayer()->HasPossession() && !CastPlayer()->AllowLastDitch()) {
    ...
    if (fabs(match->GetBall()->GetMovement().coords[2]) > 5.0f * strictnessInv)
      return;
    if ((match->GetBall()->GetMovement().Get2D() - CastPlayer()->GetMovement())
            .GetLength() > 5.0f * strictnessInv)
      return;
  }
  ...
  if (match->GetDesignatedPossessionPlayer() == player ||
    ...
    if (hasPossession && hasBestPossession &&
        ...
        stickyRunDirection) {
      radian angle =
          command.desiredDirection.GetAngle2D(CastPlayer()->GetDirectionVec());
      if (fabs(angle) > 0.125f * pi && fabs(angle) < 0.7f * pi) {
        if (angle > 0)
          command.desiredDirection =
              CastPlayer()->GetDirectionVec().GetRotated2D(0.125f * pi);
        if (angle < 0)
          command.desiredDirection =
              CastPlayer()->GetDirectionVec().GetRotated2D(0.125f * -pi);
      }
    ...
    if (keepCurrentBodyDirection &&
      ...
      command.desiredLookAt =
          CastPlayer()->GetPosition() + CastPlayer()->GetDirectionVec() * 10.0f;
    } else {
      command.desiredLookAt = CastPlayer()->GetPosition() +
                              CastPlayer()->GetMovement() * 0.2f +
                              command.desiredDirection * 10.0f;
    }

    // if (player->GetDebug()) SetRedDebugPilon(player->GetPosition() +
    // command.desiredDirection * (2.0f + command.desiredVelocityFloat)); if
    // (player->GetDebug()) SetYellowDebugPilon(command.desiredLookAt); if
    // (player->GetDebug()) SetSmallDebugCircle1(player->GetPosition());
    commandQueue.push_back(command);
  }
}
...
void PlayerController::_SlidingCommand(PlayerCommandQueue &commandQueue) {
  ...
  if (!teamHasBestPossession && possessionAmount < 0.6f &&
    ...
    Vector3 oppPos =
        _oppPlayer->GetPosition() + _oppPlayer->GetMovement() * 0.2;
    ...
    if ((ballDist > 0.7f && ballDist < 1.6f &&
      ...
      command.desiredDirection =
          ((ballPos.Get2D() + _oppPlayer->GetMovement() * 0.2f) - playerPos)
              .GetNormalized(
                  Vector3(-team->GetSide(), 0, 0)); // inputDirection;
      command.desiredVelocityFloat = sprintVelocity;
      command.useDesiredLookAt = true;
      command.desiredLookAt = CastPlayer()->GetPosition() +
                              CastPlayer()->GetMovement() * 0.1f +
                              command.desiredDirection * 10.0f;
      commandQueue.push_back(command);
    }
  }
...
void PlayerController::_MovementCommand(PlayerCommandQueue &commandQueue,
  ...
  float desiredVeloFactor =
      1.0f - std::pow(NormalizedClamp(std::min(inputVelocityFloat,
                 ...
                 0.3f;
  Vector3 focusPos =
      _mentalImage->GetBallPrediction(defaultLookAtTime_ms).Get2D();
  focusPos += player->GetDirectionVec() *
              0.5f; // to keep looking forward if ball is very close
  radian toFocusAngle = (focusPos - player->GetPosition())
                            .GetNormalized(manualDirection)
                            .GetAngle2D(manualDirection);
  ...
  float inputDirIsOwnHalfFactor = NormalizedClamp(
      inputDirection.GetDotProduct(Vector3(team->GetSide(), 0, 0)), -1.0f,
      1.0f);
  ...
  if (match->GetBallRetainer() == player) {
    ...
    if (hasBestPossession) {
      ...
      CastPlayer()->SetDesiredTimeToBall_ms(AI_GetBallControlMovement(
          _mentalImage, CastPlayer(), quantizedInputDirection,
      ...
      if (extraHaste || forceMagnet) {
        ...
        float thresholdPossessionAmountForHaste =
            1.1f; // 2.0f -
                  // AI_GetMindSet(CastPlayer()->GetFormationEntry().role)
                  // * 2.0f; // attacking players may want to gamble on the
                  // defenders missing the ball
      ...
      CastPlayer()->SetDesiredTimeToBall_ms(AI_GetToBallMovement(
          match, _mentalImage, CastPlayer(), quantizedInputDirection,
          inputVelocityFloat, autoDirection, autoVelocityFloat, autoLookAt,
          haste));
      ...
      if (match->GetDesignatedPossessionPlayer() != player) {
        float sameDirFactor =
            1.0f - clamp(fabs(autoDirection.GetAngle2D(manualDirection)) / pi,
                         0.0f, 1.0f);
        sameDirFactor = sameDirFactor * 0.5f +
                        0.5f; // todo: can't fully trust this; it just isn't a
                              // 100% indication of the player's intentions.
                              // just use this hax for now
        autoBias = 0.0f;
            ...
            float magnetBias =
                std::max(curve(sameDirFactor, 0.8f),
                         powf(GetLastSwitchBias(),
                              0.5f)); // needed for when we get passed a ball
                                      // while we are not the designated player.
                                      // we want to at least try to get there.
            magnetBias *=
                1.0f -
                inputDirIsOwnHalfFactor; // if we run for our own half, don't
  ...
  } else if (match->GetDesignatedPossessionPlayer()->GetTeamID() !=
             team->GetID()) {
    ...
    if (team->GetDesignatedTeamPossessionPlayer() == player) {
      ...
      // if we aren't the designated possession player, we don't ever want to
      // just run straight to the ball like that. we want some combination of
      // manual movement, defensive movement, and to-ball movement.

      // virtual action area in front of opponent
      PlayerImage oppImage = _mentalImage->GetPlayerImage(_oppPlayer->GetID());
      Vector3 oppPos = oppImage.position + oppImage.movement * 0.14f +
                       oppImage.directionVec * 0.6f;
      Vector3 oppToGoalDirection =
          (Vector3(pitchHalfW * team->GetSide(), 0, 0) - oppPos)
              .GetNormalized(0);
      float actionRadius = 5.0f;
      Vector3 focusPosition =
          oppPos + (oppToGoalDirection * actionRadius * 0.7f);
      float actionBias =
          1.0f - curve(NormalizedClamp(
                           (player->GetPosition() - focusPosition).GetLength(),
                           0.0f, actionRadius),
                       0.7f);

      // if we're close to the focus position, go do more defending or
      // ballhuntin' (as opposed to manual movement)
      if (team->IsHumanControlled(player->GetID()))
        autoBias = actionBias * 0.0f + GetLastSwitchBias() * 0.3f;
      else
        autoBias = actionBias * 0.0f; // not sure what would be a proper value
                                      // here. needs more testing

      autoBias *=
          1.0f - inputDirIsOwnHalfFactor; // if we want to run back to more
                                          // defensive positions, don't autobias

      // now decide on what auto direction to use, if applicable
      // when farther away from ball/opp, get towards ball/opp, when already
      // close, mimic opponent's movement (sort of an auto 'man marking' thing)
      if (autoBias > 0.0f) {
        ...
        Vector3 autoDirection_manMarking =
            oppImage.movement.GetNormalized(inputDirection);
        ...
        Vector3 huntTarget =
            oppPos + oppToGoalDirection * playerOppDistance * 0.3f;
        Vector3 huntMovement = huntTarget - player->GetPosition();
        Vector3 autoDirection_hunt = huntMovement.GetNormalized(inputDirection);
        float autoVelocityFloat_hunt =
            clamp(huntMovement.GetLength() * distanceToVelocityMultiplier,
        ...
        autoVelocityFloat =
            clamp(autoVelocityFloat_manMarking * manMarkingBias +
...

  {
    ...
    Vector3 resultingMovement =
        manualMovement * (1.0 - autoBias) + autoMovement * autoBias;
    command.desiredDirection =
        resultingMovement.GetNormalized(quantizedInputDirection);
...
void PlayerController::_CalculateSituation() {
  ...
  possessionAmount = (float)(match->GetTeam(abs(team->GetID() - 1))
  ...
  // when ball is close, don't fade possession bias (so we can take immediate
  // action)
  if (hasBestChanceOfPossession) {
    float distanceBias =
        std::pow(NormalizedClamp((match->GetBall()->Predict(300).Get2D() -
                                  player->GetPosition())
                                     .GetLength(),
                                 2.0f, 14.0f),
                 2.0f);
    fadingTeamPossessionAmount = fadingTeamPossessionAmount * distanceBias +
                                 teamPossessionAmount * (1.0f - distanceBias);
  }
