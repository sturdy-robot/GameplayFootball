...
protected:
  ...
  // 0: undefined, 1: off-the-ball button active, 2: on-the-ball button
  // active/action queued
  int actionMode;
  ...
  // stuff to keep track of analog stick (or keys even) so that we can use a
  // direction once it's been pointed in for a while, instead of directly
