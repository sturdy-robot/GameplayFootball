...
struct PreRating { // for optimization: first calculate everything but
                   // passrating, since that one is slow to calculate. cull
                   // results before rating pass freedom
...
public:
  ...
  Vector3 GetSupportPosition(const MentalImage *mentalImage,
...
protected:
  void GetOnTheBallCommands(std::vector<PlayerCommand> &commandQueue,
                            Vector3 &rawInputDirection,
                            float &rawInputVelocity);

  void _AddPass(std::vector<PlayerCommand> &commandQueue, Player *target,
