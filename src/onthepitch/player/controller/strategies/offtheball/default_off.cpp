...
DefaultOffenseStrategy::DefaultOffenseStrategy(ElizaController *controller)
    : Strategy(controller) {
...
void DefaultOffenseStrategy::RequestInput(const MentalImage *mentalImage,
  ...
  float staticPositionBias = curve(
      0.8f * actionDistance, 1.0f); // lower values = swap position with other
                                    // players' formation positions more easily
  Vector3 desiredPosition =
      desiredPosition_static * staticPositionBias +
  ...
  if (offensiveComponents) {
    // support position
    float attackBias = NormalizedClamp(
        (controller->GetFadingTeamPossessionAmount() - 0.5f) * 1.0f, 0.1f,
        0.6f);
    bool makeRun = false;
    if (attackBias > 0.7f) {
      if (team->GetController()->GetEndApplyAttackingRun_ms() >
              match->GetActualTime_ms() &&
          team->GetController()->GetAttackingRunPlayer() == player) {
        makeRun = true;
      }
    }
    Vector3 supportPosition = controller->GetSupportPosition_ForceField(
        mentalImage, desiredPosition, makeRun);
    desiredPosition =
        desiredPosition * (1.0f - attackBias) + supportPosition * attackBias;
  }
  ...
  if (defensiveComponents) {
    ...
    controller->AddDefensiveComponent(
        desiredPosition,
        pow(clamp(1.3f - mindset - controller->GetFadingTeamPossessionAmount(),
                  0.0f, 1.0f),
            0.7f));
  }
  ...
  direction = (desiredPosition - player->GetPosition())
                  .GetNormalized(player->GetDirectionVec());
  float desiredVelocity =
      (desiredPosition - player->GetPosition()).GetLength() *
