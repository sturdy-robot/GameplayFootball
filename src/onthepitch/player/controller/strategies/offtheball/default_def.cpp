...
DefaultDefenseStrategy::DefaultDefenseStrategy(ElizaController *controller)
    : Strategy(controller) {
...
void DefaultDefenseStrategy::RequestInput(const MentalImage *mentalImage,
  ...
  float staticPositionBias = curve(
      1.0f * actionDistance, 1.0f); // lower values = swap position with other
  ...
  if (offensiveComponents) {
    ...
    float attackBias = NormalizedClamp(
        (controller->GetFadingTeamPossessionAmount() - 0.5f) * 1.0f, 0.2f,
        0.9f);
  ...
  if (defensiveComponents) {
    ...
    controller->AddDefensiveComponent(
        desiredPosition,
        pow(clamp(1.9f - mindset - controller->GetFadingTeamPossessionAmount(),
                  0.0f, 1.0f),
            0.7f));
