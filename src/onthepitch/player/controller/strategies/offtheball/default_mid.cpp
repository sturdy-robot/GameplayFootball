...
void DefaultMidfieldStrategy::RequestInput(const MentalImage *mentalImage,
                                           Vector3 &direction,
                                           float &velocity) {
  ...
  float staticPositionBias = curve(
      0.9f * actionDistance, 1.0f); // lower values = swap position with other
  ...
  if (defensiveComponents) {
    ...
    controller->AddDefensiveComponent(
        desiredPosition,
        pow(clamp(1.5f - mindset - controller->GetFadingTeamPossessionAmount(),
                  0.0f, 1.0f),
            0.7f));
