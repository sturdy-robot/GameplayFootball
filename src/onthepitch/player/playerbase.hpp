...
public:
  ...
  inline Vector3 GetBodyDirectionVec() const {
    return humanoid->GetBodyDirectionVec();
  }
  ...
  inline e_Velocity GetEnumVelocity() const {
    return humanoid->GetEnumVelocity();
  }
  ...
  void TripMe(const Vector3 &tripVector, int tripType) {
    humanoid->TripMe(tripVector, tripType);
  }
  ...
  boost::intrusive_ptr<Node> GetHumanoidNode() {
    return humanoid->GetHumanoidNode();
  }
  boost::intrusive_ptr<Node> GetFullbodyNode() {
    return humanoid->GetFullbodyNode();
  }

  float GetDecayingPositionOffsetLength() {
    return humanoid->GetDecayingPositionOffsetLength();
  }
  ...
  void SetLastTouchTime_ms(unsigned long touchTime_ms) {
    this->lastTouchTime_ms = touchTime_ms;
  }
  unsigned long GetLastTouchTime_ms() { return lastTouchTime_ms; }
  void SetLastTouchType(e_TouchType touchType) {
...
protected:
  ...
  std::vector<Vector3>
      positionHistoryPerSecond; // resets too (on ResetSituation() calls)
};
