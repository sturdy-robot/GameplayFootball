...
public:
  ...
  inline void SetDynamicFormationEntry(FormationEntry entry) {
    dynamicFormationEntry = entry;
  }
  inline FormationEntry GetDynamicFormationEntry() {
    return dynamicFormationEntry;
  }
  ...
  inline int GetTimeNeededToGetToBall_ms() const {
  ...
  void TriggerControlledBallCollision() {
    triggerControlledBallCollision = true;
  }
  bool IsControlledBallCollisionTriggered() {
    return triggerControlledBallCollision;
  }
  void ResetControlledBallCollisionTrigger() {
    triggerControlledBallCollision = false;
  }

  float GetAverageVelocity(
      float timePeriod_sec); // is reset on ResetSituation() calls

  void UpdatePossessionStats(bool onInterval = true);

  float GetClosestOpponentDistance() const;

  const TacticalPlayerSituation &GetTacticalSituation() {
