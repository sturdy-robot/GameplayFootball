...
void Player::Activate(boost::intrusive_ptr<Node> humanoidSourceNode,
  ...
  humanoid = new Humanoid(this, humanoidSourceNode, fullbodySourceNode,
                          colorCoords, animCollection,
  ...
  nameCaption = new Gui2Caption(GetMenuTask()->GetWindowManager(),
                                "game_player_name_" + int_to_str(id), 0, 0, 1,
                                2.0, playerData->GetLastName());
...
void Player::Deactivate() {
  ...
  if (team->IsHumanControlled(this->GetID())) {
    team->DeselectPlayer(this); // don't want any humangamer to have control of
                                // this player anymore
  }
...
bool Player::AllowLastDitch(bool includingPossessionAmount) const {
  if (includingPossessionAmount && team->GetTeamPossessionAmount() < 1.0f)
    return true; // why team possession amount and not player's? answer: because
                 // we don't have that info here (todo: fix that)
  return (GetTimeNeededToGetToBall_optimistic_ms() * 1.7f + 800 <
...
float Player::GetAverageVelocity(float timePeriod_sec) {
  ...
  return totalDistance / timePeriod_sec; // don't divide by count, since lack of
                                         // entries should not influence average
}
...
void Player::UpdatePossessionStats(bool onInterval) {
  ...
  timeNeededToGetToBall_previous_ms =
      timeNeededToGetToBall_ms; // todo: this will fail to function as intended
                                // when this function is ran multiple times
                                // consecutively
  ...
  bool precise =
      (team->GetDesignatedTeamPossessionPlayer() == this) ? true : false;
  ...
  for (unsigned int ms = startTime_ms; ms < ballPredictionSize_ms;
    ...
    if (match->GetBall()->Predict(ms).coords[2] < 1.5f) {
      ...
      // // debug
      // if (GetDebug()) {
      //   float dist = (match->GetBall()->Predict(ms).Get2D() -
      //   GetPosition()).GetLength(); if (dist > previousDist) {
      ...
      if (timeNeeded_optimistic <= ms) {
        ...
        if (GetDebug()) {
          // SetRedDebugPilon(GetPosition() +
          // Vector3(timeNeededToGetToBall_optimistic_ms / 100.0f, 0.0f,
          // 0.05f));
        }
      }
      ...
      if (timeNeeded <= ms) {
        ...
        if (!refine) {
          ...
          if (GetDebug()) {
            ...
            // timeStep_ms, dud, (match->GetBall()->Predict(0) -
            // GetPosition()).GetLength()); printf("time needed: %u\n",
            // timeNeededToGetToBall_ms); SetGreenDebugPilon(GetPosition() +
            // Vector3(timeNeededToGetToBall_ms / 100.0f, 0.0f, 0.0f));
            // printf("found!\n");
          }
          if (team->GetDesignatedTeamPossessionPlayer() == this &&
              team->GetID() == 1) {
            // SetYellowDebugPilon(GetPosition() +
            // Vector3(timeNeededToGetToBall_ms / 100.0f, 0.0f, 0.0f));
          }
          break;
        }
    ...
    if (!refine) {
      float balldist =
          (GetPosition() - match->GetBall()->Predict(ms).Get2D()).GetLength() +
  ...
  if (TouchAnim() && TouchPending()) {
    unsigned int animTimeToBall_ms =
        (CastHumanoid()->GetTouchFrame() - GetCurrentFrame()) * 10;
    timeNeededToGetToBall_ms =
        std::min(timeNeededToGetToBall_ms, animTimeToBall_ms);
    timeNeededToGetToBall_optimistic_ms = timeNeededToGetToBall_ms;
  }
  ...
  if (timeNeededToGetToBall_ms < defaultTouchOffset_ms) {
    ...
    timeNeededToGetToBall_ms =
        NormalizedClamp(
            ((GetPosition() + GetMovement() * (defaultTouchOffset_ms * 0.001)) -
  ...
  this->hasUniquePossession =
      ...
      !match->GetTeam(abs(team->GetID() - 1))
           ->HasPossession(); // todo: this should be in a seperate function, to
                              // be called after the other team's timetoball
...
void Player::Process() {
  ...
  if (isActive) {
    ...
    // if (GetDebug() && match->GetActualTime_ms() % 1000 == 0) printf("fatigue:
    // %f\n", GetFatigueFactorInv());
...
void Player::Put2D() {
  ...
  if (GetDebugMode() == e_DebugMode_AI) {
    ...
    if (GetManMarkingID() != -1 && team->GetID() == 1) {
      //      Vector3 p1 = GetProjectedCoord(GetGeomPosition().Get2D(),
      //      match->GetCamera()); Vector3 p2 =
      ...
      //      GetMenuTask()->GetWindowManager()->GetCoordinates(p1.coords[0],
      //      p1.coords[1], 1, 1, x1, y1, dud1, dud2);
      //      GetMenuTask()->GetWindowManager()->GetCoordinates(p2.coords[0],
      //      p2.coords[1], 1, 1, x2, y2, dud1, dud2);

      GetDebugOverlayCoord(match, GetGeomPosition(), x1, y1);
      GetDebugOverlayCoord(
          match, match->GetPlayer(GetManMarkingID())->GetGeomPosition(), x2,
          y2);
      ...
      /*
      Triangle triangle;
      triangle.SetVertex(0, Vector3(x1, y1, 0) + (Vector3(x2, y2, 0) -
      Vector3(x1, y1, 0)).GetRotated2D(0.5 * pi).GetNormalized(0) * 4);
      triangle.SetVertex(1, Vector3(x1, y1, 0) + (Vector3(x2, y2, 0) -
      Vector3(x1, y1, 0)).GetRotated2D(-0.5 * pi).GetNormalized(0) * 4);
  ...
  if (fetchedbuf_nameCaptionShowCondition) {
    ...
    // Vector3 captionPos2D = GetProjectedCoord(captionPos3D,
    // match->GetCamera());
    Vector3 captionPos3D = GetProjectedCoord(
        GetGeomPosition() + Vector3(0, 0.5f, 2.4f),
    ...
    nameCaption->SetPosition(captionPos3D.coords[0] - w * 0.5f,
                             captionPos3D.coords[1] - h);
  ...
  if (fetchedbuf_debugCaptionShowCondition) {
    Vector3 captionPos3D = GetProjectedCoord(
        GetGeomPosition() + Vector3(0, 0.3f, 2.0f), match->GetCamera());
    float w, h;
    debugCaption->GetSize(w, h);
    debugCaption->SetPosition(captionPos3D.coords[0] - w * 0.5f,
                              captionPos3D.coords[1] - h);
...
void Player::SendOff() {
  ...
  if (remainingPlayers <= 6) {
    ...
    // todo: referee should do this. also, make sure the forfeiting team will
    // actually lose :P
    match->GameOver();
  }
}
...
float Player::GetStat(const char *name) const {
  ...
  multiplier *=
      0.7f + 0.3f * GetFatigueFactorInv(); // todo: some stats are more affected
                                           // by fatigue than others
  // if (GetExternalController()) printf("stat %s: %f\n", name,
  // playerData->GetStat(name)); if (GetDebug()) printf("stat %s == %f\n", name,
  // playerData->GetStat(name) * multiplier);
...
void Player::_CalculateTacticalSituation() {
  ...
  float time_sec = 0.5f;
  Vector3 checkPos = GetPosition() + Vector3(-team->GetSide(), 0, 0) *
                                         sprintVelocity * time_sec;
  tacticalSituation.forwardSpaceRating =
      AI_CalculateFreeSpace(match, mentalImage, team->GetID(), checkPos, 5.0f,
                            time_sec, true); // FREESPACE :D :D

  // calculate the amount of space this player has
  // tacticalSituation.spaceRating = AI_CalculatePersonalFreeSpace(match,
  // mentalImage, this, 8.0f, 8.0f, 0.2f);
  time_sec = 0.1f;
  checkPos = GetPosition() + GetMovement() * time_sec;
  tacticalSituation.spaceRating =
      AI_CalculateFreeSpace(match, mentalImage, team->GetID(), checkPos, 5.0f,
                            time_sec, true); // FREESPACE :D :D
  ...
  tacticalSituation.forwardRating =
      1.0f -
      clamp((Vector3(pitchHalfW * -team->GetSide(), 0, 0) - GetPosition())
                    .GetLength() /
                (pitchHalfW * 2.0f),
            0.0f, 1.0f);
  tacticalSituation.forwardRating =
