...
public:
  ...
  void SetOffset(const std::string &nodeName, float bias,
                 const Quaternion &orientation, bool isRelative = false);
  ...
  inline e_Velocity GetEnumVelocity() const {
    return spatialState.enumVelocity;
  }
  inline e_FunctionType GetCurrentFunctionType() const {
    return currentAnim->functionType;
  }
  inline e_FunctionType GetPreviousFunctionType() const {
    return previousAnim->functionType;
  }
  ...
  virtual float GetDecayingDifficultyFactor() const {
    return decayingDifficultyFactor;
  }
...
protected:
  ...
  void _KeepBestDirectionAnims(
      ...
      radian allowedAngle = 0, int allowedVelocitySteps = 0,
      int forcedQuadrantID =
          -1); // ALERT: set sorting predicates before calling this function.
               // strict kinda overrules the allowedstuff
  void _KeepBestBodyDirectionAnims(
      DataSet &dataset, const PlayerCommand &command, bool strict = true,
      radian allowedAngle =
          0); // ALERT: set sorting predicates before calling this function.
              // strict kinda overrules the allowedstuff
  virtual bool
  ...
  void CalculatePredictedSituation(Vector3 &predictedPos,
                                   radian &predictedAngle);
  Vector3
  CalculateOutgoingMovement(const std::vector<Vector3> &positions) const;

  void CalculateSpatialState(); // realtime properties, based on 'physics'
  void CalculateFactualSpatialState(); // realtime properties, based on anim.
                                       // usable at last frame of anim. more
                                       // riggid than above function

  void AddTripCommandToQueue(PlayerCommandQueue &commandQueue,
                             const Vector3 &tripVector, int tripType);
  PlayerCommand GetTripCommand(const Vector3 &tripVector, int tripType);
  PlayerCommand GetBasicMovementCommand(const Vector3 &desiredDirection,
                                        float velocityFloat);
  ...
  bool CompareIncomingBodyDirectionSimilarity(int animIndex1,
  ...
  void SetNumericVariableSimilarityPredicate(const std::string &varName,
  ...
  Vector3
  CalculatePhysicsVector_disabled(Animation *anim,
                                  const Vector3 &desiredMovement,
                                  std::vector<Vector3> &positions_ret) const;

  Vector3 ForceIntoAllowedBodyDirectionVec(const Vector3 &src) const;
  radian ForceIntoAllowedBodyDirectionAngle(radian angle)
      const; // for making small differences irrelevant while sorting
  ...
  std::vector<std::vector<WeightedVertex>>
      weightedVerticesVec; // < subgeoms < vertices > >
