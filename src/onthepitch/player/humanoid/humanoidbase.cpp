...
void FillTemporalHumanoidNodes(
    boost::intrusive_ptr<Node> targetNode,
...
HumanoidBase::HumanoidBase(PlayerBase *player, Match *match,
    ...
    : fullbodyTargetNode(fullbodyTargetNode), match(match), player(player),
      anims(animCollection), buf_bodyUpdatePhaseOffset(bodyUpdatePhaseOffset) {
  ...
  boost::intrusive_ptr<Node> bla2(new Node(
      *fullbodySourceNode.get(), int_to_str(player->GetID()), GetScene3D()));
  fullbodyNode = bla2;
  ...
  std::vector<MaterializedTriangleMesh> &tmesh =
      bodyGeom->GetResource()->GetTriangleMeshesRef();
  for (unsigned int i = 0; i < tmesh.size(); i++) {
    if (tmesh.at(i).material.diffuseTexture !=
  ...
  boost::intrusive_ptr<Resource<GeometryData>> geometry =
      ResourceManagerPool::GetInstance()
          ...
          ->Fetch("media/objects/players/hairstyles/" +
  ...
  for (unsigned int i = 0; i < hairtmesh.size(); i++) {
    if (hairtmesh.at(i).material.diffuseTexture !=
        boost::intrusive_ptr<Resource<Surface>>()) {
...
HumanoidBase::~HumanoidBase() {
  ...
  // the other full body mesh ref is connected to a geometry object which has
  // taken over ownership and will clean it up
...
void HumanoidBase::PrepareFullbodyModel(
  ...
  // base anim with default angles - all anims' joints will be inversely rotated
  // by the joints in this anim. this way, the fullbody mesh doesn't need to
  // have 0 degree angles
  ...
  animApplyBuffer.anim->Apply(
      nodeMap, animApplyBuffer.frameNum, 0, animApplyBuffer.smooth,
  ...
  for (unsigned int subgeom = 0; subgeom < fullbodySubgeomCount; subgeom++) {
    ...
    int elementOffset =
        meshRef.size / GetTriangleMeshElementCount(); // was: fullbodyMeshSize
    ...
    for (int v = 0; v < elementOffset; v += 3) {
      ...
      for (int e = 0; e < GetTriangleMeshElementCount(); e++) {
        elementalVertex.push_back(
            Vector3(meshRef.data[v + e * elementOffset],
                    meshRef.data[v + e * elementOffset + 1],
                    meshRef.data[v + e * elementOffset + 2]));
        // test: if (e == 2) elementalVertex.at(elementalVertex.size() - 1) +=
        // 0.2f;
      }
      ...
      for (unsigned int i = 0; i < uniqueVertices.size(); i++) {
        if (uniqueVertices[i][0] == elementalVertex[0] &&
            uniqueVertices[i][2] ==
                elementalVertex[2]) { // texcoord also needs to be shared
          duplicate = true;
          index = i;
          break;
        }
    ...
    // uniqueVertices.size()); printf("meshRef.size: %i, uniqueIndices.size:
    ...
    for (unsigned int v = 0; v < uniqueVertices.size(); v++) {
      for (int e = 0; e < GetTriangleMeshElementCount(); e++) {
        uniqueMesh.data[v * 3 + e * uniqueElementOffset + 0] =
            uniqueVertices.at(v).at(e).coords[0];
        uniqueMesh.data[v * 3 + e * uniqueElementOffset + 1] =
            uniqueVertices.at(v).at(e).coords[1];
        uniqueMesh.data[v * 3 + e * uniqueElementOffset + 2] =
            uniqueVertices.at(v).at(e).coords[2];
      }
    }
        ...
        if (c == 0) {
          if (weightedBones[c].weight == 0.f)
            printf("offending jointID: %i (coord %i) (vertexpos %f, %f, %f)\n",
                   weightedBones[c].jointID, c, vertexPos.coords[0],
                   vertexPos.coords[1], vertexPos.coords[2]);
    ...
    assert(materializedTriangleMeshes.at(subgeom).verticesDataSize /
           ...
           elementOffset);
    assert(uniqueMesh.size ==
           uniqueElementOffset * GetTriangleMeshElementCount());
    delete[] materializedTriangleMeshes.at(subgeom).vertices;
    materializedTriangleMeshes.at(subgeom).vertices =
        new float[uniqueMesh.size];
    ...
    for (int v = 0; v < elementOffset; v += 3) {
      materializedTriangleMeshes.at(subgeom).indices.push_back(
          uniqueIndices[v / 3]);
    }
    ...
    /*
        ...
        for (int i = 0; i < elementOffset; i += 3) {
          printf("%f, ",
       materializedTriangleMeshes.at(subgeom).vertices[uniqueIndices[i / 3] * 3
  ...
  for (unsigned int i = 0; i < joints.size(); i++) {
    joints.at(i).orientation =
        jointsVec[i]->GetDerivedRotation().GetInverse().GetNormalized();
  }
  ...
  for (unsigned int i = 0; i < joints.size(); i++) {
    joints.at(i).position =
        jointsVec[i]->GetDerivedPosition(); // * zMultiplier;
  }
...
void HumanoidBase::UpdateFullbodyNodes() {
  ...
  // todo: something is wrong with the hairdo update logic, so just always
  // update now
  /*
  if (NeedsModelUpdate()) {
    hairStyle->SetRotation(joints[2].orientation, false);
    hairStyle->SetPosition(joints[2].position * zMultiplier + fullbodyOffset,
  false); hairStyle->RecursiveUpdateSpatialData(e_SpatialDataType_Both); } else
  { hairStyle->SetPosition(hairStyle->GetPosition() + (fullbodyOffset -
  previousFullbodyOffset), false);
...
/* moved this to gametask upload thread, so it can be multithreaded, whilst
assuring its lifetime void HumanoidBase::UploadFullbodyModel() {
...
void HumanoidBase::Process() {
  ...
  if (interruptAnim != e_InterruptAnim_None) {
    ...
    if (found) {
      ...
      animApplyBuffer.smoothFactor =
          (interruptAnim == e_InterruptAnim_Switch) ? 0.6f : 1.0f;
  ...
  // radian beginAngle = currentAnim->rotationSmuggle.begin;// * (1.0f -
  // frameBias); // more influence in the beginning; act more like it was 0
  // later on. (yes, next one is a bias within a bias) *edit: disabled, looks
  // better without currentAnim->rotationSmuggleOffset = beginAngle * (1.0f -
  // frameBias) +
  //                                      currentAnim->rotationSmuggle.end *
  //                                      frameBias;

  currentAnim->rotationSmuggleOffset =
      currentAnim->rotationSmuggle.begin * (1.0f - frameBias) +
...
void HumanoidBase::PreparePutBuffers(unsigned long snapshotTime_ms) {
  ...
  // offsets
  CalculateGeomOffsets(); // todo: in a perfect world, we don't want to do cpu
                          // intensive and/or stuff that uses a lot of mutex
                          // locking in this here function

  buf_animApplyBuffer = animApplyBuffer;
  buf_animApplyBuffer.snapshotTime_ms = snapshotTime_ms;
  /*
  // some temporal buffers fail when switching anims - can't interpolate between
  these values from previous and new anim if (currentAnim->frameNum == 0) {
  ...
  */
  ...
  if (!player->GetExternalController() && !match->GetPause()) {
    ...
    if (match->GetDesignatedPossessionPlayer()) {
      focusPos = focusPos * 0.5f +
                 match->GetDesignatedPossessionPlayer()->GetPosition() * 0.5f;
    }
...
void HumanoidBase::Put() {
  ...
  unsigned long timeDiff_ms = fetchedbuf_animApplyBuffer.snapshotTime_ms -
                              fetchedbuf_previousSnapshotTime_ms;
  // printf("diff: %lu\n", timeDiff_ms);
  ...
  for (unsigned int i = 0; i < buf_TemporalHumanoidNodes.size(); i++) {
    // first, restore the previous non-temporal-smoothed values, so the Apply()
    // function can use them for smoothing
  ...
  fetchedbuf_animApplyBuffer.anim->Apply(
      nodeMap, fetchedbuf_animApplyBuffer.frameNum, -1,
      fetchedbuf_animApplyBuffer.smooth,
      fetchedbuf_animApplyBuffer.smoothFactor,
      fetchedbuf_animApplyBuffer.position,
      fetchedbuf_animApplyBuffer.orientation,
      fetchedbuf_animApplyBuffer.offsets, &movementHistory, timeDiff_ms,
      fetchedbuf_animApplyBuffer.noPos, false);

  humanoidNode->RecursiveUpdateSpatialData(e_SpatialDataType_Both);

  // we've just set the humanoid positions for time
  // fetchedbuf_animApplyBuffer.snapshotTime_ms. however, it's eventually going
  // to be displayed in a historic position, for temporal smoothing. thus; read
  // out the current values we've just set, insert them in the temporal
  // smoother, and get the historic spatial data instead (GetValue).
  for (unsigned int i = 0; i < buf_TemporalHumanoidNodes.size(); i++) {
    ...
    // sudden realisation: by only SetValue'ing here instead of in
    // prepareputbuffers, aren't we missing out on valuable interpolateable
    // data?
    ...
    buf_TemporalHumanoidNodes.at(i).actualNode->SetPosition(
        buf_TemporalHumanoidNodes.at(i).position.GetValue(
            match->GetPreviousPutTime_ms()),
        false);
    buf_TemporalHumanoidNodes.at(i).actualNode->SetRotation(
        buf_TemporalHumanoidNodes.at(i).orientation.GetValue(
            match->GetPreviousPutTime_ms()),
        false);
  }
...
void HumanoidBase::CalculateGeomOffsets() {
  ...
  spatialState.position).GetNormalized(spatialState.directionVec);
    ...
    if (fabs(angle) < 0.8 * pi) {
      ...
      float bias = clamp(0.25f * pi - (fabs(mx) + fabs(my)) - 0.15f, 0, 0.25f *
  pi) * 0.4f; SetOffset("middle", bias, middleOrientation.GetNormalized());
    }
...
void HumanoidBase::SetOffset(const std::string &nodeName, float bias,
  ...
  if (iter == offsets.end()) {
    if (bias != 0) {
      ...
      offsets.insert(
          std::pair<std::string, BiasedOffset>(nodeName, biasedOffset));
    }
  } else {
    if (bias != 0) {
      BiasedOffset biasedOffset;
...
int HumanoidBase::GetIdleMovementAnimID() {
  ...
  SetBodyDirectionSimilarityPredicate(
      ...
      Vector3(0, -10, 0).GetRotated2D(spatialState.angle)); // lookat
#ifdef dataSetSortable
  dataSet.sort(
      boost::bind(&HumanoidBase::CompareBodyDirectionSimilarity, this, _1, _2));
#else
  std::stable_sort(
      dataSet.begin(), dataSet.end(),
      boost::bind(&HumanoidBase::CompareBodyDirectionSimilarity, this, _1, _2));
...
#ifdef dataSetSortable
  dataSet.sort(
      boost::bind(&HumanoidBase::CompareMovementSimilarity, this, _1, _2));
#else
  std::stable_sort(
      dataSet.begin(), dataSet.end(),
      boost::bind(&HumanoidBase::CompareMovementSimilarity, this, _1, _2));
...
void HumanoidBase::ResetPosition(const Vector3 &newPos,
  ...
  startPos = newPos;
  startAngle = FixAngle(
      (focusPos - newPos).GetNormalized(Vector3(0, -1, 0)).GetAngle2D());
  ...
  currentAnim->frameNum =
      random(0, currentAnim->anim->GetEffectiveFrameCount() - 1);
...
void HumanoidBase::TripMe(const Vector3 &tripVector, int tripType) {
  if (match->GetBallRetainer() == player)
    return;
  if (currentAnim->anim->GetVariable("incoming_special_state").compare("") ==
          0 &&
...
void HumanoidBase::SetKit(boost::intrusive_ptr<Resource<Surface>> newKit) {
  ...
  if (newKit != boost::intrusive_ptr<Resource<Surface>>()) {
    for (unsigned int i = 0; i < tmesh.size(); i++) {
      if (tmesh.at(i).material.diffuseTexture !=
          boost::intrusive_ptr<Resource<Surface>>()) {
        if (tmesh.at(i).material.diffuseTexture->GetIdentString() ==
            kitDiffuseTextureIdentString) {
          tmesh.at(i).material.diffuseTexture = newKit;
          tmesh.at(i).material.specular_amount =
              0.01f;                              // 0.02f;//0.033f;//0.01f;
...
bool HumanoidBase::_HighOrBouncyBall() const {
  ...
  float ballHeight2 =
      match->GetBall()->Predict(defaultTouchOffset_ms).coords[2];
  float ballBounce = fabs(match->GetBall()->GetMovement().coords[2]);
  bool highBall = false;
  if (ballHeight1 > 0.3f || ballHeight2 > 0.3f) {
    highBall = true;
  } else if (ballBounce >
             1.0f) { // low balls are also treated as 'high ball' when there's a
  ...
  while (iter != dataSet.end()) {
    ...
    if (strict) {
      if (atoi(anim->GetVariable("quadrant_id").c_str()) == bestQuadrantID) {
        // if (player->GetDebug() && bestAnim->GetAnimType().compare("movement")
        // == 0) printf("keeping %s\n", anim->GetName().c_str());
        iter++;
      } else {
        // if (player->GetDebug() && bestAnim->GetAnimType().compare("movement")
      ...
      if (anim->GetVariable("lastditch").compare("true") !=
        ...
        if (abs(GetVelocityID(quadrant.velocity, true) -
                GetVelocityID(bestQuadrant.velocity, true)) >
      ...
      if (predicate) {
        ...
        // if (player->GetDebug() && bestAnim->GetAnimType().compare("movement")
        // == 0) printf("deleting %s (quadrant.angle %f - bestQuadrant.angle
...
void HumanoidBase::_KeepBestBodyDirectionAnims(DataSet &dataSet,
  ...
  while (iter != dataSet.end()) {
    ...
    radian animOutgoingBodyAngle =
        ForceIntoAllowedBodyDirectionAngle(anim->GetOutgoingBodyAngle());
    radian animOutgoingAngle =
    ...
    if (fabs(animLookAngle - bestLookAngle) <= adaptedAllowedAngle) {
      // if (player->GetDebug() &&
      // bestAnim->GetAnimType().compare("ballcontrol") == 0) printf("keeping %s
      // (%f, %f (%f), %f)\n", anim->GetName().c_str(), animOutgoingBodyAngle,
      // animOutgoingAngle, anim->GetOutgoingAngle(), animLookAngle);
      iter++;
    } else {
      // if (player->GetDebug() &&
      // bestAnim->GetAnimType().compare("ballcontrol") == 0) printf("deleting
      // %s (%f, %f (%f), %f)\n", anim->GetName().c_str(),
      // animOutgoingBodyAngle, animOutgoingAngle, anim->GetOutgoingAngle(),
      // animLookAngle);
      iter = dataSet.erase(iter);
    }
  }

  // if (player->GetDebug() && bestAnim->GetAnimType().compare("ballcontrol") ==
  // 0) printf("\n");
}

/* deprecated
void HumanoidBase::GetRatedDataSet(RatedDataSet &ratedDataSet, const DataSet
&dataSet, const IdealAnimDescription &idealAnimDescription, float weight_foot,
float weight_incomingVelocity, float weight_outgoingMovement, float
weight_incomingBodyDirection, float weight_outgoingBodyDirection, float
weight_baseAnim, float weight_side, float penalty_nonLinearVelocity, float
penalty_nonLinearBodyDirection) { DataSet::const_iterator iter =
dataSet.begin();
  ...
  while (iter != dataSet.end()) {
    ...
    if (weight_foot > 0.0f) {
      if (anim->GetCurrentFoot() == idealAnimDescription.foot) rating_foot
= 1.0f; if (FloatToEnumVelocity(anim->GetIncomingVelocity()) == e_Velocity_Idle)
rating_foot = 1.0f; // actually: disable weighting?
    }
...
GetVelocityID(FloatToEnumVelocity(idealAnimDescription.incomingVelocityFloat));
      ...
      int anim_incomingVelocityID =
GetVelocityID(FloatToEnumVelocity(anim->GetIncomingVelocity()));
      rating_incomingVelocity = 1.0f -
pow(NormalizedClamp(fabs(anim_incomingVelocityID - currentVelocityID), 0, 3),
0.5f);

      // also add a penalty for anim incoming velocities which aren't between
actual incoming and anim outgoing
      // todo: also make variable ratings per amount of excess?
      int anim_outgoingVelocityID =
GetVelocityID(FloatToEnumVelocity(anim->GetOutgoingVelocity())); if
(anim_incomingVelocityID > std::max(currentVelocityID, anim_outgoingVelocityID))
rating_nonLinearVelocity -= penalty_nonLinearVelocity; if
(anim_incomingVelocityID < std::min(currentVelocityID, anim_outgoingVelocityID))
rating_nonLinearVelocity -= penalty_nonLinearVelocity;
    }
    ...
    if (weight_outgoingMovement > 0.0f) {
      ...
      rating_outgoingMovement = 1.0f -
pow(NormalizedClamp((idealAnimDescription.outgoingMovementRel -
...
FloatToEnumVelocity(anim->GetOutgoingVelocity())) rating_outgoingMovement *=
0.1f;
    }
...
/ pi, 0.5f); if (FloatToEnumVelocity(anim->GetIncomingVelocity()) ==
e_Velocity_Idle) rating_incomingBodyDirection = 1.0f;

      bool nonLinearCondition = false;
      // if anim incoming body dir != between (including) query incoming and
anim outgoing dir, then this anim may look annoying
      // how do we check this?
      // 1. if we look at the smallest angles between (anim incoming -> query
incoming) and (anim incoming -> anim outgoing), one has to be positive, the
other negative.
      // 2. the (absolute) angles added up have to be < pi radians. else, we
could be on the 'other side' of the 'virtual half circle' and still have the
former condition met radian marginRadians = 0.06f * pi; // anims can deviate a
few degrees from the desired (quantized) directions Vector3 incomingBodyDirRel =
anim->GetIncomingBodyDirection(); Vector3 outgoingBodyDirRel = Vector3(0, -1,
0).GetRotated2D(anim->GetOutgoingBodyAngle() + anim->GetOutgoingAngle()); radian
shortestAngle1 = incomingBodyDirRel.GetAngle2D(outgoingBodyDirRel); radian
shortestAngle2 =
incomingBodyDirRel.GetAngle2D(idealAnimDescription.incomingBodyDirectionRel); if
...
nonLinearCondition = true; else nonLinearCondition = false;
      }

      if (nonLinearCondition) rating_nonLinearBodyDirection -=
penalty_nonLinearBodyDirection;
    }
    ...
    Vector3 relDesiredBodyDirection; // possibly used later in rotational side
weighting if (weight_outgoingBodyDirection > 0.0f) { float translationFactor
= 1.0f; relDesiredBodyDirection = ((idealAnimDescription.desiredLookAtAbs -
spatialState.position).Get2D().GetRotated2D(-spatialState.angle) -
anim->GetTranslation() * translationFactor).GetNormalized(Vector3(0, -1, 0));
      //relDesiredBodyDirection =
ForceIntoAllowedBodyDirectionVec(relDesiredBodyDirection);
...
/ pi, 0.5f);
    ...
    if (weight_baseAnim > 0.0f) {
      if (anim->GetVariable("baseanim").compare("true") == 0) rating_baseAnim
= 1.0f;
    }
...
anim->GetOutgoingDirection().GetRotated2D(anim->GetOutgoingBodyAngle()); radian
animRotationAngle = animOutgoingBodyDir.GetAngle2D(animIncomingBodyDir);
      // desired direction
      radian desiredRotationAngle =
relDesiredBodyDirection.GetAngle2D(animIncomingBodyDir);

      //rating_side = 1.0f - (fabs(animRotationAngle - desiredRotationAngle) /
(2.0f * pi)); if (fabs(animRotationAngle) > 0.3f * pi &&
fabs(desiredRotationAngle) > 0.3f * pi && fabs(animRotationAngle -
desiredRotationAngle) > 0.06f) { rating_side = (signSide(animRotationAngle) ==
...
bool HumanoidBase::SelectAnim(
  ...
  if (command.useDesiredMovement) {
    ...
    SetMovementSimilarityPredicate(
        relDesiredDirection, FloatToEnumVelocity(adaptedDesiredVelocityFloat));
  ...
  std::stable_sort(
      dataSet.begin(), dataSet.end(),
      boost::bind(&HumanoidBase::CompareFootSimilarity, this, _1, _2));
#endif

  SetIncomingBodyDirectionSimilarityPredicate(spatialState.relBodyDirectionVec);
#ifdef dataSetSortable
  dataSet.sort(boost::bind(
      &HumanoidBase::CompareIncomingBodyDirectionSimilarity, this, _1, _2));
#else
  std::stable_sort(
      dataSet.begin(), dataSet.end(),
      boost::bind(&HumanoidBase::CompareIncomingBodyDirectionSimilarity, this,
                  _1, _2));
#endif

  SetIncomingVelocitySimilarityPredicate(spatialState.enumVelocity);
#ifdef dataSetSortable
  dataSet.sort(boost::bind(&HumanoidBase::CompareIncomingVelocitySimilarity,
                           this, _1, _2));
#else
  std::stable_sort(dataSet.begin(), dataSet.end(),
                   boost::bind(&HumanoidBase::CompareIncomingVelocitySimilarity,
                               this, _1, _2));
#endif
...
#ifdef dataSetSortable
    dataSet.sort(boost::bind(&HumanoidBase::CompareTripDirectionSimilarity,
                             this, _1, _2));
#else
    std::stable_sort(dataSet.begin(), dataSet.end(),
                     boost::bind(&HumanoidBase::CompareTripDirectionSimilarity,
                                 this, _1, _2));
#endif
...
#ifdef dataSetSortable
    dataSet.sort(
        boost::bind(&HumanoidBase::CompareBaseanimSimilarity, this, _1, _2));
#else
    std::stable_sort(
        dataSet.begin(), dataSet.end(),
        boost::bind(&HumanoidBase::CompareBaseanimSimilarity, this, _1, _2));
  ...
  // check if we really want to requeue - only requeue movement to movement, for
  // example, when we want to go a different direction
...
void HumanoidBase::CalculatePredictedSituation(Vector3 &predictedPos,
  ...
  if (currentAnim->positions.size() > (unsigned int)currentAnim->frameNum) {
    ...
    predictedPos = spatialState.position +
                   currentAnim->anim->GetTranslation().Get2D().GetRotated2D(
                       spatialState.angle) +
                   currentAnim->actionSmuggle +
                   currentAnim->actionSmuggleSustain +
                   currentAnim->movementSmuggle;
  }

  predictedAngle = spatialState.angle + currentAnim->anim->GetOutgoingAngle() +
                   currentAnim->rotationSmuggle.end;
  predictedAngle = ModulateIntoRange(-pi, pi, predictedAngle);
  assert(predictedPos.coords[2] == 0.0f);
}

Vector3 HumanoidBase::CalculateOutgoingMovement(
    const std::vector<Vector3> &positions) const {
  if (positions.size() < 2)
    return 0;
  return (positions.at(positions.size() - 1) -
          positions.at(positions.size() - 2)) *
         100.0f;
}
...
void HumanoidBase::CalculateSpatialState() {
  ...
  if (currentAnim->positions.size() > (unsigned int)currentAnim->frameNum) {
    ...
    currentAnim->anim->GetKeyFrame("player", currentAnim->frameNum, orientation,
                                   position);
    position.coords[2] = 0.0f;
    position = startPos + position.GetRotated2D(startAngle) +
               currentAnim->actionSmuggleOffset +
               currentAnim->actionSmuggleSustainOffset +
               currentAnim->movementSmuggleOffset;
  }
  ...
  spatialState.physicsMovement = spatialState.actualMovement -
                                 ...
                                 (spatialState.positionOffsetMovement *
                                  positionOffsetMovementIgnoreFactor);
  spatialState.animMovement = spatialState.physicsMovement;
  if (currentAnim->positions.size() > 0) {
    // this way, action cheating is being omitted from the current movement,
    // making for better requeues. however, keep in mind that movementoffsets,
    // from bumping into other players, for example, will also be ignored this
    // way.
    const std::vector<Vector3> &origPositionCache =
        match->GetAnimPositionCache(currentAnim->anim);
    spatialState.animMovement =
        CalculateMovementAtFrame(origPositionCache, currentAnim->frameNum, 1)
            .GetRotated2D(startAngle);
  }
  // if (player->GetDebug()) SetRedDebugPilon(position +
  // spatialState.physicsMovement); if (player->GetDebug())
  // SetGreenDebugPilon(position + spatialState.animMovement);
  ...
  Vector3 bodyDirectionVec = Vector3(0, -1, 0).GetRotated2D(
      z + startAngle + currentAnim->rotationSmuggleOffset);
  ...
  spatialState.angle = ModulateIntoRange(
      -pi, pi, FixAngle(spatialState.directionVec.GetAngle2D()));

  if (spatialState.enumVelocity != e_Velocity_Idle) {
    Vector3 adaptedBodyDirectionVec =
        bodyDirectionVec.GetRotated2D(-spatialState.angle);
    ...
    // adaptedBodyDirectionVec = (adaptedBodyDirectionVec * 0.95f + Vector3(0,
    // -1, 0) * 0.05f).GetNormalized(0);
    ...
    if (spatialState.enumVelocity == e_Velocity_Sprint &&
      ...
      if (preferCorrectVeloOverCorrectAngle) {
        // on impossible combinations of velocity and body angle, decrease body
        // angle
        adaptedBodyDirectionVec =
            Vector3(0, -1, 0).GetRotated2D(0.12f * pi * signSide(bodyAngleRel));
      } else {
        // on impossible combinations of velocity and body angle, decrease
        // velocity
        spatialState.floatVelocity = walkSprintSwitch - 0.1f;
        spatialState.enumVelocity =
            FloatToEnumVelocity(spatialState.floatVelocity);
      }
    ...
    spatialState.relBodyDirectionVec =
        ForceIntoAllowedBodyDirectionVec(adaptedBodyDirectionVec);
  } else {
  ...
  spatialState.relBodyAngleNonquantized =
      spatialState.relBodyDirectionVecNonquantized.GetAngle2D(
          Vector3(0, -1, 0));
  spatialState.bodyDirectionVec = spatialState.relBodyDirectionVec.GetRotated2D(
      spatialState
          .angle); // rotate back, we now have it forced into allowed angle
...
void HumanoidBase::CalculateFactualSpatialState() {
  ...
  if (currentAnim->anim->GetVariable("outgoing_special_state").compare("") !=
      0) {
    spatialState.floatVelocity = 0;
    spatialState.enumVelocity = e_Velocity_Idle;
    spatialState.movement = Vector3(0);
...
PlayerCommand HumanoidBase::GetTripCommand(const Vector3 &tripVector,
  ...
  command.desiredVelocityFloat =
      spatialState.floatVelocity; // e_Velocity_Sprint;
...
PlayerCommand
HumanoidBase::GetBasicMovementCommand(const Vector3 &desiredDirection,
                                      float velocityFloat) {
  PlayerCommand command;
  ...
  command.desiredLookAt =
      spatialState.position + command.desiredDirection * 10.0f;
  return command;
}
...
bool HumanoidBase::CompareFootSimilarity(int animIndex1, int animIndex2) const {
  ...
  if (anims->GetAnim(animIndex2)->GetCurrentFoot() == predicate_DesiredFoot)
    two = 0;
  if (FloatToEnumVelocity(anims->GetAnim(animIndex1)->GetIncomingVelocity()) ==
      e_Velocity_Idle)
    one = 0;
  if (FloatToEnumVelocity(anims->GetAnim(animIndex2)->GetIncomingVelocity()) ==
      e_Velocity_Idle)
    two = 0;
  return one < two;
}

void HumanoidBase::SetIncomingVelocitySimilarityPredicate(
    e_Velocity velocity) const {
  predicate_IncomingVelocity = velocity;
}

bool HumanoidBase::CompareIncomingVelocitySimilarity(int animIndex1,
                                                     int animIndex2) const {
  /* old version
  float rating1 =
  fabs(clamp(RangeVelocity(anims->GetAnim(animIndex1)->GetIncomingVelocity()) -
  EnumToFloatVelocity(predicate_IncomingVelocity), -sprintVelocity,
  sprintVelocity)); float rating2 =
  ...
  */
  ...
  // rate difference anim incoming / actual incoming
  int anim1_incomingVelocityID = GetVelocityID(
      FloatToEnumVelocity(anims->GetAnim(animIndex1)->GetIncomingVelocity()));
  int anim2_incomingVelocityID = GetVelocityID(
      FloatToEnumVelocity(anims->GetAnim(animIndex2)->GetIncomingVelocity()));
  float rating1 =
      fabs(clamp(anim1_incomingVelocityID - currentVelocityID, -3, 3));
  float rating2 =
      fabs(clamp(anim2_incomingVelocityID - currentVelocityID, -3, 3));

  // also add a penalty for anim incoming velocities which aren't between actual
  // incoming and anim outgoing
  int anim1_outgoingVelocityID = GetVelocityID(
      FloatToEnumVelocity(anims->GetAnim(animIndex1)->GetOutgoingVelocity()));
  int anim2_outgoingVelocityID = GetVelocityID(
      FloatToEnumVelocity(anims->GetAnim(animIndex2)->GetOutgoingVelocity()));
  if (anim1_incomingVelocityID >
      std::max(currentVelocityID, anim1_outgoingVelocityID))
    rating1 += 0.5f;
  if (anim1_incomingVelocityID <
      std::min(currentVelocityID, anim1_outgoingVelocityID))
    rating1 += 0.5f;
  if (anim2_incomingVelocityID >
      std::max(currentVelocityID, anim2_outgoingVelocityID))
    rating2 += 0.5f;
  if (anim2_incomingVelocityID <
      std::min(currentVelocityID, anim2_outgoingVelocityID))
    rating2 += 0.5f;

  return rating1 < rating2;
}

void HumanoidBase::SetMovementSimilarityPredicate(
    const Vector3 &relDesiredDirection, e_Velocity desiredVelocity) const {
  predicate_RelDesiredDirection = relDesiredDirection;
  predicate_DesiredVelocity = desiredVelocity;
  // if (relDesiredDirection.GetDotProduct(Vector3(0, -1, 0)) < 0)
  // predicate_DesiredVelocity = e_Velocity_Idle;
  //  this isn't working all too well: if targetmovement is set to 0 (aka
  //  corneringbias towards 1), both dribble @ 0 deg and dribble @ 90 deg will
  //  be the same distance (from 0), so still no preference for braking straight
  predicate_CorneringBias = CalculateBiasForFastCornering(
      Vector3(0, -1.0f * spatialState.floatVelocity, 0),
      relDesiredDirection * EnumToFloatVelocity(desiredVelocity), 1.0f,
      0.9f); // anim space values!
}

float HumanoidBase::GetMovementSimilarity(int animIndex,
                                          const Vector3 &relDesiredDirection,
                                          e_Velocity desiredVelocity,
                                          float corneringBias) const {

  Vector3 desiredMovement =
      relDesiredDirection * EnumToFloatVelocity(desiredVelocity);

  Vector3 outgoingDirection = ForceIntoPreferredDirectionVec(
      anims->GetAnim(animIndex)->GetOutgoingDirection());
  float outgoingVelocity =
      RangeVelocity(anims->GetAnim(animIndex)->GetOutgoingVelocity());
  Vector3 outgoingMovement = outgoingDirection * outgoingVelocity;

  // anims that end at lower velocities have an advantage: they don't get
  // dragged into the currentmovement that much thus: have a bias that is higher
  // at higher outgoing velocities, which means anim outgoingmovement gets more
  // % of current movement and less % of their own disabled for now - the new
  // physics system disregards most of the anims movement anyway :) *enabled
  // again: altered the physics system so that it will regard anim movement
  // more, so this became useful again for proper hard cornering

  // alternative to this system (drags back movement)
  // outgoingMovement += -desiredMovement.GetNormalized(0) *
  // outgoingMovement.GetLength() * 0.5f; desiredMovement =
  // desiredMovement.GetNormalized(0) * clamp(desiredMovement.GetLength() -
  // 0.8f, idleVelocity, sprintVelocity);
  ...
  /* disabled quantization:
    // maximum quantization (20 degree angle @ dribble velocity, smallest
    distance to be measured)
    // faulty calculation (cornering distance instead of straight line) for
    optimization, makes little difference anyway for 20 degrees float maxQuant =
    (dribbleVelocity * 2.0f * pi) / (360.0f / 20.0f);// * (1.0 - velocityBias *
    0.5) // == ~1.22 at the moment of writing
    //float maxQuant = sqrt(2.0f * pow(dribbleVelocity, 2.0f) * (1.0f - cos(pi /
    180.0f * 20.0f))); // the actual correct calculation (straight line)
  ...
  */
  ...
  // never turn the wrong way around ** BUGGY, causes weird acceleration when we
  // want idle velo **
  // if (fabs(outgoingDirection.GetAngle2D() - relDesiredDirection.GetAngle2D())
  // > pi) value += 100000;

  return value;
}

bool HumanoidBase::CompareMovementSimilarity(int animIndex1,
                                             int animIndex2) const {
  float rating1 =
      GetMovementSimilarity(animIndex1, predicate_RelDesiredDirection,
                            predicate_DesiredVelocity, predicate_CorneringBias);
  float rating2 =
      GetMovementSimilarity(animIndex2, predicate_RelDesiredDirection,
                            predicate_DesiredVelocity, predicate_CorneringBias);
  return rating1 < rating2;
}

bool HumanoidBase::CompareDirectionSimilarity(int animIndex1,
                                              int animIndex2) const {
  float rating1 = fabs(predicate_RelDesiredDirection.GetAngle2D(
      anims->GetAnim(animIndex1)->GetOutgoingDirection()));
  float rating2 = fabs(predicate_RelDesiredDirection.GetAngle2D(
      anims->GetAnim(animIndex2)->GetOutgoingDirection()));
  return rating1 < rating2;
}

bool HumanoidBase::CompareOutgoingVelocitySimilarity(int animIndex1,
                                                     int animIndex2) const {
  float rating1 = fabs(
      clamp(RangeVelocity(anims->GetAnim(animIndex1)->GetOutgoingVelocity()) -
                EnumToFloatVelocity(predicate_DesiredVelocity),
            -sprintVelocity, sprintVelocity));
  float rating2 = fabs(
      clamp(RangeVelocity(anims->GetAnim(animIndex2)->GetOutgoingVelocity()) -
                EnumToFloatVelocity(predicate_DesiredVelocity),
            -sprintVelocity, sprintVelocity));
  return rating1 < rating2;
}
...
bool HumanoidBase::CompareIncomingBodyDirectionSimilarity(
    int animIndex1, int animIndex2) const {
  float rating1 =
      fabs(ForceIntoAllowedBodyDirectionVec(
               anims->GetAnim(animIndex1)->GetIncomingBodyDirection())
               .GetAngle2D(ForceIntoAllowedBodyDirectionVec(
                   predicate_RelIncomingBodyDirection))) /
      pi;
  float rating2 =
      fabs(ForceIntoAllowedBodyDirectionVec(
               anims->GetAnim(animIndex2)->GetIncomingBodyDirection())
               .GetAngle2D(ForceIntoAllowedBodyDirectionVec(
                   predicate_RelIncomingBodyDirection))) /
      pi;
  if (FloatToEnumVelocity(anims->GetAnim(animIndex1)->GetIncomingVelocity()) ==
...
void HumanoidBase::SetBodyDirectionSimilarityPredicate(
    const Vector3 &lookAt) const {
  predicate_LookAt = lookAt;
}

bool HumanoidBase::CompareBodyDirectionSimilarity(int animIndex1,
                                                  int animIndex2) const {
  ...
  Vector3 relDesiredBodyDirection1 = ((predicate_LookAt - spatialState.position)
                                          .GetRotated2D(-spatialState.angle) -
                                      a1->GetTranslation() * translationFactor)
                                         .GetNormalized(Vector3(0, -1, 0));
  Vector3 relDesiredBodyDirection2 = ((predicate_LookAt - spatialState.position)
                                          .GetRotated2D(-spatialState.angle) -
                                      a2->GetTranslation() * translationFactor)
                                         .GetNormalized(Vector3(0, -1, 0));

  /* this is quirky somehow; when accelerating, sometimes the humanoid will
  actually go 135 when it shouldn't. could debug it, but then again, maybe KISS
  is a better idea; this whole setup seems a bit overly complicated. probably
  should look for solution elsewhere
  ...
  // doesn't work when ending idle
    ...
    if (anims->GetAnim(animIndex1)->GetOutgoingVelocity() > idleDribbleSwitch &&
      ...
      relDesiredBodyDirection1 = Vector3(0, -1, 0).GetRotated2D(d1 * (1.0f -
  origAngleBias) + a1 * origAngleBias);
      ...
      relDesiredBodyDirection2 = Vector3(0, -1, 0).GetRotated2D(d2 * (1.0f -
  origAngleBias) + a2 * origAngleBias);
    }
  }
  */

  // this version corrects for rotation smuggle; we will probably end up walking
  // in a direction a bit rotated towards the user desired direction, instead of
  // pure anim direction. we need some heuristic to use that fact, because else,
  // we may end up looking in the wrong direction somewhat radian outgoingAngle1
  // = FixAngle(((a1->GetOutgoingDirection() + predicate_RelDesiredDirection *
  ...
  radian maxAngleSmuggle =
      0.1f *
      pi; // mind you, for anims ending idle, this shouldn't be >= 0.125f * pi,
          // because then we end up allowing multiple outgoing angles. after
          // all, outgoing direction will be skewed by this value as max, which
          // will make outgoing body direction follow in the same dir but since
          // idle anims don't have outgoing body directions, it will just change
          // their outgoing directions and they will be pointing in exactly the
          // same direction
  radian outgoingAngle1 =
      a1->GetOutgoingDirection()
          .GetRotated2D(clamp(predicate_RelDesiredDirection.GetAngle2D(
                                  a1->GetOutgoingDirection()),
  ...
  radian rating1 = fabs(
      predictedOutgoingBodyDirection1.GetAngle2D(relDesiredBodyDirection1));
  radian rating2 = fabs(
      predictedOutgoingBodyDirection2.GetAngle2D(relDesiredBodyDirection2));

  // penalty for body angles (as opposed to straight forward), to get a slight
  // preference for forward angles
...
bool HumanoidBase::CompareTripDirectionSimilarity(int animIndex1,
                                                  int animIndex2) const {
  float rating1 = -GetVectorFromString(
                       anims->GetAnim(animIndex1)->GetVariable("bumpdirection"))
                       .GetDotProduct(predicate_RelDesiredTripDirection);
  float rating2 = -GetVectorFromString(
                       anims->GetAnim(animIndex2)->GetVariable("bumpdirection"))
                       .GetDotProduct(predicate_RelDesiredTripDirection);
  return rating1 < rating2;
}
...
bool HumanoidBase::CompareBallDirectionSimilarity(int animIndex1,
                                                  int animIndex2) const {
  float rating1 = -GetVectorFromString(
                       anims->GetAnim(animIndex1)->GetVariable("balldirection"))
                       .Get2D()
                       .GetDotProduct(predicate_RelDesiredBallDirection);
  float rating2 = -GetVectorFromString(
                       anims->GetAnim(animIndex2)->GetVariable("balldirection"))
                       .Get2D()
                       .GetDotProduct(predicate_RelDesiredBallDirection);
  return rating1 < rating2;
}

bool HumanoidBase::CompareBaseanimSimilarity(int animIndex1,
                                             int animIndex2) const {
  bool isBase1 =
      (anims->GetAnim(animIndex1)->GetVariable("baseanim").compare("true") ==
       0);
  bool isBase2 =
      (anims->GetAnim(animIndex2)->GetVariable("baseanim").compare("true") ==
       0);
...
bool HumanoidBase::CompareCatchOrDeflect(int animIndex1, int animIndex2) const {
  bool catch1 = (anims->GetAnim(animIndex1)
                     ->GetVariable("outgoing_retain_state")
                     .compare("") != 0);
  bool catch2 = (anims->GetAnim(animIndex2)
                     ->GetVariable("outgoing_retain_state")
                     .compare("") != 0);
...
void HumanoidBase::SetNumericVariableSimilarityPredicate(
    const std::string &varName, float desiredValue) const {
  predicate_NumericVariableName = varName;
  predicate_NumericVariableValue = desiredValue;
}

bool HumanoidBase::CompareNumericVariable(int animIndex1,
                                          int animIndex2) const {
...
Vector3 HumanoidBase::CalculatePhysicsVector(
    Animation *anim, bool useDesiredMovement, const Vector3 &desiredMovement,
    bool useDesiredBodyDirection, const Vector3 &desiredBodyDirectionRel,
    std::vector<Vector3> &positions_ret, radian &rotationOffset_ret) const {
  ...
  float incomingSwitchBias =
      0.0f; // anything other than 0.0 may result in unpuristic behavior
  ...
  if (animType.compare("ballcontrol") == 0) {
    ...
    outgoingSwitchBias = 0.5f; // direction partly predecided by collision
                               // function in match class
  } else if (touch) {
    outgoingSwitchBias = 1.0f;
  }
  ...
  float difficultyFactor =
      atof(anim->GetVariable("animdifficultyfactor").c_str());
  ...
  float powerFactor =
      1.0f - clamp(std::pow(player->GetLastTouchBias(1000), 0.8f) *
                       (0.8f - stat_dribble * 0.3f),
                   0.0f, 0.4f); // todo: put lasttouchbias thing in loop, so
                                // it'll change over time (in that loop)
  // moved to per ms timeloop penalty
  powerFactor *=
      1.0f -
      clamp(decayingPositionOffset.GetLength() *
  ...
  if (touch) {
    ...
    bonus *=
        0.6f +
        0.4f * player->GetStat(
                   "technical_ballcontrol"); // todo: shouldn't this be agility?
  ...
  if (mod_AllowRotation && physicsBias > 0.0f) {
    ...
    if (FloatToEnumVelocity(predictedOutgoingMovement.GetLength()) ==
        e_Velocity_Idle)
      animOutgoingVector =
          anim->GetOutgoingDirection().GetRotated2D(spatialState.angle);
    Vector3 desiredVector = adaptedDesiredMovement.GetNormalized(0);
    if (FloatToEnumVelocity(adaptedDesiredMovement.GetLength()) ==
        e_Velocity_Idle)
    ...
    if (fabs(toDesiredAngle) <= 0.5f * pi ||
        animType.compare("sliding") ==
            0) { // if we want > x degrees, just skip it to next anim, it'll
                 // only look weird otherwise

      radian animChange =
          animOutgoingVector.GetAngle2D(spatialState.directionVec);
      if (fabs(animChange) > 0.06f * pi) {
        int sign = signSide(animChange);
        if (signSide(toDesiredAngle) == sign) {
          toDesiredAngle_capped =
              clamp(toDesiredAngle, -maxAngleMod_overAnimAngle,
                    maxAngleMod_overAnimAngle);
        } else {
          toDesiredAngle_capped =
              clamp(toDesiredAngle, -maxAngleMod_underAnimAngle,
                    maxAngleMod_underAnimAngle);
        }
      } else {
        // straight ahead anim, has no specific side.
        toDesiredAngle_capped =
            clamp(toDesiredAngle, -maxAngleMod_straightAnimAngle,
                  maxAngleMod_straightAnimAngle);
      }
      // printf("animChange: %f radians, to desired angle: %f radians, sign: %i,
      // sign side of to desired angle: %i, resulting capped angle: %f
      // radians\n", animChange, toDesiredAngle, sign, signSide(toDesiredAngle),
      // toDesiredAngle_capped);
    }
  }
  ...
  for (int time_ms = 0; time_ms < anim->GetFrameCount() * 10;
    ...
    // 'spiritually' the one-to-last, since the first frame of the next anim is
    // actually 'same-tempered' as the current anim's last frame. however, it
    // works best to have all values 'done' at this one-to-last frame, so the
    // next anim can read out these correct (new starting) values.
    float frameBias =
        (time_ms + 10) / (float)((anim->GetEffectiveFrameCount() + 1) * 10);

    float lagExp = 1.0f;
    if (mod_PointinessCurve && physicsBias > 0.0f &&
        (animType.compare("ballcontrol") == 0 ||
         animType.compare("movement") == 0)) {
      ...
      if (touch && time_ms < animTouchFrame * 10)
        lagExp =
            std::max(lagExp, 0.7f); // else, we could 'miss' the ball because
                                    // we're already turned around too much
    ...
    if (animVelo > walkSprintSwitch && (animType.compare("movement") == 0 ||
                                        animType.compare("ballcontrol") == 0 ||
                                        animType.compare("trap") == 0)) {

      if (maxVelocity >
          animVelo) { // only speed up, don't slow down. may be faster parts
                      // (jumps and such) within anim, allow this
        adaptedAnimVelo =
            StretchSprintTo(animVelo, animSprintVelocity, maxVelocity);
        adaptedAnimMovement =
            adaptedAnimMovement.GetNormalized(0) * adaptedAnimVelo;
      }
    }

    float maxSlower =
        1.6f; // rationale: don't want to end up below dribbleVelocity -
    ...
    if (temporalMovement.GetLength() > adaptedAnimVelo)
      maxFaster = std::min(
          ...
          std::max(maxFaster, temporalMovement.GetLength() -
                                  adaptedAnimVelo)); // ..already going faster..
                                                     // well okay, allow this
    if (maxFaster > 0)
      maxFaster *= std::max(
          0.0f, adaptedAnimMovement.GetNormalizedMax(1.0f).GetDotProduct(
    ...
    adaptedAnimVelo = clamp(desiredVelocity, adaptedAnimVelo - maxSlower,
    ...
    if (mod_CorneringBraking) {
      float frameBiasedMaximumOutgoingVelocity =
          sprintVelocity * (1.0f - frameBias) +
          maximumOutgoingVelocity * frameBias;
    ...
    if (mod_MaximumAccelDecel) {
      // this is basically meant to enforce transitions to be smoother,
      // disallowing bizarre steps. however, with low enough max values, it can
      // also serve as a physics slowness thing. in that regard, the maxaccel
      // part is somewhat similar to the air resistance mod below. they can live
      // together; this one can serve as a constant maximum, and the air
      // resistance as a velocity-based maximum. update: decided to not let them
      // live together, this should now purely be used for capping transition
      // speed
      float maxAccelMPS = 20.0f;
      float maxDecelMPS = 20.0f;
      float currentVelo = temporalMovement.GetLength();
      float veloChangeMPS =
          (adaptedAnimVelo - currentVelo) / ((float)timeStep_ms * 0.001f);
      if (veloChangeMPS < -maxDecelMPS || veloChangeMPS > maxAccelMPS) {
        adaptedAnimVelo =
            currentVelo + clamp(veloChangeMPS, -maxDecelMPS, maxAccelMPS) *
    ...
    if (mod_BrakeOnTouch) {
      ...
      // (precalc at touchframe, because temporalMovement will change because of
      // this, so if we don't precalc then changing numBrakeFrames will change
      ...
      if (touch && time_ms >= animTouchFrame * 10 &&
        ...
        float touchDifficultyFactor =
            clamp((difficultyFactor + 0.7f) * (1.0f - stat_dribble * 0.4f), 0.0,
                  1.0f);
        ...
        float veloFactor = NormalizedClamp(temporalMovement.GetLength(),
                                           walkVelocity, sprintVelocity);

        penaltyBreakFactor = touchDifficultyFactor * veloFactor *
                             brakeFrameFactor * touchBrakeFactor;
      }
    }
    ...
    numBrakeFrames) * 10) { int framesInto = (time_ms - (animTouchFrame * 10)) /
    10; toDesired += -temporalMovement.GetNormalized(0) *
    (ballTouchSlowdownAmount / (float)(numBrakeFrames - framesInto));
      // if (player->GetDebug()) {
      //   printf("divisor: %i, frame %i, time: %i, vector: ", numBrakeFrames,
    animTouchFrame, time_ms);
    ...
    */
    ...
    if (mod_MaxCornering) {
      ...
      if (temporalMovement.GetLength() > startVelo &&
        ...
        radian angle = predictedMovement.GetNormalized().GetAngle2D(
            temporalMovement.GetNormalized());
        ...
        if (fabs(angle) > maxAngle) {
          ...
          if (mode == 0) {
            Vector3 restrictedPredictedMovement =
                predictedMovement.GetRotated2D((fabs(angle) - maxAngle) *
                                               -signSide(angle));
            Vector3 newToDesired =
                restrictedPredictedMovement - temporalMovement;
            toDesired = newToDesired;
          } else if (mode == 1) {
            radian overAngle = fabs(angle) - maxAngle; // > 0
            toDesired += -temporalMovement *
                         clamp(overAngle / pi * 3.0f, 0.0f, 1.0f); // was: 3
          }
        }
      }
    }
    ...
    if (mod_MaxChange) {
      ...
      // maxChange *= 0.3f + 0.7f * curve(NormalizedClamp(time_ms,
      // 0.0f, 80.0f), 1.0f);
      float veloFactor = std::pow(
          NormalizedClamp(temporalMovement.GetLength(), 0, sprintVelocity),
          1.5f);
    ...
    if (mod_AirResistance && animType.compare("sliding") != 0 &&
        animType.compare("deflect") != 0) {
      float veloExp = 1.8f;
      ...
      if ((temporalMovement + toDesired).GetLength() > falloffStartVelo) {
        ...
        // wolfram alpha to show difference in sin before/after exp order: 10 *
        // (sin(((1.0 - x ^ 2.0) - 0.5) * pi) * 0.5 + 0.5), 10 * ((1.0 - (sin((x
        // - 0.5) * pi) * 0.5 + 0.5)) ^ 2.0) | from x = 0.0 to 1.0
        ...
        // veloAirResistanceFactor = 1.0f - ((1.0f - veloAirResistanceFactor) *
        // pow(sin(frameBias * pi), 2.0f));
        ...
        float maxAccelerationMPS = accelPower *
                                   (1.0f - veloAirResistanceFactor) *
                                   (stat_acceleration * 0.3f + 0.7f);
        float maxAccelerationAddition =
            maxAccelerationMPS * (timeStep_ms / 1000.0f);
        if (accelerationAddition > maxAccelerationAddition) {
          float remainingFactor =
              maxAccelerationAddition / accelerationAddition;
    ...
    // MAKE IT SEW!
    // http://static.wixstatic.com/media/fc58ad_c0ef2d69d98f4f7ba8e7e488f0e28ece.jpg
    ...
    if (time_ms >= (anim->GetFrameCount() - 2) * 10) {
      ...
      if (!hardQuantize) {
        // soft version
        if (FloatToEnumVelocity(anim->GetOutgoingVelocity()) ==
                e_Velocity_Idle &&
            FloatToEnumVelocity(tmpTemporalMovement.GetLength()) !=
                e_Velocity_Idle)
          tmpTemporalMovement.NormalizeTo(idleDribbleSwitch - 0.01f);
        else if (FloatToEnumVelocity(anim->GetOutgoingVelocity()) !=
                     e_Velocity_Idle &&
                 FloatToEnumVelocity(tmpTemporalMovement.GetLength()) ==
                     e_Velocity_Idle)
  ...
  if (FloatToEnumVelocity(anim->GetOutgoingVelocity()) != e_Velocity_Idle &&
    ...
    rotationOffset_ret = resultingMovement.GetRotated2D(-spatialState.angle)
                             .GetAngle2D(anim->GetOutgoingMovement());
  } else {
  ...
  if (mod_CheatBodyDirection && useDesiredBodyDirection &&
    ...
    radian predictedAngleRel = anim->GetOutgoingAngle() +
                               anim->GetOutgoingBodyAngle() +
                               rotationOffset_ret;
    radian desiredRotationOffset =
        desiredBodyDirectionRel.GetRotated2D(-predictedAngleRel)
            .GetAngle2D(Vector3(0, -1, 0));

    if (fabs(desiredRotationOffset) < 0.5f * pi) { // else: too much

      float outgoingVelocityFactorInv =
          1.0f - NormalizedClamp(resultingMovement.GetLength(),
                                 idleDribbleSwitch, sprintVelocity - 1.0f) *
                     1.0f;
      float animLengthFactor = NormalizedClamp(anim->GetFrameCount(), 0, 25);
      radian maximizedRotationOffset =
          clamp(desiredRotationOffset,
                outgoingVelocityFactorInv * animLengthFactor * angleFactor *
                    -maxAngle,
                outgoingVelocityFactorInv * animLengthFactor * angleFactor *
                    maxAngle);
      ...
      //   resultingMovement.GetNormalized(0) * 3.0f);
      //   SetGreenDebugPilon(spatialState.position + positions_ret.back() +
      //   Vector3(0, -1, 0).GetRotated2D(spatialState.angle + predictedAngleRel
      //   + rotationOffset) * 3.1f); SetRedDebugPilon(spatialState.position +
      //   positions_ret.back() + Vector3(0, -1,
      //   0).GetRotated2D(spatialState.angle + predictedAngleRel +
      //   rotationOffset + maximizedRotationOffset) * 3.2f);
      // }
      rotationOffset_ret += maximizedRotationOffset;

      // if (player->GetDebug() && animType.compare("movement") == 0) {
      //   SetYellowDebugPilon(spatialState.position +
      //   Vector3(GetDirectionVec()) * 3.0f);
      //   SetGreenDebugPilon(spatialState.position +
      //   Vector3(GetDirectionVec().GetRotated2D(maximizedRotationOffset))
      //   * 3.0f);
      // }

      // mee bezig. zie ook eerder in deze functie, moet ook aangepast, maximum
      // angle shizzle, wil niet over 180 graden if (fabs(predictedAngleRel +
      // rotationOffset) > 0.75f * pi && SignSide(maximizedRotationOffset) ==
      // SignSide(predictedAngleRel)) {
      // }
    }
  }
...
Vector3
HumanoidBase::ForceIntoAllowedBodyDirectionVec(const Vector3 &src) const {
