...
unsigned int
CalculateTimeNeededToChangeMovement_ms(const Vector3 &currentMovement,
                                       const Vector3 &desiredMovement) {

  // this function is quick 'n dirty, just how i like it
  // just don't expect exact results

  int time_ms = NormalizedClamp((desiredMovement - currentMovement).GetLength(),
                                idleVelocity, sprintVelocity * 2.0f) *
                1000;

  float currentVeloFactor =
      1.0f - NormalizedClamp(currentMovement.GetLength(), idleVelocity,
                             sprintVelocity);
...
float CalculateBiasForFastCornering(const Vector3 &currentMovement,
  ...
  radian angle = desiredMovement.GetNormalized(currentMovement)
                     .GetAngle2D(currentMovement);
  // wolfram alpha: sin(x - 0.5 * pi) * 0.5 + 0.5 | from x = 0.0 to pi
  float currentMovementBias =
      sin(fabs(angle) - 0.5f * pi) * 0.5f +
      0.5f; // this one is better for anim selection, else 135 anim is never
            // used (since 135 @ idle is worse than 90 @ dribble then - with
            // this one, 90 @ idle will be preferred (which will allow 135 @
            // idle as well))
  // float currentMovementBias = sin(2.0f * fabs(angle) - 0.5f * pi) * 0.5f +
  // 0.5f; // this one seems more correct puristically though.. hmm. maybe make
  // a 135 anim that ends in dribble? or would that effectively disable the 180
  // @ idle?

  // effect is less pronounced at low velocities
  float velocityBias = pow(
      clamp(currentMovement.GetLength() / (sprintVelocity - 0.5f), 0.0f, 1.0f),
      veloPow);
...
Vector3 CalculateMovementAtFrame(const std::vector<Vector3> &positions,
  ...
  */
  ...
  // special case: want the exit movement to be unsmoothed, for we don't want
  // the wrong quantized velocity and such
  ...
  // special case: if we want the movement at frame 0, we can't get -1 to 0, we
  // need to get 0 to 1 instead
  if (frameNum == 0) {
  ...
  for (int frame = (signed int)(frameNum - smoothFrames);
    ...
    if (frame > 0 &&
        frame < (signed int)positions
                    .size()) { // was: frame > 1 (i think that was a bug)
      totalMovement +=
          (positions.at(frame) - positions.at(frame - 1)).Get2D() * 100.0f;
      count++;
    }
  }
  if (count > 0) {
...
Vector3 GetFrontOfFootOffsetRel(float velocity, radian bodyAngleRel,
  ...
  float distance =
      0.34f + velocity * defaultTouchOffset_ms * 0.001f *
...
bool NeedDefendingMovement(int mySide, const Vector3 &position,
  ...
  float howDeepIsTarget =
      std::max((target.coords[0] - position.coords[0]) * -mySide, 0.0f);
...
void GetDifficultyFactors(Match *match, Player *player,
  ...
  ballMovementFactor =
      0.0f; // how much of the current ball movement is maintained
  // being pushed is tough
  float positionOffsetPenalty =
      NormalizedClamp(positionOffset.GetLength(), 0.0f, 0.1f) *
      2.0f; // was: 7.0f
  ...
  if (Verbose())
    printf("distanceFactor: positionoffset: %f, ballbodyvelodiff: %f, "
           "fartheraway: %f, ",
  ...
  if (match->GetLastTouchTeamID() != player->GetTeam()->GetID()) {
    Player *lastTouchPlayer =
        match->GetTeam(abs(player->GetTeam()->GetID() - 1))
            ->GetLastTouchPlayer();
    if (lastTouchPlayer) {
  ...
  if (Verbose())
    printf("skill multiplier: %f.. makes for result: distancefactor: %f; "
           "heightfactor: %f; ballmovementfactor: %f\n",
           skillPenaltyMultiplier, distanceFactor, heightFactor,
           ballMovementFactor);
}
...
Vector3 GetBallControlVector(Ball *ball, Player *player,
  ...
  // part of the resulting direction is physics, the other part is
  // anim/controller. so originatingBias is only applied on the (1.0 -
  // physicsBias) part of the result
  ...
  float dotFactor1 = spatialState.directionVec.GetDotProduct(
  ...
  // hold velo to max player velo - else slow players will kick the ball too far
  // away, which is sad
  if (desiredMovement.GetLength() > walkSprintSwitch)
    desiredMovement = desiredMovement.GetNormalized(0) *
                      StretchSprintTo(desiredMovement.GetLength(),
                                      sprintVelocity, player->GetMaxVelocity());

  // don't want to go below physics velo, else we will run over ball and stuff
  // like that don't want to go over physics velo too much either, else ball
  // will be so far away!
  float velocity =
      clamp(desiredMovement.GetLength(), outgoingMovement.GetLength(),
            outgoingMovement.GetLength() + maximumOverdrive_mps);
  desiredMovement = desiredMovement.GetNormalized(
                        Vector3(0, -1, 0).GetRotated2D(nextStartAngle)) *
                    velocity;
  Vector3 physicsMovement =
      Vector3(0, -1, 0).GetRotated2D(nextStartAngle) * velocity;

  float desiredVelocity = desiredMovement.GetLength();
  float physicsVelocity = physicsMovement.GetLength();

  Vector3 FFOsrc = GetFrontOfFootOffsetRel(physicsVelocity,
                                           nextBodyAngle - spatialState.angle,
                                           ball->Predict(0).coords[2]);
  float annoyanceVeloFactor =
      curve(NormalizedClamp(currentAnim->anim->GetOutgoingVelocity(),
                            idleVelocity, sprintVelocity),
            0.7f); // do not apply effect to low velo's; makes it too chaotic
  float opponentAnnoyanceFactor =
      (1.0f -
       NormalizedClamp(player->GetClosestOpponentDistance(), 0.5f, 1.5f)) *
      (1.0f - (player->GetStat("mental_calmness") * 0.5f +
               player->GetStat("physical_balance") * 0.3f)) *
      annoyanceVeloFactor;
  Vector3 FFO =
      Vector3(0, -1, 0).GetRotated2D(nextBodyAngle) *
      (FFOsrc.GetLength() + ffoOffset +
       opponentAnnoyanceFactor * 3.0f); // positionOffset is already in
                                        // ffoOffset (though only for trap atm)
  float heightFFOOffset =
      NormalizedClamp(ball->Predict(0).coords[2], 0.5f, 1.0f) *
      0.5f; // bounce high balls off body - else they keep colliding inside body
            // and stuff like that
  ...
  // we actually want the ball to be further away than our next position - after
  // all, we want to hit it again a few steps later
  ...
  Vector3 plannedBallPos = physicsPlannedBallPos * physicsBias +
                           desiredPlannedBallPos * (1.0f - physicsBias);
  Vector3 toPlannedBall = plannedBallPos - ball->Predict(0).Get2D();

  float timeToGo =
      ((currentAnim->anim->GetEffectiveFrameCount() - frameNum) * 10) * 0.001f;
  timeToGo +=
      physicsDelayTime * physicsBias + desiredDelayTime * (1.0f - physicsBias);
  timeToGo +=
      defaultTouchOffset_ms *
      0.001f; // 0.08f; // time into next anim where we want to hit the ball

  float divisor =
      timeToGo * (0.38f + 0.02f * player->GetStat(
                                      "technical_dribble")); // higher == closer
  divisor *= 1.1f;

  // to get the ball to the planned position in timeToGo seconds, we need to do
  // some pow() magic, since the ball also slows down faster at higher ball
  // velos
  float power = std::pow((toPlannedBall.GetLength() / divisor), 0.7f);
  Vector3 direction = toPlannedBall.GetNormalized(
      Vector3(0, -1, 0).GetRotated2D(nextBodyAngle));
  ...
  float powerMultiplier = 1.2f - (player->GetStat("technical_ballcontrol") *
                                  0.03f); // 1.24 .. * 0.1
  float veloBias = NormalizedClamp(
  ...
  Vector3 touchVec =
      direction * power * powerMultiplier + Vector3(0, 0, height);
  /*
    if (player->GetDebug()) {
  ...
  */
  ...
  xRot = (FFO.GetNormalized(0) * 0.6f + touchVec.GetNormalized(0) * 0.4f)
             .GetNormalized(0)
             .coords[1] *
         ((2 * pi * velo) - (backspinFactor * veloFactor));
  yRot = (FFO.GetNormalized(0) * 0.6f + touchVec.GetNormalized(0) * 0.4f)
             .GetNormalized(0)
             .coords[0] *
         ((2 * pi * velo) - (backspinFactor * veloFactor));
...
Vector3 GetTrapVector(Match *match, Player *player, const Vector3 &nextStartPos,
  ...
  float ballMovementFactor =
      0.0f; // how much of the current ball movement is maintained
  GetDifficultyFactors(match, player, positionOffset, distanceFactor,
                       heightFactor, ballMovementFactor);

  // base vector to start with
  Vector3 ballControl = GetBallControlVector(
      ball, player, nextStartPos, nextStartAngle, nextBodyAngle,
      outgoingMovement, currentAnim, frameNum, spatialState, positionOffset,
      xRot, yRot, distanceFactor);
...
Vector3 GetShotVector(Match *match, Player *player, const Vector3 &nextStartPos,
  ...
  Vector3 touchMovement =
      CalculateMovementAtFrame(origPositionCache, currentAnim->frameNum)
          .GetRotated2D(
              spatialState.angle); // spatialState.movement isn't reliable
                                   // because of smuggles and such
  // SetRedDebugPilon(player->GetPosition() + touchMovement);
  Vector3 touchDirection =
      touchMovement.GetNormalized(spatialState.directionVec);
  ...
  float expressionModifierFactor =
      NormalizedClamp(currentAnim->originatingCommand.desiredVelocityFloat,
                      idleVelocity, sprintVelocity);

  // previous: float directionFactor =
  // touchDirection.GetDotProduct(currentAnim->originatingCommand.touchInfo.desiredDirection)
  // * 0.5f + 0.5f; ideal angle is ~36 degrees == 0.2 * pi
  radian idealAngle = 0.2f * pi;
  radian angle1 = fabs(touchDirection.GetAngle2D(
      currentAnim->originatingCommand.touchInfo.desiredDirection.GetRotated2D(
          -idealAngle)));
  ...
  // calculate power. take desired power as maximum, and subtract based on
  // player/ball movement and such
  ...
  float power = clamp(powerFactor * adaptedDesiredPower, 0.0f,
                      (32.0f + player->GetStat("physical_shotpower") * 13.0f) *
                          (0.2f + animMaxPowerFactor * 0.8f));

  // add this after previous stat-clamp, because using the current ball movement
  // is like an added (power) bonus that everybody profits from, even sucky
  // players
  float playerMovBallMovPowerFactor =
      (touchMovement - ball->GetMovement()).GetLength();
  playerMovBallMovPowerFactor =
      NormalizedClamp(playerMovBallMovPowerFactor, 0.0f, 10.0f);
  ...
  // calculate difficulty, based on factors like desired power, player/ball
  // movement, skill, positionoffset etcetera
  ...
  playerMovBallMovEasinessFactor =
      1.0f - playerMovBallMovPowerFactor *
                 (0.5f - player->GetStat("technical_volley") * 0.3f);
  if (Verbose())
  ...
  Vector3 desiredShot =
      (currentAnim->originatingCommand.touchInfo.desiredDirection.Get2D() +
  ...
  Vector3 worstCaseDirection =
      currentAnim->originatingCommand.touchInfo.desiredDirection.Get2D();

  // direction lag
  float laggyDirectionBias = difficultyFactor * 0.8f;
  worstCaseDirection = (touchDirection * laggyDirectionBias +
                        worstCaseDirection * (1.0f - laggyDirectionBias))
                           .GetNormalized(0);

  // random dir
  worstCaseDirection = worstCaseDirection +
                       (Vector3(random(-1, 1), random(-1, 1), random(-1, 1)) *
  ...
  Vector3 worstCaseShot =
      (worstCaseDirection + Vector3(0, 0, worstCaseHeight)).GetNormalized() *
      worstCasePower;
  if (Verbose()) {
    worstCaseShot.Print();
  ...
  Vector3 shot =
      desiredShot * (1.0f - worstCaseFactor) + worstCaseShot * worstCaseFactor;

  // add a little curve

  float randomCurveFactor = 0.3f + worstCaseFactor * 0.7f;
  float plannedCurveFactor =
      0.7f; // todo: use curve as actual planned thing, not random :p
  ...
  shot.Rotate2D(amount *
                (0.4f + 0.6f * NormalizedClamp(shot.GetLength(), 0.0f, 70.0f)));
  zRot = amount * -420 + (random(-20, 20) * plannedCurveFactor);

  // SetRedDebugPilon(match->GetBall()->Predict(0).Get2D() + touchVec.Get2D() *
  // 0.4f);
