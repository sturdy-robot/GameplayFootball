...
struct MissingAnim {
  ...
  bool operator == (const MissingAnim &comp) const {
    if (this->outgoingVelocity == comp.outgoingVelocity &&
        ...
        this->outgoingBodyDirection.GetDistance(comp.outgoingBodyDirection) <
0.001f) { return true; } else { return false;
    }
  }
...
public:
  ...
  void
  SetLastTouchTeamID(int id,
                     e_TouchType touchType = e_TouchType_Intentional_Kicked) {
    lastTouchTeamIDs[touchType] = id;
    lastTouchTeamID = id;
    referee->BallTouched();
  }
  int GetLastTouchTeamID(e_TouchType touchType) const {
    return lastTouchTeamIDs[touchType];
  }
  ...
  void GetCameraParams(float &zoom, float &height, float &fov,
                       float &angleFactor);
  ...
  void FollowCamera(Quaternion &orientation, Quaternion &nodeOrientation,
  ...
  unsigned long GetPreviousPreparePutTime_ms() {
    return previousPreparePutTime_ms;
  }
  ...
  int GetTimeSincePreviousPreparePut_ms() {
    return timeSincePreviousPreparePut_ms;
  }
  int GetTimeSincePreviousPut_ms() { return timeSincePreviousPut_ms; }

  // void AddMissingAnim(const MissingAnim &someAnim);

  // not sure about how signals work in this game at the moment. whole menu/game
  // thing needs a rethink, i guess
...
protected:
  ...
  void CheckHumanoidCollision(Player *p1, Player *p2,
  ...
  // for stuff like animation smoothing, we could need the time elapsed since
  // last Put() and such
  ...
  bool pause;
  e_MatchPhase matchPhase; // 0 - first half; 1 - second half; 2 - 1st extra
                           // time; 3 - 2nd extra time; 4 - penalties
  bool inPlay;
  bool inSetPiece;
  bool goalScored; // true after goal scored, false again after next match state
                   // change
  bool ballIsInGoal;
  int lastGoalTeamID;
