...
Uint32 GetPitchDiffuseColor(SDL_Surface *pitchSurf, float xCoord,
                            float yCoord) {
  ...
  float contrast =
      0.4f; // g <=> rb contrast. lower = less saturation, higher = greener
  float rToB =
  ...
  float seamlessX =
      ((xCoord / pitchFullHalfW) * 0.5 + 0.5) * seamlessTexW * 18.0 * texScale;
  float seamlessY =
      ((yCoord / pitchFullHalfH) * 0.5 + 0.5) * seamlessTexH * 12.0 * texScale;
  seamlessX = std::fmod(seamlessX, seamlessTexW);
  seamlessY = std::fmod(seamlessY, seamlessTexH);
  // printf("%f, %f - ", seamlessX, seamlessY);
  Vector3 tex = BilinearSample(seamlessTex, seamlessX, seamlessY, seamlessTexW,
                               seamlessTexH);
  ...
  float perlinNoise =
      BilinearSample(perlinTex, perlX, perlY, perlinTexW, perlinTexH) - 0.5f;
  ...
  */
  ...
  r += ((perlinNoiseR * perlinNoiseMultiplier) +
        (randomNoise * randomNoiseMultiplier)) *
       40.0f;
  g += ((perlinNoiseG * perlinNoiseMultiplier) +
        (randomNoise * randomNoiseMultiplier)) *
       40.0f;
  b += ((perlinNoiseB * perlinNoiseMultiplier) +
        (randomNoise * randomNoiseMultiplier)) *
       40.0f;
  ...
  Vector3 overlay =
      BilinearSample(overlayTex, overlayX, overlayY, overlayTexW, overlayTexH);
  float overlay_alpha = BilinearSample(overlay_alphaTex, overlayX, overlayY,
                                       overlayTexW, overlayTexH);
  r = clamp(r * (1.0 - overlay_alpha) + overlay.coords[0] * overlay_alpha, 0,
            255);
  g = clamp(g * (1.0 - overlay_alpha) + overlay.coords[1] * overlay_alpha, 0,
            255);
  b = clamp(b * (1.0 - overlay_alpha) + overlay.coords[2] * overlay_alpha, 0,
            255);
...
inline Uint32 GetPitchSpecularColor(SDL_Surface *pitchSurf, float xCoord,
                                    float yCoord) {
  ...
  perlY = clamp(perlY + randomY * randomSpread, 0, perlinTexH - 1);
  float noise =
      base + BilinearSample(perlinTex, perlX, perlY, perlinTexW, perlinTexH) *
                 noisefac;
...
float GetSmoothGrassDirection(float coord, float repeat,
  ...
  float bias =
      coord - repeat * iteration; // goes from 0 to 1 over two bands (since
                                  // bands are split by being < 0.5 and > 0.5)
...
Uint32 GetPitchNormalColor(SDL_Surface *pitchSurf, float xCoord, float yCoord,
  ...
  if (fabs(xCoord) < pitchHalfW && fabs(yCoord) < pitchHalfH) {
    ...
    float yStrength =
        0.1; // i *think* the mowing of the 'lateral' lines may undo the
             // strength of these medial lines. so make this less apparent

    int transitionSharpness = 5;
    if (repeatMultiplier > 0.75f)
      transitionSharpness =
          7; // wider mow lines == more sharpening to correct for upscale
    normal += Vector3(
        GetSmoothGrassDirection(yCoord / yRepeat, 1.0f, transitionSharpness) *
            yStrength,
        GetSmoothGrassDirection(xCoord / xRepeat, 1.0f, transitionSharpness) *
            xStrength,
        0);
  }
  ...
  Uint32 color = SDL_MapRGB(pitchSurf->format, normal.coords[0] * 255,
                            normal.coords[1] * 255, normal.coords[2] * 255);
  return color;
}

void ConvertCoord(int resX, int resY, float x1, float y1, signed int offsetW,
                  signed int offsetH, float &x, float &y) {
  x = x1;
  y = y1;
  ...
  y *= resY;
  y /= pitchFullHalfH;
  // see which of the 4 parts of the pitch is to be drawed on, mirror
  // accordingly
  if (offsetW == -1)
    x = resX - x - 1;
  if (offsetH == -1)
...
void BmpRect(SDL_PixelFormat *pixelFormat, Uint32 *bitmap, int resX, int resY,
             float x1, float y1, float x2, float y2, signed int offsetW,
             signed int offsetH) {
  float rx1, ry1;
...
void BmpArc(SDL_PixelFormat *pixelFormat, Uint32 *bitmap, int resX, int resY,
            float x1, float y1, float radius, radian begin, radian end,
...
void DrawLines(SDL_PixelFormat *pixelFormat, Uint32 *diffuseBitmap, int resX,
               int resY, signed int offsetW, signed int offsetH) {

  // only draw lowerright section, other sections are mirrored through offsetW
  // and offsetH

  BmpRect(pixelFormat, diffuseBitmap, resX, resY, pitchHalfW - lineHalfW, 0,
          pitchHalfW + lineHalfW, pitchHalfH + lineHalfW, offsetW,
          offsetH); // backline
  BmpRect(pixelFormat, diffuseBitmap, resX, resY, 0, pitchHalfH - lineHalfW,
          pitchHalfW - lineHalfW, pitchHalfH + lineHalfW, offsetW,
          offsetH); // sideline
  BmpRect(pixelFormat, diffuseBitmap, resX, resY, 0, 0, lineHalfW * 0.5f,
          pitchHalfH - lineHalfW, offsetW,
          offsetH); // middleline // for some reason, the middle line needs to
                    // be half-half-width to become the right width. i don't
                    // know why either.

  // 16.5m box
  BmpRect(pixelFormat, diffuseBitmap, resX, resY, pitchHalfW - 16.5 - lineHalfW,
          0, pitchHalfW - 16.5 + lineHalfW, 20.15 - lineHalfW, offsetW,
          offsetH); // vert
  BmpRect(pixelFormat, diffuseBitmap, resX, resY, pitchHalfW - 16.5 - lineHalfW,
          20.15 - lineHalfW, pitchHalfW - lineHalfW, 20.15 + lineHalfW, offsetW,
          offsetH); // horiz

  // keeper box (9.16m half width)
  BmpRect(pixelFormat, diffuseBitmap, resX, resY, pitchHalfW - 5.5 - lineHalfW,
          0, pitchHalfW - 5.5 + lineHalfW, 9.16 - lineHalfW, offsetW,
          offsetH); // vert
  BmpRect(pixelFormat, diffuseBitmap, resX, resY, pitchHalfW - 5.5 - lineHalfW,
          9.16 - lineHalfW, pitchHalfW - lineHalfW, 9.16 + lineHalfW, offsetW,
          offsetH); // horiz

  BmpArc(pixelFormat, diffuseBitmap, resX, resY, 0, 0, 9.15, 0.5 * pi, 1 * pi,
         offsetW, offsetH); // middle circle
  BmpArc(pixelFormat, diffuseBitmap, resX, resY, pitchHalfW - 11, 0, 9.15,
         1.208 * pi, 1.5 * pi, offsetW, offsetH); // penalty arc

  BmpArc(pixelFormat, diffuseBitmap, resX, resY, pitchHalfW, pitchHalfH, 0.5,
         1.55 * pi, 2 * pi, offsetW, offsetH); // corner arc

  // penalty spot
  BmpArc(pixelFormat, diffuseBitmap, resX, resY, pitchHalfW - 11, 0, 0.08,
         0.5 * pi, 1.5 * pi, offsetW, offsetH);
  BmpArc(pixelFormat, diffuseBitmap, resX, resY, pitchHalfW - 11, 0, 0.04,
         0.5 * pi, 1.5 * pi, offsetW, offsetH);

  // center spot
  BmpArc(pixelFormat, diffuseBitmap, resX, resY, 0, 0, 0.08, 0.5 * pi, 1.0 * pi,
         offsetW, offsetH);
  BmpArc(pixelFormat, diffuseBitmap, resX, resY, 0, 0, 0.04, 0.5 * pi, 1.0 * pi,
         offsetW, offsetH);
}

/* todo
void DrawMud(SDL_PixelFormat *pixelFormat, Uint32 *diffuseBitmap, int resX, int
resY, signed int offsetW, signed int offsetH) {
}
*/

void CreateChunk(int i, int resX, int resY, int resSpecularX, int resSpecularY,
                 int resNormalX, int resNormalY,
                 float grassNormalRepeatMultiplier = 0.5f) {
  ...
  for (int x = 0; x < resX; x++) {
    for (int y = 0; y < resY; y++) {
      float xCoord =
          x / (resX * 1.0) * pitchFullHalfW + pitchFullHalfW * offsetW;
      float yCoord =
          y / (resY * 1.0) * pitchFullHalfH + pitchFullHalfH * offsetH;
      diffuseBitmap[y * resX + x] =
          GetPitchDiffuseColor(pitchDiffuseSurf, xCoord, yCoord);
    }
  }
  if (Verbose())
    printf("1a\n");
  // DrawLines(pitchDiffuseSurf->format, diffuseBitmap, resX, resY, offsetW,
  // offsetH);
  if (Verbose())
    printf("1b\n");
  // DrawMud(pitchDiffuseSurf->format, diffuseBitmap, resX, resY, offsetW,
  // offsetH);
  for (int x = 0; x < resSpecularX; x++) {
    for (int y = 0; y < resSpecularY; y++) {
      float xSpecularCoord =
          x / (resSpecularX * 1.0) * pitchFullHalfW + pitchFullHalfW * offsetW;
      float ySpecularCoord =
          y / (resSpecularY * 1.0) * pitchFullHalfH + pitchFullHalfH * offsetH;
      specularBitmap[y * resSpecularX + x] = GetPitchSpecularColor(
          pitchSpecularSurf, xSpecularCoord, ySpecularCoord);
    }
  }
  if (Verbose())
    printf("1c\n");
  for (int x = 0; x < resNormalX; x++) {
    for (int y = 0; y < resNormalY; y++) {
      float xNormalCoord =
          x / (resNormalX * 1.0) * pitchFullHalfW + pitchFullHalfW * offsetW;
      float yNormalCoord =
          y / (resNormalY * 1.0) * pitchFullHalfH + pitchFullHalfH * offsetH;
      normalBitmap[y * resNormalX + x] =
          GetPitchNormalColor(pitchNormalSurf, xNormalCoord, yNormalCoord,
                              grassNormalRepeatMultiplier);
    }
  }

  memcpy(pitchDiffuseSurf->pixels, diffuseBitmap, resX * resY * sizeof(Uint32));
  memcpy(pitchSpecularSurf->pixels, specularBitmap,
         resSpecularX * resSpecularY * sizeof(Uint32));
  memcpy(pitchNormalSurf->pixels, normalBitmap,
         resNormalX * resNormalY * sizeof(Uint32));
  ...
  boost::intrusive_ptr<Resource<Texture>> pitchDiffuseTex =
      ResourceManagerPool::GetInstance()
          ...
          ->Fetch("pitch_0" + int_to_str(i) + ".png", false, alreadyThere,
                  true);
  assert(alreadyThere);
  boost::intrusive_ptr<Resource<Texture>> pitchSpecularTex =
      ResourceManagerPool::GetInstance()
          .GetManager<Texture>(e_ResourceType_Texture)
          ->Fetch("pitch_specular_0" + int_to_str(i) + ".png", false,
                  alreadyThere, true);
  assert(alreadyThere);
  boost::intrusive_ptr<Resource<Texture>> pitchNormalTex =
      ResourceManagerPool::GetInstance()
          .GetManager<Texture>(e_ResourceType_Texture)
          ->Fetch("pitch_normal_0" + int_to_str(i) + ".png", false,
                  alreadyThere, true);
  assert(alreadyThere);

  // todo: don't directly access texture resources anymore, work via engine's
  // Surface resources.
  ...
  pitchDiffuseTex->GetResource()->CreateTexture(e_InternalPixelFormat_SRGB8,
                                                e_PixelFormat_RGB, resX, resY,
  ...
  pitchSpecularTex->GetResource()->CreateTexture(
      e_InternalPixelFormat_RGB8, e_PixelFormat_RGB, resSpecularX, resSpecularY,
      false, true, true, true);
  pitchSpecularTex->GetResource()->UpdateTexture(pitchSpecularSurf, false,
                                                 true);
  ...
  pitchNormalTex->GetResource()->CreateTexture(
      e_InternalPixelFormat_RGB8, e_PixelFormat_RGB, resNormalX, resNormalY,
      false, true, true, true);
...
void GeneratePitch(int resX, int resY, int resSpecularX, int resSpecularY,
  ...
  Perlin *perlin2 =
      new Perlin(4, 0.14 * scale, 0.5, time(NULL) + 139882); // mid freq
  ...
  for (int y = 0; y < perlinTexH; y++) {
    ynoise[y] = (sin(y / (float)perlinTexH * 13 * sinScale) +
                 sin(y / (float)perlinTexH * 43 * sinScale) +
  ...
  for (int x = 0; x < perlinTexW; x++) {
    float xnoise = (sin(x / (float)perlinTexW * 15 * sinScale) +
                    sin(x / (float)perlinTexW * 41 * sinScale) +
    ...
    for (int y = 0; y < perlinTexH; y++) {
      ...
      perlinTex[y * perlinTexW + x] =
          ...
          perlin2->Get(x, y) * 0.6f; // + 0.5f to get it from [0..1]; the
                                     // multiplier is bias between the noises
      perlinTex[y * perlinTexW + x] =
          perlinTex[y * perlinTexW + x] * 1.7f +
          0.5f; // most of perlin noise is well between -0.5 and 0.5, so expand
                // a bit
      perlinTex[y * perlinTexW + x] +=
          (NormalizedClamp(noise, -0.9f, 0.9f) * 2.0f - 1.0f) *
      ...
      perlinTex[y * perlinTexW + x] =
          curve(perlinTex[y * perlinTexW + x], 0.4f);
    }
  }
  ...
  for (int i = 0; i < 4; i++) {
    pitchThread[i] = boost::thread(&CreateChunk, i + 1, resX, resY,
                                   resSpecularX, resSpecularY, resNormalX,
                                   resNormalY, grassNormalRepeatMultiplier);
  }
