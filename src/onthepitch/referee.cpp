...
Referee::Referee(Match *match) : match(match) {
  ...
  whistle[1] = boost::static_pointer_cast<Sound>(
      ObjectFactory::GetInstance().CreateObject("whistle1",
                                                e_ObjectType_Sound));
  GetScene3D()->CreateSystemObjects(whistle[1]);
  whistle[1]->SetSoundBuffer(soundBufferRes);
  // whistle[1]->SetGain(0.3 * GetConfiguration()->GetReal("audio_volume",
  // 0.5));
  ...
  whistle[3] = boost::static_pointer_cast<Sound>(
      ObjectFactory::GetInstance().CreateObject("whistle3",
                                                e_ObjectType_Sound));
  GetScene3D()->CreateSystemObjects(whistle[3]);
  whistle[3]->SetSoundBuffer(soundBufferRes);
  // whistle[3]->SetGain(0.3 * GetConfiguration()->GetReal("audio_volume",
  // 0.5));
...
void Referee::Process() {
  ...
  if (match->IsInPlay() && !match->IsInSetPiece()) {
    ...
    if (((match->GetMatchPhase() == e_MatchPhase_1stHalf &&
          match->GetMatchTime_ms() > 2700000) ||
         (match->GetMatchPhase() == e_MatchPhase_2ndHalf &&
          match->GetMatchTime_ms() > 5400000) ||
    ...
    if (!match->IsInPlay() && !match->IsInSetPiece() && buffer.active == true) {
      ...
      if (buffer.stopTime + 300 == match->GetActualTime_ms() &&
          buffer.endPhase == false &&
          buffer.desiredSetPiece != e_SetPiece_KickOff) {
        whistle[1]->SetGain(0.3 *
                            GetConfiguration()->GetReal("audio_volume", 0.5));
...
void Referee::PrepareSetPiece(e_SetPiece setPiece) {
  ...
  buffer.taker =
      match->GetTeam(buffer.teamID)->GetController()->GetPieceTaker();
}
...
void Referee::BallTouched() {
  ...
  if (match->IsInPlay() && !match->IsInSetPiece() && buffer.active == false &&
    ...
    while (playerIter != offsidePlayers.end()) {
      if (match->GetTeam(lastTouchTeamID)->GetLastTouchPlayer() ==
          playerIter->first) {
        foul.advantage = false;
        if (!CheckFoul()) {
  ...
  if (match->IsInPlay() && (buffer.active == false ||
    ...
    float offside = AI_GetOffsideLine(match, match->GetMentalImage(0),
                                      abs(lastTouchTeamID - 1));
    ...
    for (unsigned int i = 0; i < players.size(); i++) {
      if (players.at(i) != team->GetLastTouchPlayer()) {
        if (players.at(i)->GetPosition().coords[0] * team->GetSide() <
          ...
          offsidePlayers.insert(std::pair<Player *, Vector3>(
              players.at(i), players.at(i)->GetPosition()));
        }
      }
    }
  }
...
void Referee::TripNotice(Player *tripee, Player *tripper, int tackleType) {
  ...
  } else if (tackleType == 3 && (tripper != foul.foulPlayer ||
                                 foul.foulType == 0)) { // sliding tackle
    ...
    if (match->GetActualTime_ms() - tripper->GetLastTouchTime_ms() > 600 &&
      ...
      if (tripper->TouchAnim()) {
        ...
        severity += NormalizedClamp(
                        (match->GetBall()->Predict(0) - tripper->GetTouchPos())
                            .GetLength(),
                        0.0, 2.0) *
                    0.5;
      }
  ...
  if (foul.foulType != 0 && foul.advantage == false && !foul.hasBeenProcessed) {
    ...
    if (!penalty) {
      ...
      buffer.restartPos = Vector3(
          (pitchHalfW - 11.0) * foul.foulPlayer->GetTeam()->GetSide(), 0, 0);
    }
