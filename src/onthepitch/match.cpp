...
Match::Match(MatchData *matchData, const std::vector<IHIDevice *> &controllers)
  ...
  for (unsigned int i = 0; i < animationsTmp.size(); i++) {
    ...
    // printf("\n");
    animPositionCache.insert(
        std::pair<Animation *, std::vector<Vector3>>(someAnim, positions));
  }
  ...
  ObjectLoader loader;
  fullbodyNode =
      loader.LoadObject(GetScene3D(), "media/objects/players/fullbody.object");
  ...
  cameraUserZoom =
      GetConfiguration()->GetReal("camera_zoom", _default_CameraZoom);
  cameraUserHeight =
      GetConfiguration()->GetReal("camera_height", _default_CameraHeight);
  cameraUserFOV = GetConfiguration()->GetReal("camera_fov", _default_CameraFOV);
  cameraUserAngleFactor = GetConfiguration()->GetReal(
      "camera_anglefactor", _default_CameraAngleFactor);
  ...
  if (!SuperDebug()) {
    tmpStadiumNode = loader.LoadObject(
        GetScene3D(), "media/objects/stadiums/test/test.object");
    RandomizeAdboards(tmpStadiumNode);
  }
  if (SuperDebug())
    tmpStadiumNode = loader.LoadObject(
        GetScene3D(), "media/objects/stadiums/test/pitchonly.object");
  std::list<boost::intrusive_ptr<Geometry>> stadiumGeoms;

  // split stadium geometry into multiple geometry objects, for more efficient
  // culling
  ...
  std::list<boost::intrusive_ptr<Geometry>>::iterator iter =
      stadiumGeoms.begin();
  ...
  goalsNode =
      loader.LoadObject(GetScene3D(), "media/objects/stadiums/goals.object");
  ...
  sunNode =
      loader.LoadObject(GetScene3D(), "media/objects/lighting/generic.object");
  ...
  crowd01 = boost::static_pointer_cast<Sound>(
      ObjectFactory::GetInstance().CreateObject("crowd01sound",
                                                e_ObjectType_Sound));
  ...
  crowd02 = boost::static_pointer_cast<Sound>(
      ObjectFactory::GetInstance().CreateObject("crowd02sound",
                                                e_ObjectType_Sound));
  ...
  previousProcessTime_ms = EnvironmentManager::GetInstance().GetTime_ms() -
                           gameSequenceInfo.startTime_ms;
  previousPutTime_ms = EnvironmentManager::GetInstance().GetTime_ms() -
                       gameSequenceInfo.startTime_ms;
  ...
  radar = new Gui2Radar(menuTask->GetWindowManager(), "game_radar", 38, 78, 24,
                        18, this, matchData->GetTeamData(0)->GetColor1(),
  ...
  if (1 == 2) {
    tacticsDebug =
        new Gui2TacticsDebug(menuTask->GetWindowManager(), "game_tacticsdebug",
                             22, 1.3f, 56, 26, this);
  ...
  messageCaption = new Gui2Caption(menuTask->GetWindowManager(),
                                   "game_messages", 0, 0, 80, 8, "");
  ...
  std::list<boost::intrusive_ptr<Spatial>>::iterator spatialIter =
      spatials.begin();
  ...
  replayBallTouchesNetFrames =
      boost::circular_buffer<ReplayBallTouchesNetFrame>(GetReplaySize_ms() /
                                                        10);
  ...
  if (maxTestLights > 0) {
    ...
    for (int li = 0; li < maxTestLights; li++) {
      ...
      lightTest[li]->SetColor(Vector3(sin(li) * 0.5f + 0.5f,
                                      sin(li + 0.66f * pi) * 0.5f + 0.5f,
                                      sin(li * 1.33f * pi) * 0.5f + 0.5f));
      lightTest[li]->SetPosition(
          Vector3(sin(li / (float)maxTestLights * 2 * pi) * 40,
  ...
  LoadingMatchPage *loadingMatchPage =
      static_cast<LoadingMatchPage *>(menuTask->GetWindowManager()
                                          ->GetPageFactory()
                                          ->GetMostRecentlyCreatedPage());
  ...
  /*
    if (missingAnims.size() > 0) {
      ...
      for (unsigned int i = 0; i < missingAnims.size(); i++) {
        //printf("[%i times] dir %f, %f, %f; velo %i; bodydir(abs) %f, %f,
    %f\n", missingAnims.at(i).timesMissed,
    missingAnims.at(i).outgoingDirection.coords[0],
    missingAnims.at(i).outgoingDirection.coords[1],
    missingAnims.at(i).outgoingDirection.coords[2],
    missingAnims.at(i).outgoingVelocity,
    missingAnims.at(i).outgoingBodyDirectionAbs.coords[0],
    missingAnims.at(i).outgoingBodyDirectionAbs.coords[1],
    missingAnims.at(i).outgoingBodyDirectionAbs.coords[2]); printf("[%i times]
    dir %i; velo %i; bodydir(rel) %i; average difference: %i\n",
    missingAnims.at(i).timesMissed,
    int(round(missingAnims.at(i).outgoingDirection.GetAngle2D(Vector3(0, -1, 0))
    / pi * 180.0f)), missingAnims.at(i).outgoingVelocity,
    int(round(missingAnims.at(i).outgoingBodyDirection.GetAngle2D(Vector3(0, -1,
    0)) / pi * 180.0f)), int(round(missingAnims.at(i).angleDifference / pi *
    180.0f)));
      }
      printf("*********************\n");
      missingAnims.clear();
...
void Match::SetRandomSunParams() {
  ...
  sunPos =
      Vector3(clamp(random(-1.7f, 1.7f), -1.0, 1.0),
              clamp(random(-1.7f, 1.7f), -1.0, 1.0), averageHeightMultiplier);
  sunPos.Normalize();
  ...
  Vector3 randomAddition(random(-0.1, 0.1), random(-0.1, 0.1),
                         random(-0.1, 0.1));
  randomAddition *= 1.2f;
  ...
  static_pointer_cast<Light>(sunNode->GetObject("sun"))
      ->SetColor(sunColor * brightness);
}
...
void Match::RandomizeAdboards(boost::intrusive_ptr<Node> stadiumNode) {
  ...
  std::list<boost::intrusive_ptr<Geometry>>::const_iterator stadiumGeomsIter =
      stadiumGeoms.begin();
  while (stadiumGeomsIter != stadiumGeoms.end()) {
    ...
    boost::intrusive_ptr<Resource<GeometryData>> adboardGeom =
        geomObject->GetGeometryData();
    ...
    for (unsigned int i = 0; i < tmesh.size(); i++) {
      if (tmesh.at(i).material.diffuseTexture !=
        ...
        std::string identString =
            tmesh.at(i).material.diffuseTexture->GetIdentString();
        // printf("%s\n", identString.c_str());
...
signed int Match::GetBestPossessionTeamID() { return bestPossessionTeamID; }

void Match::GameOver() { gameOver = true; }

void Match::GetCameraParams(float &zoom, float &height, float &fov,
                            float &angleFactor) {
  zoom = cameraUserZoom;
...
void Match::SetCameraParams(float zoom, float height, float fov,
                            float angleFactor) {
  cameraUserZoom = zoom;
...
void Match::UpdateIngameCamera() {
  ...
  ballPos += Vector3(((teams[0]->GetFadingTeamPossessionAmount() - 1.0f) *
                          -teams[0]->GetSide() +
                      (teams[1]->GetFadingTeamPossessionAmount() - 1.0f) *
                          -teams[1]->GetSide()) *
                         4.0f,
                     0, 0);
  ...
  float maxW = pitchHalfW * 0.84f *
               (1.0 / (zoom + 0.01f)); // * (height * 0.75f + 0.25f);
  float maxH = pitchHalfH * 0.60f * (1.0 / (zoom + 0.01f)) *
               (height * 0.75f + 0.25f); // 0.52f
  ...
  shudder *= 0.2f;
  camPos.push_back(ballPos +
                   shudder * ((float)camPos.size() / (float)camPosSize));
  ...
  while (camIter != camPos.end()) {
    ...
    weight *=
        pow(1.0f - index / indexSize,
            0.3f); // sharp cutoff @ latest (because cameraperson can't
                   // 'foresee' the current moment that fast) | wa: (1.0 - x /
                   // 100) ^ 0.3 * (<prev formula>) | from x = 0 to 100
  ...
  radian angleFac =
      1.0f - cameraUserAngleFactor *
                 0.4f; // 0.0 == 90 degrees max, 1.0 == sideline view
  ...
  if (!IsGoalScored() || (IsGoalScored() && goalScoredTimer < 1000)) {
    ...
    if (camMethod == 1) {
      ...
      cameraOrientation.SetAngleAxis(distRot + (0.42f - height * 0.01f) * pi,
                                     Vector3(1, 0, 0));
      cameraNodeOrientation.SetAngleAxis((-average.coords[0] / pitchHalfW) *
                                             (1.0f - angleFac) * 0.25f * pi *
                                             1.24f,
                                         Vector3(0, 0, 1));
      cameraNodePosition =
          ...
          Vector3(0,
                  -41.4f - (cameraUserFOV * 3.7f) + pow(height, 1.2f) * 0.46f,
                  10.0f + height) *
              zoom;
      ...
      cameraNodePosition =
          average * Vector3(1, 1, 0) + Vector3(0, 0, 50 + zoom * 20.0);
      cameraFOV = 28;
      ...
      cameraOrientation.SetAngleAxis(
          0.3f * pi * height + 0.4f * pi * (1.0 - height), Vector3(1, 0, 0));
      cameraNodeOrientation = QUATERNION_IDENTITY;
      Vector3 offset = Vector3(0, -175.0f, 125.0f) * height +
                       Vector3(0, -230.0f, 65.0f) * (1.0 - height);
      cameraNodePosition =
          average * Vector3(0.9f, 0.7f, 0.2f) + offset * zoom * 0.4f;
      cameraFOV = 15.0f;
    ...
    cameraNodePosition = targetPos +
                         Vector3(0, -1, 0).GetRotated2D(rot) * 15.0f +
                         Vector3(0, 0, 3);
    cameraFOV = 35.0f;
...
void Match::Process() {
  ...
  if (!pause) {
    ...
    if (GetBallRetainer() == 0) {
      ...
      if (bestTeamID != -1) {
        Player *candidate = teams[GetBestPossessionTeamID()]
        ...
        if (candidate != GetDesignatedPossessionPlayer()) {
          ...
          float timeRating =
              (float)(candidateTime + 10) / (float)(designatedTime + 10);
          if (timeRating < 0.85f)
            designatedPossessionPlayer = candidate;
        }
      } else {
        // just stick with current team
        designatedPossessionPlayer =
            teams[GetDesignatedPossessionPlayer()->GetTeamID()]
                ->GetDesignatedTeamPossessionPlayer();
      }
    } else {
    ...
    if (GetBestPossessionTeamID() != -1) {
      ...
      if (!IsGoalScored()) {
        if (IsInPlay()) {
          float distance =
              (Vector3(pitchHalfW *
                           -teams[GetBestPossessionTeamID()]->GetSide(),
                       0, 0) -
               GetPlayer(teams[GetBestPossessionTeamID()]
      ...
      if (cur_excitement > excitement) {
        // fast rise
        excitement =
            clamp(excitement * 0.98f + cur_excitement * 0.02f, 0.0f, 1.0f);
      } else {
        // slow decay
        excitement =
            clamp(excitement * 0.998f + cur_excitement * 0.002f, 0.0f, 1.0f);
      }
      crowd01->SetGain((excitement) * 0.5f *
    ...
    if (IsInPlay() && !IsInSetPiece())
      GetMatchData()->AddPossessionTime_10ms(
          designatedPossessionPlayer->GetTeamID());
    ...
    if (IsInPlay()) {
      if (t1goal) {
        matchData->SetGoalCount(teams[1]->GetID(),
                                matchData->GetGoalCount(1) + 1);
      ...
      if (t2goal) {
        matchData->SetGoalCount(teams[0]->GetID(),
                                matchData->GetGoalCount(0) + 1);
      ...
      if (t1goal || t2goal) {
        ...
        // find out who scored
        bool ownGoal = true;
        if (GetLastTouchTeamID(e_TouchType_Intentional_Kicked) ==
                GetLastGoalTeamID() ||
        ...
        if (!ownGoal) {
          ...
          if (lastGoalScorer) {
            SpamMessage(
                "GOAL for " +
                    matchData->GetTeamData(GetLastGoalTeamID())->GetName() +
          ...
          lastGoalScorer =
              teams[abs(GetLastGoalTeamID() - 1)]->GetLastTouchPlayer();
          if (lastGoalScorer) {
            SpamMessage("OWN GOAL! " +
                            lastGoalScorer->GetPlayerData()->GetLastName() +
                            " is so unlucky!",
                        4000);
          } else {
    ...
    if (GetReferee()->GetBuffer().active == true &&
      ...
      if (GetReferee()->GetBuffer().prepareTime >
          GetActualTime_ms()) { // FOUL, film referee
        SetAutoUpdateIngameCamera(false);
        FollowCamera(
            cameraOrientation, cameraNodeOrientation, cameraNodePosition,
            cameraFOV,
            officials->GetReferee()->GetPosition() + Vector3(0, 0, 0.8f), 1.5f);
        cameraNearCap = 1;
        cameraFarCap = 220;
        if (officials->GetReferee()->GetCurrentFunctionType() ==
  ...
  if (!pause) {
    ...
    if (actualTime_ms < zoomTime + startTime) { // nice effect at the start
      ...
      cameraOrientation =
          cameraOrientation.GetSlerped(bias, QUATERNION_IDENTITY);
      cameraNodeOrientation =
          cameraNodeOrientation.GetSlerped(bias, initialOrientation);
      cameraNodePosition =
          cameraNodePosition * (1.0f - bias) + initialPosition * bias;
  ...
  if (tacticsDebug && actualTime_ms % 1000 == 0) {
    for (unsigned int teamID = 0; teamID < 2; teamID++) {
      ...
      while (tacIter != userMods->end()) {
        ...
        // (*tacIter).first.c_str(), teamID, i, userValue, autoValue,
        // liveValue);
  ...
  if (!pause && _positionLogging) {
    ...
    for (int teamID = 0; teamID < 2; teamID++) {
      ...
      for (unsigned int i = 0; i < playas.size(); i++) {
        ...
        bla = "        player" + int_to_str(count) + ": " +
              real_to_str(pos.coords[0]) + ", " + real_to_str(pos.coords[1]) +
              ", 0\n";
        positionLogFile << bla.c_str();
        count++;
      }
...
void Match::PreparePutBuffers() {
  ...
  unsigned long snapshotTime_ms =
      gameSequenceInfo.timesRan * gameSequenceInfo.sequenceTime_ms;
  // printf("%lu, %lu\n", (GetIterations() - 1) * 10, gameSequenceInfo.timesRan
  // * gameSequenceInfo.sequenceTime_ms); printf("PREP time: %lu, snapshot time:
  ...
  // sin((float)(EnvironmentManager::GetInstance().GetTime_ms() +
  // PredictFrameTimeToGo_ms(7)) * 0.001f) * 60; float xfun =
  // sin(snapshotTime_ms * 0.001f) * 60.0f;
  // buf_cameraNodePosition.SetValue(cameraNodePosition + Vector3(xfun, 0, 0),
  // snapshotTime_ms);
...
void Match::FetchPutBuffers() {
  ...
  unsigned long putTime_ms =
      time_ms; // - gameSequenceInfo.startTime_ms; // test: +
               // PredictFrameTimeToGo_ms(7) - 15;
  // printf("FETCH time: %lu - seqstarttime: %lu = put time: %lu, times ran *
  // 10: %i\n", time_ms, gameSequenceInfo.startTime_ms, putTime_ms,
  // (int)gameSequenceInfo.timesRan * gameSequenceInfo.sequenceTime_ms);
  // printf("FETCH buf - snapshot time delta: %i\n", (int)putTime_ms -
  // (int)gameSequenceInfo.timesRan * gameSequenceInfo.sequenceTime_ms);
  fetchedbuf_timeDelta = (int)putTime_ms - (int)gameSequenceInfo.timesRan *
  ...
  fetchedbuf_cameraNodeOrientation =
      buf_cameraNodeOrientation.GetValue(putTime_ms);
...
void Match::Put() {
  ...
  if (GetIterations() < 2)
    return; // no processes done yet (todo: this is not the correct way to
            // measure that :p)

  // fun!
  // sunNode->SetPosition(Vector3(sin(buf_actualTime_ms * 0.001) * 3000,
  // cos(buf_actualTime_ms * 0.001) * 3000, 1000.0));
  ...
  /*
    ...
    + PredictFrameTimeToGo_ms(7) - 15;
    //printf("PUT time: %lu - seqstarttime: %lu = put time: %lu, times ran * 10:
    %i\n", time_ms, gameSequenceInfo.startTime_ms, putTime_ms,
    (int)gameSequenceInfo.timesRan * gameSequenceInfo.sequenceTime_ms);
  ...
  */
  ...
  /*
    ...
    float bias = NormalizedClamp(targetTime, buf_testTime[0], buf_testTime[1] +
    1);
    //printf("%i (%i - %i)\n", targetTime, buf_testTime[0], buf_testTime[1]);

    unsigned long time_ms = EnvironmentManager::GetInstance().GetTime_ms();
    int diff = buf_testTime[1] - buf_testTime[0];
    printf("%i to %i (%i)\n", buf_testTime[0] - time_ms, buf_testTime[1] -
    time_ms, diff);
  ...
  */
  ...
  if (!pause) {
    ...
    // if (buf_actualTime_ms % 100 == 0) { // a better way would be to count
    // iterations (this modulo is irregular since not all process runs are put)
    //  clock
...
boost::intrusive_ptr<Node> Match::GetDynamicNode() { return dynamicNode; }
...
void Match::ApplyReplayFrame(unsigned long replayTime_ms) {
  ...
  for (unsigned int i = 0; i < replay.size(); i++) {
    ...
    /* todo: smoothing
    std::map<unsigned long, ReplayFrame>::iterator iter1 =
    replay.at(i).frames.find(first_ms); if (iter1 == replay.at(i).frames.end())
    printf("FRAME NOT FOUND, SHOULD NOT HAPPEN AAARGHH\n"); std::map<unsigned
    long, ReplayFrame>::iterator iter2 = replay.at(i).frames.find(last_ms); if
    (iter2 == replay.at(i).frames.end()) printf("FRAME NOT FOUND, SHOULD NOT
    HAPPEN AAARGHH\n");
    */

    boost::circular_buffer<ReplaySpatialFrame>::iterator iter =
        replay.at(i)->frames.begin();
    while (iter != replay.at(i)->frames.end()) {
      if (iter->frameTime_ms >= replayTime_ms) {
        ...
        replay.at(i)->spatial->SetPosition(
            frame1.position * (1.0f - bias) + frame2.position * bias, false);
        // frame1.orientation.MakeSameNeighborhood(frame2.orientation); only
        // needed for Lerp
        replay.at(i)->spatial->SetRotation(
            frame1.orientation.GetSlerped(bias, frame2.orientation)
                .GetNormalized(),
            false);
        replay.at(i)->spatial->RecursiveUpdateSpatialData(
  ...
  boost::circular_buffer<ReplayBallTouchesNetFrame>::iterator iter =
...
bool Match::CheckForGoal(signed int side) {
  ...
  goal1.SetVertex(1,
                  Vector3((pitchHalfW + lineHalfW + 0.11f) * side, -3.7f, 0));
  goal1.SetVertex(2,
                  Vector3((pitchHalfW + lineHalfW + 0.11f) * side, 3.7f, 2.5f));
  goal1.SetNormals(Vector3(-side, 0, 0));
  Triangle goal2;
  goal2.SetVertex(0,
                  Vector3((pitchHalfW + lineHalfW + 0.11f) * side, -3.7f, 0));
  goal2.SetVertex(
      1, Vector3((pitchHalfW + lineHalfW + 0.11f) * side, -3.7f, 2.5f));
  goal2.SetVertex(2,
                  Vector3((pitchHalfW + lineHalfW + 0.11f) * side, 3.7f, 2.5f));
...
void Match::CheckHumanoidCollisions() {
  ...
  for (unsigned int i1 = 0; i1 < players.size() - 1; i1++) {
    for (unsigned int i2 = i1 + 1; i2 < players.size(); i2++) {
      ...
      CheckHumanoidCollision(players.at(i1), players.at(i2),
                             playerBounces.at(i1), playerBounces.at(i2));
    }
  }
  ...
  for (unsigned int i1 = 0; i1 < players.size(); i1++) {
    ...
    // if (playerBounces.at(i1).size() > 0) printf("  player %i.. ",
    // players.at(i1)->GetID());
    ...
    if (totalForce > 0.0f) {
      ...
      // if (playerBounces.at(i1).size() > 0) printf("%f, %f; ", totalBias,
      // multiplier);
      ...
      for (unsigned int i2 = 0; i2 < playerBounces.at(i1).size(); i2++) {
        ...
        const PlayerBounce &bounce = playerBounces.at(i1).at(i2);
        bounceVec +=
            (bounce.opp->GetMovement() - players.at(i1)->GetMovement()) *
            bounce.force * (bounce.force / totalForce);

        // //SetGreenDebugPilon(bounce.opp->GetPosition() +
        // bounce.opp->GetMovement()); if (players.at(i1)->GetTeamID() == 0) {
        //   SetRedDebugPilon(players.at(i1)->GetPosition() +
        //   players.at(i1)->GetMovement());
        // } else {
        //   SetGreenDebugPilon(players.at(i1)->GetPosition() +
        //   players.at(i1)->GetMovement());
        // }
      }
...
void Match::CheckHumanoidCollision(Player *p1, Player *p2,
  ...
  if (distance < bouncePlayerRadius * 2.0f ||
    ...
    /*
        // skew a bit so the bounce is mostly sideways from possession player -
       this way, players are more likely to walk side by side in ball battles
       instead of behind each other if (p1 == GetDesignatedPossessionPlayer() ||
       p2 == GetDesignatedPossessionPlayer()) {
          //radian angle =
       GetDesignatedPossessionPlayer()->GetDirectionVec().GetAngle2D(); radian
       angle =
    ...
    */
    ...
    // back facing
    Vector3 p1facing =
        p1->GetDirectionVec().GetRotated2D(p1->GetRelBodyAngle() * 0.7f);
    Vector3 p2facing =
        p2->GetDirectionVec().GetRotated2D(p2->GetRelBodyAngle() * 0.7f);
    p1backFacing = clamp(p1facing.GetDotProduct(bounceVec) * 0.5f + 0.5f, 0.0f,
                         1.0f); // 0 .. 1 == worst .. best
    p2backFacing =
        clamp(p2facing.GetDotProduct(-bounceVec) * 0.5f + 0.5f, 0.0f, 1.0f);
    ...
    if (distance < bouncePlayerRadius * 2.0f) {
      ...
      bounceBias -= clamp(((p1velocity - p2velocity) / sprintVelocity) * 0.2f,
                          -0.2f, 0.2f);
      ...
      if (p1->TouchPending() &&
          p1->GetCurrentFunctionType() == e_FunctionType_Sliding)
      ...
      if (p2->TouchPending() &&
          p2->GetCurrentFunctionType() == e_FunctionType_Sliding)
        bounceBias -= 0.1f + 0.4f * p2->GetStat("technical_slidingtackle");

      // problem is, once possession is lost (usually directly after ball is
      // touched), bias may turn around the other way. (well, maybe that's not a
      // problem. dunno.) if (p1->HasPossession() == true) bounceBias -= 0.3f;
      ...
      if (p1 == p1->GetTeam()->GetDesignatedTeamPossessionPlayer() &&
        ...
        float p1BallDistance =
            (GetBall()->Predict(10).Get2D() - p1->GetPosition()).GetLength();
        float p2BallDistance =
            (GetBall()->Predict(10).Get2D() - p2->GetPosition()).GetLength();
        float ballDistanceDiffFactor =
            clamp(std::min(p2BallDistance, 1.2f) -
                      std::min(p1BallDistance, 1.2f),
                  -0.6f, 0.6f) *
            1.0f; // std::min is cap so difference won't matter if ball is far
                  // away (so only used in battles about the ball)
      ...
      Vector3 offset1 = (p1pos - p2pos).GetNormalized(0) *
                        (bouncePlayerRadius - distance * 0.5f) *
                        (1.0f - bounceBias0to1) * 2.0f;
      Vector3 offset2 = (p2pos - p1pos).GetNormalized(0) *
                        (bouncePlayerRadius - distance * 0.5f) *
                        bounceBias0to1 * 2.0f;
      ...
      */
      ...
      // make players snap to the side of opponents (rather, just a bit in front
      // of them too) todo: make less binary, and more based on stats. maybe
      // make this whole push/pull thing a separate system?

      if (GetDesignatedPossessionPlayer() == p2 && p2->HasPossession()) {
        Vector3 p2_leftside =
            p2pos + p2->GetDirectionVec().GetRotated2D(0.3f * pi) *
                        bouncePlayerRadius * 2;
        ...
        Vector3 p2side =
            p1_to_p2_left < p1_to_p2_right ? p2_leftside : p2_rightside;
        // SetYellowDebugPilon(p2side);
        offset1 += (p2side - p1pos).GetNormalizedMax(0.01f) *
                   p1->GetStat("physical_balance") * 0.3f;
      }

      else if (GetDesignatedPossessionPlayer() == p1 && p1->HasPossession()) {
        Vector3 p1_leftside =
            p1pos + p1->GetDirectionVec().GetRotated2D(0.3f * pi) *
                        bouncePlayerRadius * 2;
        ...
        Vector3 p1side =
            p2_to_p1_left < p2_to_p1_right ? p1_leftside : p1_rightside;
        // SetRedDebugPilon(p1side);
        offset2 += (p1side - p2pos).GetNormalizedMax(0.01f) *
                   p2->GetStat("physical_balance") * 0.3f;
      }
    ...
    if (similarForceFactor > 0.0f &&
        distance < (bouncePlayerRadius + similarPlayerRadius) * 2.0f) {
      float shellDistance =
          std::max(0.0f, distance - bouncePlayerRadius * 2.0f);
      ...
      float p1velocity = p1->GetFloatVelocity();
      float p2velocity = p2->GetFloatVelocity();
      similarBias -= clamp(((p1velocity - p2velocity) / sprintVelocity) * 0.2f,
                           -0.2f, 0.2f);
      ...
      float similarForce = clamp(
          1.0f - (shellDistance / (similarPlayerRadius * 2.0f)), 0.0f, 1.0f);
    ...
    if (distance < bouncePlayerRadius * 2.0f) {
      ...
      float p2velocity = p2->GetFloatVelocity();
      p1sensitivity +=
          NormalizedClamp(p1velocity, idleVelocity, sprintVelocity) * 1.0f;
      p2sensitivity +=
          NormalizedClamp(p2velocity, idleVelocity, sprintVelocity) * 1.0f;
      ...
      p1sensitivity +=
          (1.0f - p1->GetStat("physical_balance") * 1.0f) * balanceWeight;
      p2sensitivity +=
          (1.0f - p2->GetStat("physical_balance") * 1.0f) * balanceWeight;

      if ((p1->GetDebug() || p2->GetDebug()) && verbose)
        printf("balance: %f - %f; ", p1sensitivity, p2sensitivity);

      p1sensitivity +=
          clamp(p1->GetDecayingPositionOffsetLength() * 10.0f, 0.0f, 1.0f);
      p2sensitivity +=
          clamp(p2->GetDecayingPositionOffsetLength() * 10.0f, 0.0f, 1.0f);
      ...
      // pow(1.0f - NormalizedClamp(penetration, 0.0f, bouncePlayerRadius
      // * 2.0f), 0.4f));
      p1sensitivity += pow(1.0f - NormalizedClamp(penetration, 0.0f,
                                                  bouncePlayerRadius * 2.0f),
                           0.4f) *
                       penetrationWeight;
      p2sensitivity += pow(1.0f - NormalizedClamp(penetration, 0.0f,
                                                  bouncePlayerRadius * 2.0f),
                           0.4f) *
                       penetrationWeight;
      ...
      // ball proximity (usually means: stability is less because we sacrifice
      // balance to control the ball)
      float p1BallDistance =
          (GetBall()->Predict(10).Get2D() - p1->GetPosition()).GetLength();
      float p2BallDistance =
          (GetBall()->Predict(10).Get2D() - p2->GetPosition()).GetLength();
      ...
      if (p1sensitivity > trip0threshold) {
        ...
        if (p1sensitivity > trip2threshold)
          tripType = 2;
        if (tripType > 0) {
          p1->TripMe((p1->GetMovement() * 0.1f + p2->GetMovement() * 0.06f +
                      bounceVec * 1.0f)
      ...
      if (p2sensitivity > trip0threshold) {
        ...
        if (p2sensitivity > trip2threshold)
          tripType = 2;
        if (tripType > 0) {
          p2->TripMe((p2->GetMovement() * 0.1f + p1->GetMovement() * 0.06f -
                      bounceVec * 1.0f)
                         .GetNormalized(-bounceVec),
                     tripType);
  ...
  if ((p2->GetCurrentFunctionType() == e_FunctionType_Sliding ||
    ...
    tackle += 2;
  if (distance < 2.0f && tackle > 0 &&
      tackle < 3) { // if tackle is 3, ignore both
    ...
    if (tackle == 0) { // todo: this way, p1 would have an advantage
      if (p1->GetCurrentFunctionType() == e_FunctionType_Trap ||
          ...
          p1->GetCurrentFunctionType() == e_FunctionType_Interfere) {
        p1->GetHumanoidNode()->GetObjects(e_ObjectType_Geometry,
    ...
    victimObjectList); p1action = true;
               ...
               p2->GetCurrentFunctionType() == e_FunctionType_Interfere) {
        p2->GetHumanoidNode()->GetObjects(e_ObjectType_Geometry,
    ...
    */
    ...
    std::list<boost::intrusive_ptr<Geometry>>::iterator objIter =
        tacklerObjectList.begin();
    while (objIter != tacklerObjectList.end()) {
      ...
      std::list<boost::intrusive_ptr<Geometry>>::iterator victimIter =
          victimObjectList.begin();
...
void Match::CheckBallCollisions() {
  ...
  // printf("%i - %i hihi\n", actualTime_ms, lastBodyBallCollisionTime_ms +
  // 150);
  ...
  Vector3 bounceVec;
  float bias = 0.0;
  int bounceCount = 0; // this shit is shit, average properly in combination
                       // with bias or something like that

  // printf("lasttouchbias: %f, isnul?: %s\n", GetLastTouchBias(200),
  // GetLastTouchBias(200) == 0.0f ? "true" : "false");
  for (int i = 0; i < (signed int)players.size(); i++) {
    ...
    float oppLastTouchBias =
        GetTeam(abs(teamID - 1))->GetLastTouchBias(touchTimeThreshold_ms);
    float lastTouchBias = players[i]->GetLastTouchBias(touchTimeThreshold_ms);
    float oppLastTouchBiasLong =
        GetTeam(abs(teamID - 1))->GetLastTouchBias(1600);

    if (lastTouchBias <= 0.01f &&
        oppLastTouchBias >
            0.01f /* && ballTowardsPlayer*/) { // cannot collide if opp didn't
                                               // recently touch ball (we would
                                               // be able to predict ball by
                                               // then), or if player itself
                                               // already did (to overcome the
                                               // 'perpetuum collision' problem,
                                               // and to allow for 'controlled
                                               // ball collisions' in humanoid
                                               // class)
      ...
      // if (onlyWhenDirectionChangedUnexpectedly) directionChangedUnexpectedly
      // = true;

      if (Verbose())
        if (players[i]->GetCurrentFunctionType() == e_FunctionType_Deflect) {
          printf("onlyWhenDirectionChangedUnexpectedly: %i, "
                 "directionChangedUnexpectedly: %i\n",
      ...
      if (collisionAnim && !players[i]->HasUniquePossession() &&
        ...
        float boundingBoxSizeOffset =
            -0.1f; // fake a big AABB for more blocking fun, or a small one for
                   // less bouncy bounce
        ...
        if (((players[i]->GetPosition() + Vector3(0, 0, 0.8f)) -
          ...
          players[i]->GetHumanoidNode()->GetObjects(e_ObjectType_Geometry,
                                                    objectList);
          ...
          while (objIter != objectList.end()) {
            ...
            if (objAABB.Intersects(ball->Predict(0), ballRadius)) {
              ...
              if (players[i] == players[i]
                                    ...
                                    ->GetDesignatedTeamPossessionPlayer() &&
                  GetLastTouchBias(200) <
                      0.01f) { // todo: use reaction time stat
                ...
                bounceVec +=
                    (ball->Predict(0) - (*objIter)->GetDerivedPosition())
                ...
                bias +=
                    (1.0f - clamp(((ball->Predict(0) - aabbCenter).GetLength() -
                                   ballRadius) /
                                      objAABB.GetRadius(),
                                  0.0f, 1.0f)) *
                        0.9f +
  ...
  if (bias > 0.0f) {
    ...
    Vector3 fullCollisionVec =
        (bounceVec * 6.0f) + (bounceVec * currentMovement.GetLength() * 0.6f) +
        (currentMovement * -0.2f);
    bias = clamp(bias, 0.0f, 1.0f);
    bias = bias * 0.5f + 0.5f;
    Vector3 resultVector =
        fullCollisionVec * bias + currentMovement * (1.0f - bias);
    if (resultVector.GetLength() > currentMovement.GetLength())
      resultVector =
          resultVector.GetNormalized(0) * currentMovement.GetLength();
    // resultVector = resultVector.GetNormalized(0) *
    // (currentMovement.GetLength() * 0.7f + resultVector.GetLength() * 0.3f);
    // // EXPERIMENT!
    resultVector *= 0.7f;

    ball->Touch(resultVector);
    ball->SetRotation(random(-30, 30), random(-30, 30), random(-30, 30),
                      0.5f * bias);
    ball->TriggerBallTouchSound(
        pow(NormalizedClamp(resultVector.GetLength(), 4.0f, 40.0f), 0.7f));
...
void Match::FollowCamera(Quaternion &orientation, Quaternion &nodeOrientation,
  ...
  nodeOrientation.SetAngleAxis(targetPosition.GetAngle2D() + 1.5 * pi,
                               Vector3(0, 0, 1));
  position = targetPosition -
             targetPosition.Get2D().GetNormalized(Vector3(0, -1, 0)) * 10 *
                 (1.0f / zoom) +
             Vector3(0, 0, 3);
  FOV = 60.0f;
}

void Match::SetReplayCamera(int camType, const Vector3 &target,
                            float modifierValue) {
  ...
  case 0: {
    ...
    cameraFOV = 30 * zoom;
    cameraNodePosition =
        Vector3(target.coords[0], target.coords[1] - 50.0f, 20.0f);
  ...
  case 1: {
    ...
    float targetDist =
        clamp((cameraNodePosition - target).GetLength() / 100.0f, 0.2f, 1.0f);
    cameraFOV = (56 - targetDist * 50) * zoom;
    cameraOrientation.SetAngleAxis((0.3f + targetDist * 0.15f) * pi,
                                   Vector3(1, 0, 0));
    cameraNodeOrientation.SetAngleAxis(
        (target - cameraNodePosition).GetAngle2D() + 1.5f * pi,
        Vector3(0, 0, 1));
    cameraNearCap = 20.0;
    cameraFarCap = 250.0;
  } break;
...
int Match::GetReplaySize_ms() { return replaySize_ms; }

int Match::GetReplayCamCount() { return 4; }
    ...
    for (int i = 0; i < triangleMesh.at(m).verticesDataSize /
      ...
      if (goalID >= 0) {
        nettingMeshesSrc[goalID].push_back(
            Vector3(triangleMesh.at(m).vertices[i + 0],
  ...
  if (ballTouchesNet) {
    ...
    for (unsigned int i = 0; i < nettingMeshes[sideID].size(); i++) {
      ...
      float woodworkTensionBiasInv = clamp(
          (fabs(ball->GetBallGeom()->GetPosition().coords[0]) - pitchHalfW) *
              2.0f,
          0.0f, 1.0f);
      ...
      if (influenceBias > 0.0f) {
        Vector3 result = vertex * (1.0f - influenceBias) +
                         ball->GetBallGeom()->GetPosition() * influenceBias;
    ...
    for (int sideID = 0; sideID < 2; sideID++) {
      for (unsigned int i = 0; i < nettingMeshes[sideID].size(); i++) {
        static_cast<float *>(nettingMeshes[sideID][i])[0] =
            nettingMeshesSrc[sideID][i].coords[0];
        static_cast<float *>(nettingMeshes[sideID][i])[1] =
            nettingMeshesSrc[sideID][i].coords[1];
        static_cast<float *>(nettingMeshes[sideID][i])[2] =
            nettingMeshesSrc[sideID][i].coords[2];
      }
    }
    resetNetting = false;
    nettingHasChanged = true;
...
missingAnims.at(i).timesMissed + someAnim.angleDifference) /
(missingAnims.at(i).timesMissed + 1.0f); missingAnims.at(i).timesMissed++; found
= true; break;
    }
  }
  if (!found) {
