...
Vector3 AI_GetAdaptedInitialPos(Match *match, const Vector3 &initialPosition,
                                ...
                                float ballMagnetDistance = 50.0,
                                float ballMagnetDistancePow = 1.5);
...
TimeNeeded AI_GetTimeNeededForDistance_ms(
    ...
    bool precise = false, int maxTime_ms = -1, bool debug = false);
...
Vector3
AI_GetShotDirection(Player *player, const Vector3 &inputDirection,
                    float autoDirectionBias =
                        1.0f); // this is to get a 'rough' idea of where to
                               // shoot, so we can pick the proper animation.
                               // exact direction will be tweaked later on.
