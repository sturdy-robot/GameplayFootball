...
// todo: do we need "match" stuff into MentalImage? especially GetPlayer, at the
// moment of writing that's the only method used here, so we could get rid of
// the match pointers.
...
Vector3 AI_GetAdaptedInitialPos(Match *match, const Vector3 &initialPosition,
                                Vector3 focusPoint, float ballMagnetDistance,
  ...
  boundedInitialPosition.coords[0] =
      clamp(boundedInitialPosition.coords[0], -1.0, 1.0);
  boundedInitialPosition.coords[1] =
      clamp(boundedInitialPosition.coords[1], -1.0, 1.0);
  ...
  if (ballDistance < ballMagnetDistance)
    ballMagnetOffset =
        ...
        pow(((ballMagnetDistance - ballDistance) / ballMagnetDistance),
            ballMagnetDistancePow);
...
Vector3 AI_GetAdaptedFormationPosition(
  ...
  if (midfieldFocusStrength > 0.0f) {
    ...
    float stretchBias =
        clamp(1.0f - fabs(position.coords[0] * 1.2f), 0.0f,
    ...
    // position.coords[0]);
    position.coords[0] = position.coords[0] * (1.0f - stretchBias) +
  ...
  position.coords[0] =
      backXBound + (position.coords[0] * 0.5f + 0.5f) * xLength;
  float yLength = highYBound - lowYBound;
  position.coords[1] =
      lowYBound + (position.coords[1] * -side * 0.5f + 0.5f) * yLength;

  Vector3 purePosition = position;

  if (xFocusStrength > 0.0f) {
    float bias = 1.0f - clamp(fabs(xFocus - position.coords[0]) /
                                  fabs(backXBound - frontXBound),
                              0.0f, 1.0f);
  ...
  if (yFocusStrength > 0.0f) {
    float distance =
        clamp(fabs(yFocus - position.coords[1]) / fabs(highYBound - lowYBound),
              0.0f, 1.0f);
  ...
  if (microFocusStrength > 0.0f) {
    ...
    float homogeneousYInfluenceBias =
        0.2f; // 1.0f == act as if everybody is on the same Y position (so, as
              // if we're in 1D, with only X)
    float homogeneousYPositionBias =
        0.4f; // 1.0f == don't influence player's resulting Y position (so, as
              // if we're in 1D, with only X)

    // this way, players are more strictly keeping to their positions
    float distStatic = ((microFocus - purePosition) *
                        Vector3(1, 1.0f - homogeneousYInfluenceBias, 0))
                           .GetLength() /
                       50.0f; // 26.0f;//32.0f;
    // this way, microfocus gets more direct attention from players around the
    // action - they kinda 'forget' their base positions a bit though
    // float distDynamic = (microFocus - player->GetPosition()).GetLength()
    // / 34.0f;
    ...
    if (dist < 1.0f) {
      /*
      // wolfram alpha: (sin((x + 0.5) * pi) * 0.5 + 0.5) * 0.7 + (sin((x -
      0.25) * 2.0 * pi) * 0.5 + 0.5) * 0.3 | from x = 0 to 1 float
      microFocusBias1 = sin((dist + 0.5f) * pi) * 0.5f + 0.5f; // -\_ float
      microFocusBias2 = sin((dist - 0.25f) * 2.0f * pi) * 0.5f + 0.5f; // _/-\_
      // mates in team in possession want to keep some distance for easier
      pass-to-ability, while defenders just want to jump into the action more
      // todo ^ this is bullshit, right? as in, it's not about the target
      position, but about the target players to reposition (towards the focus)..
      float sineBias = 0.15f;// +
      clamp(player->GetTeam()->GetFadingTeamPossessionAmount() - 0.5f,
      0.0f, 1.0f) * 0.2f; float microFocusBias = microFocusBias1 * (1.0f -
      sineBias) + microFocusBias2 * sineBias;
      */

      float microFocusBias = 1.0f;

      // less serious when far away
      // we need this, because we use bias to vary between pos and
      // microfocuspos. so for far away players, microfocuspos has way more
      // influence since it's farther away.
      ...
      microFocusBias +=
          (1.0f - NormalizedClamp(fabs(dist - peakLocation), 0.0f, peakWidth)) *
          peakHeight;
      ...
      Vector3 microFocusPosition =
          microFocus * Vector3(1, 1.0f - homogeneousYPositionBias, 1) +
          position * Vector3(0, homogeneousYPositionBias, 0);
      position = position * (1.0f - microFocusBias) +
                 microFocusPosition * (microFocusBias);
    }
  }
...
float AI_CalculatePassingOdds(
    Match *match, const Vector3 &origin, const Vector3 &target,
  ...
  Vector3 step = (target - origin) * (1.0 / (float)checkCount) *
                 0.96; // * 0.96: we don't mind players standing behind target
                       // that much anyway
  ...
  for (int i = 1; i < checkCount + 1; i++) {
    ...
    float maxOpponentDistance =
        0.5 + currentDistance *
                  0.25; // at this distance, opponents start being a threat

    for (int opp = 0; opp < (signed int)opponentPlayerImages.size(); opp++) {
      float opponentDistance =
          (opponentPlayerImages.at(opp).position - ballPos).GetLength();
      float odds =
          clamp(opponentDistance, 0, maxOpponentDistance) / maxOpponentDistance;
      if (odds < currentOdds)
        currentOdds = odds;
    }
  }
...
float AI_CalculatePassingOdds(
    Match *match, PlayerImage thisPlayerImage, PlayerImage targetPlayerImage,
    const std::vector<PlayerImage> &opponentPlayerImages) {

  // player position predictions
  thisPlayerImage.position =
      thisPlayerImage.position +
      thisPlayerImage.directionVec * thisPlayerImage.velocity * 0.1;
  targetPlayerImage.position =
      targetPlayerImage.position + targetPlayerImage.directionVec *
                                       targetPlayerImage.velocity *
                                       0.5; // situation in half a second

  float currentOdds =
      AI_CalculatePassingOdds(match, thisPlayerImage.position,
                              targetPlayerImage.position, opponentPlayerImages);

  return currentOdds;
}

void AI_GetPassRatings(Match *match, int thisPlayerID,
                       const MentalImage *mentalImage, float opportunism,
                       PassRatings &passRatings) {
  ...
  mentalImage->GetTeamPlayerImages(match->GetPlayer(thisPlayerID)->GetTeamID(),
                                   thisPlayerID, playerImages);
  ...
  for (int i = 0; i < (signed int)opponentPlayerImages.size(); i++) {
    opponentPlayerImages.at(i).position =
        opponentPlayerImages.at(i).position +
        opponentPlayerImages.at(i).directionVec *
  ...
  for (int i = 0; i < (signed int)playerImages.size(); i++) {
    ...
    PlayerImage targetPlayerImage =
        mentalImage->GetPlayerImage(playerImages.at(i).playerID);

    float bodyDirPenalty = thisPlayerImage.directionVec.GetDotProduct(
        (targetPlayerImage.position - thisPlayerImage.position)
            .GetNormalized(Vector3(side, 0, 0)));
    bodyDirPenalty = clamp(bodyDirPenalty, -1.0, 0.0) + 1; // 0 .. 1
    bodyDirPenalty *= 0.7;
    bodyDirPenalty += 0.3; // 0.3 .. 1.0

    float odds = AI_CalculatePassingOdds(
        match, thisPlayerImage, targetPlayerImage, opponentPlayerImages);
    odds *= bodyDirPenalty;

    // distance to goal rating
    float goalDistance =
        clamp((Vector3(pitchHalfW * -side, 0, 0) - thisPlayerImage.position)
                  .GetLength(),
              8.0, 100.0) *
        0.01; // ~= 0.18 .. 1.0
    float mateGoalDistance =
        clamp((Vector3(pitchHalfW * -side, 0, 0) - playerImages.at(i).position)
                  .GetLength(),
              8.0, 100.0) *
        0.01; // ~= 0.18 .. 1.0
    ...
    // situational rating
    float sit =
        AI_GetSituationRating(match, playerImages.at(i).playerID, mentalImage);

    PassRating passRating(playerImages.at(i).playerID, pow(odds, 0.8),
                          pow(pos, 0.7), pow(sit, 0.6));
...
float AI_GetSituationRating(Match *match, int thisPlayerID,
  ...
  std::vector<PlayerImage> opponentPlayerImages;
  mentalImage->GetTeamPlayerImages(
      abs(match->GetPlayer(thisPlayerID)->GetTeamID() - 1), -1,
      opponentPlayerImages);

  // player position predictions
  thisPlayerImage.position =
      thisPlayerImage.position + thisPlayerImage.directionVec *
  ...
  for (int i = 0; i < (signed int)opponentPlayerImages.size(); i++) {
    opponentPlayerImages.at(i).position =
        ...
        opponentPlayerImages.at(i).directionVec *
            ...
            0.5; // situation in half a second
    float situation =
        clamp((opponentPlayerImages.at(i).position - thisPlayerImage.position)
                  .GetLength(),
              0, safeDistance) /
        safeDistance;
...
float AI_CalculateFreeSpace(Match *match, const MentalImage *mentalImage,
                            ...
                            bool ignoreKeeper) {
  // void AI_GetClosestPlayers(Team *team, const Vector3 &position, bool
  // onlyAIControlled, std::vector<Player*> &result, unsigned int playerCount)
  //  todo: only closest?
  ...
  for (int i = 0; i < (signed int)opponentPlayerImages.size(); i++) {
    if (ignoreKeeper == false ||
        opponentPlayerImages.at(i).dynamicFormationEntry.role !=
            e_PlayerRole_GK) {
      ...
      Vector3 toFocusMovement =
          (focusPos - opponentPlayerImages.at(i).position).GetNormalized(0) *
      ...
      float situation =
          1.0f -
          clamp((opponentPlayerImages.at(i).position - focusPos).GetLength(), 0,
                safeDistance) /
              safeDistance;
...
float AI_GetOffsideLine(Match *match, const MentalImage *mentalImage,
  ...
  // offside: we are actually looking for the one-but-deepest opponent
  // (association football rule! EAT THAT, PES6!! :P)
  for (int i = 0; i < (signed int)opponentPlayerImages.size(); i++) {
    if (opponentPlayerImages.at(i).position.coords[0] * side >
...
void AI_GetBestDribbleMovement(Match *match, int thisPlayerID,
                               const MentalImage *mentalImage,
  ...
  AI_GetClosestPlayers(match->GetTeam(abs(team->GetID() - 1)), myPos, false,
                       opponents, 5);
  for (unsigned int i = 0; i < opponents.size(); i++) {
    opponentPlayerImages.push_back(
        mentalImage->GetPlayerImage(opponents.at(i)->GetID()));
  }

  float nearBackline = NormalizedClamp(
      fabs(player->GetPosition().coords[0]) / pitchHalfW, 0.0f, 1.0f);
  float centerModifierInv =
      1.0f -
      std::pow(nearBackline,
               2.0f); // near the end of the pitch, we want to get inside again
  centerModifierInv *=
      0.5f; // stop going to the sides! wtf, todo, why does it prefer the
            // sideline so much (probably because of no opponents :P)
  Vector3 oppGoalPos = Vector3(-side * pitchHalfW,
                               myPos.coords[1] *
                                   (1.0f - teamTactics.userProperties.GetReal(
                                               "dribble_centermagnet", 0.5f)) *
                                   centerModifierInv,
                               0);
  ...
  {
    ForceSpot spot;
    spot.origin = Vector3(myPos.coords[0],
                          (pitchHalfH + 5.0f) * signSide(myPos.coords[1]), 0);
    spot.magnetType = e_MagnetType_Repel;
    spot.decayType = e_DecayType_Variable;
    ...
    spot.origin = Vector3((pitchHalfW + 5.0f) * signSide(myPos.coords[0]),
                          myPos.coords[1], 0);
    spot.magnetType = e_MagnetType_Repel;
    spot.decayType = e_DecayType_Variable;
    spot.power = 4.0f * powerMultiplier;
  ...
  desiredDirection =
      forceFieldMovement.GetNormalized(player->GetDirectionVec());
  desiredVelocity =
      clamp(forceFieldMovement.GetLength() * distanceToVelocityMultiplier,
...
Vector3 AI_GetForceFieldMovement(const std::vector<ForceSpot> &forceField,
  ...
  // attractorDampingDistance: from this distance to attractor, dampen influence
  // so we won't overshoot target
...
TimeNeeded AI_GetTimeNeededForDistance_ms(const Vector3 &playerPos,
  ...
  float radius_usual =
      0.28f; // starting distance from our base position where we can reach
             // balls (effectively: leg extension length)
  ...
  float adaptedMaxVelocity =
      maxVelocity *
      0.94f; // don't use full maxvelocity, since the last part of that velo is
             // very hard to attain (due to exponential air resistance)

  while (true) { // =]

    // too unstable! timeStep_ms = clamp(int(round(previousDistance * 30)) - 20,
    // 10, 40); // variable timestep may not be 100% correct, so don't overdo it
    // round to 10s
    // timeStep_ms = int(floor(timeStep_ms / 10.0f)) * 10;

    float bias =
        clamp((float)currentTime_ms / (float)changeTime_ms, 0.0f, 1.0f);
    ...
    if (bias >= 1.0f) {
      ...
      float remainingDistance_usual = clamp(
          (targetPos - currentPos).GetLength() - radius_usual, 0.0f, 100000.0f);
      result.usual_ms = currentTime_ms +
                        (remainingDistance_usual / adaptedMaxVelocity) * 1000;
      resultingRadius_usual = radius_usual + remainingDistance_usual;

      if (!foundOptimisticTime) {
        float remainingDistance_optimistic =
            clamp((targetPos - currentPos).GetLength() - radius_optimistic,
                  0.0f, 100000.0f);
        result.optimistic_ms =
            currentTime_ms +
            (remainingDistance_optimistic / adaptedMaxVelocity) * 1000;
      }
      ...
      // manual copy: this function is called a lot, so if we use the
      // Vector3::operator*, it's creating a lot of temp vars. currentMovement =
      // playerMovement * (1.0f - bias);
      ...
      // if (currentTime_ms > 1000 && currentTime_ms % 100 == 0)
      // printf("currentTime_ms: %i, targetDistance: %f, currentMovementLength:
      // %f, bias: %f, radius: %f, changeTime_ms: %i\n", currentTime_ms,
      // targetDistance, currentMovement.GetLength(), bias, radius,
      // changeTime_ms); if (debug) if (maxTime_ms != -1 && currentTime_ms ==
      // (unsigned int)maxTime_ms) printf("CANNOT GO FURTHER: %i\n",
      // currentTime_ms);
      ...
      if (targetDistance < radius_usual ||
          (maxTime_ms != -1 && currentTime_ms > (unsigned int)maxTime_ms)) {
        // currentTime_ms += int(round(((targetPos - currentPos).GetLength() /
        // radius) * 40.0)); if (debug) printf("ACTUALLY FOUND AT: %i\n",
        // currentTime_ms);
  ...
  if (maxTime_ms != -1 && currentTime_ms > (unsigned int)maxTime_ms) {
    result.usual_ms =
        std::max(defaultOptimizedTime_ms, (currentTime_ms + 100) * 2);
    if (!foundOptimisticTime)
      result.optimistic_ms = result.usual_ms;
    return result;
  }

  // very, very close! just take distance as time, so we can still compare to
  ...
  /* too simple version
  return int(round((targetPos - (playerPos + playerMovement * 0.02)).GetLength()
  / (sprintVelocity * 0.9) * 1000));
  */
}
...
unsigned int AI_GetToBallMovement(Match *match, const MentalImage *mentalImage,
  ...
  // precalc shortest distance to ball 'line' for perpendicularity rating (so we
  // have a minimum value there)

  Line ballLine(mentalImage->GetBallPrediction(0).Get2D(),
                mentalImage->GetBallPrediction(1000).Get2D());
  float u = 0;
  float playerBallLineShortestDistance =
      ballLine.GetDistanceToPoint(playerPos, u);
  Vector3 playerBallShortestTargetPos =
      ballLine.GetVertex(0) +
      (ballLine.GetVertex(1) - ballLine.GetVertex(0)) * u;

  // quantize input 8- or 16-way in relation to 'ball direction space'

  Vector3 ballSpaceDirection = (mentalImage->GetBallPrediction(0).Get2D() -
                                mentalImage->GetBallPrediction(1000).Get2D())
  ...
  float bias = 1.0f;
  adaptedDesiredDirectionBallSpace =
      (adaptedDesiredDirectionBallSpace * (1.0 - bias) +
       (Vector3(1, 0, 0).GetRotated2D(angle) * bias))
          .GetNormalized(adaptedDesiredDirectionBallSpace);

  adaptedDesiredDirection =
      adaptedDesiredDirectionBallSpace.GetRotated2D(toBallSpaceAngle);
  ...
  /*
    // only use player desired movement if it's a lot different from
    perpendicular float desiredVsPerpendicularDot =
    adaptedDesiredDirection.GetDotProduct((playerBallShortestTargetPos -
    playerPos).GetNormalized(0));
    //if (desiredVsPerpendicularDot < 0.3f) movementWeight = 1.0f; else
    perpendicularWeight = 1.0f; float desiredVsPerpendicularBias =
    pow(NormalizedClamp(desiredVsPerpendicularDot, 0.0f, 1.0f), 0.5f);
    // if (player->GetDebug()) {
    //   SetGreenDebugPilon(playerPos + (playerBallShortestTargetPos -
    playerPos).GetNormalized(0) * 5.0f);
    //   SetYellowDebugPilon(playerPos +
    adaptedDesiredDirection.GetNormalized(0) * 5.0f);
  ...
  */
  ...
  unsigned int startTime_ms = clamp(player->GetTimeNeededToGetToBall_ms(), 40,
                                    ballPredictionSize_ms - 10);
  // we can't start optimized at a later moment, because that would mean we'd
  // aim outside the pitch to start with
  ...
  float ffoLength =
      GetFrontOfFootOffsetRel(
          desiredVelocityFloat * 0.2f + player->GetFloatVelocity() * 0.8f, 0, 0)
          .GetLength();
  ...
  Vector3 ballMovementRough =
      (mentalImage->GetBallPrediction(player->GetTimeNeededToGetToBall_ms() +
       ...
       mentalImage->GetBallPrediction(player->GetTimeNeededToGetToBall_ms()))
          .Get2D() *
      100.0f;
  float desiredBallDot =
      ballMovementRough.GetNormalized(0).GetDotProduct(adaptedDesiredDirection);
  Vector3 ballDirectionRough =
      ballMovementRough.GetNormalized(adaptedDesiredDirection);
  ...
  for (unsigned int time_ms = startTime_ms; time_ms < ballPredictionSize_ms;
    ...
    TimeNeeded timeNeeded = AI_GetTimeNeededForDistance_ms(
        playerPos, player->GetMovement(), targetPos, player->GetMaxVelocity(),
        true, time_ms);
    ...
    if (timeNeeded_ms <= time_ms) {
      ...
      // todo: for this to work, it would have to only remove distance
      // perpendicular to ballmovement
      // targetPos -= (targetPos - playerPos).GetNormalizedMax(ffoLength);//
      // 0.25f;

      float dot = (targetPos - playerPos)
                      .GetNormalized(0)
                      .GetDotProduct(ballDirectionRough);
      ...
      < sprintVelocity * 2.0f) { if (dot >= 0.4f) forced = true; // when ball
      goes slower, allow less walking in ball movement direction
      }
      */
      // if (dot >= 0.0f) forced = true; // force <= perpendicular
      float justInTimeFactor =
          clamp(player->GetTimeNeededToGetToBall_ms() / (float)time_ms, 0.0f,
      ...
      // LANCHANGE if (dot > 0.45f) forced = true; // too shallow angle, just go
      // to ball already
      if (dot > 0.0f)
      ...
      float targetVelocity =
          clamp(targetDistance * distanceToVelocityMultiplier, idleVelocity,
                sprintVelocity);
      Vector3 targetMovement =
          (targetPos - playerPos).GetNormalized(0) * targetVelocity;
      float movementRating =
          1.0f - NormalizedClamp(
                     (desiredMovement - targetMovement).GetLength(), 0.0f,
                     sprintVelocity); // > sprintvelocity will often get us
                                      // farther from where we want to go to
      float directionRating =
          1.0f -
          NormalizedClamp(
              fabs(adaptedDesiredDirection.GetAngle2D(
                  targetMovement.GetNormalized(adaptedDesiredDirection))),
              0.0f, 0.5f * pi); // > 0.5f * pi will often only get us farther
                                // from where we want to go to
      movementRating =
          movementRating * 0.4f +
          directionRating * 0.6f; // directionrating omits velocity and
                                  // therefore has another quality
      ...
      float perpendicularRating =
          1.0f -
          NormalizedClamp((playerBallShortestTargetPos - targetPos).GetLength(),
                          0, 2.0f * sprintVelocity);
      ...
      if (previousDesiredTargetTime_ms >= timeNeededToGetToBall_ms) {
        previousTargetRating = 1.0f - abs(previousDesiredTargetTime_ms -
    ...
    if (forced) {
      if (bestChoice.time_ms ==
          -10) { // this happens if timeNeeded_ms > time_ms
        bestChoice.time_ms = time_ms;
        bestChoice.timeNeeded_ms =
            time_ms; // just fake it - get as close as possible
      }
      break;
    }
  }
  ...
  Vector3 targetPos =
      mentalImage->GetBallPrediction(bestChoice.time_ms).Get2D();
  float distance = (targetPos - playerPos).GetLength();
  bestDirection =
      (targetPos - playerPos).GetNormalized(player->GetDirectionVec());

  // velocity

  float bestVelocityRelaxed =
      clamp(distance *
                ((bestChoice.timeNeeded_ms + defaultTouchOffset_ms) /
                 (float)bestChoice.time_ms) *
                distanceToVelocityMultiplier,
            idleVelocity, sprintVelocity);
  float bestVelocityTimeBased =
      clamp((bestChoice.timeNeeded_ms / (float)bestChoice.time_ms) *
                player->GetMaxVelocity(),
            idleVelocity, sprintVelocity);
  float bestVelocityASAP = clamp(distance * distanceToVelocityMultiplier,
                                 idleVelocity, sprintVelocity);

  // simple version
  bestVelocityFloat = bestVelocityTimeBased; // bestVelocityRelaxed;
  // maybe just for sprinting? if (bestChoice.timeNeeded_ms > 500 &&
  // desiredVelocityFloat > bestVelocityFloat) bestVelocityFloat =
  // bestVelocityFloat * 0.5f + desiredVelocityFloat * 0.5f;//sprintVelocity;
  // if (distance > 0.25f * sprintVelocity) bestVelocityFloat =
  // clamp(desiredVelocityFloat, bestVelocityFloat, sprintVelocity); // allow
  // premature arrival at target if (bestVelocityFloat < desiredVelocityFloat &&
  // bestVelocityASAP <= desiredVelocityFloat) bestVelocityFloat =
  // bestVelocityASAP;
  // stick to current velo, to prevent switching back and forth
  if (bestChoice.time_ms > 250)
    bestVelocityFloat =
        bestVelocityFloat * 0.97f + player->GetFloatVelocity() * 0.03f;
  ...
  // if we're close to target, allow looking in desired direction more. if we
  // are farther away, we don't want to walk backwards yet, it makes no sense.
  if (bestChoice.time_ms < 100) {
    ...
    if (FloatToEnumVelocity(bestVelocityFloat) != e_Velocity_Idle) {
      lookAtDesiredDir =
          adaptedDesiredDirection.GetClamped2D(lookAtBall, lookAtDirection);
    } else {
      lookAtDesiredDir = adaptedDesiredDirection;
    }
  }
  ...
  // combine!
  bestLookAt = playerPos + lookAtBall.GetRotated2D(clamp(
                               lookAtDesiredDir.GetAngle2D(lookAtBall) * 1.0f,
                               -0.25f * pi, 0.25f * pi)) *
                               10.0f;
  // if (player->GetDebug()) SetYellowDebugPilon(playerPos + lookAtBall * 5.0f);
  // if (player->GetDebug()) SetRedDebugPilon(playerPos + lookAtDesiredDir
  // * 5.0f);
...
unsigned int
AI_GetBallControlMovement(const MentalImage *mentalImage, Player *player,
                          const Vector3 &desiredDirection,
                          float desiredVelocityFloat, Vector3 &bestDirection,
                          float &bestVelocityFloat, Vector3 &bestLookAt) {

  unsigned int desiredTimeToBall_ms = 250 + defaultTouchOffset_ms;

  Vector3 toBallMovement =
      mentalImage->GetBallPrediction(desiredTimeToBall_ms).Get2D() -
      player->GetPosition();
  ...
  Vector3 autoDirection =
      toBallMovement.GetNormalized(player->GetDirectionVec());
  Vector3 manualDirection = player->GetDirectionVec(); // desiredDirection;
  // test this: if (player->GetDirectionVec().GetDotProduct(desiredDirection) <
  // 0) manualDirection = desiredDirection;

  bestDirection = autoDirection * autoDirectionBias +
                  manualDirection * (1.0f - autoDirectionBias);
  ...
  // bestVelocityFloat = bestVelocityFloat * 0.5f +
  // RangeVelocity(bestVelocityFloat) * 0.5f; // quantization is the root of all
  // happiness
  if (bestVelocityFloat < dribbleVelocity) { // don't quantize low velos
    // bestVelocityFloat = idleVelocity;
  } else {
    float clampedDesiredVelocityFloat = clamp(
        desiredVelocityFloat, bestVelocityFloat, bestVelocityFloat + 8.0f);
    bestVelocityFloat = clampedDesiredVelocityFloat;
    if (RangeVelocity(bestVelocityFloat) < bestVelocityFloat)
      bestVelocityFloat =
          bestVelocityFloat * 0.9f + RangeVelocity(bestVelocityFloat) * 0.1f;
    if (RangeVelocity(bestVelocityFloat) > bestVelocityFloat)
      bestVelocityFloat =
          bestVelocityFloat * 0.1f + RangeVelocity(bestVelocityFloat) * 0.9f;
  }
...
float AI_GetTeamPossessionFactor(Match *match, Team *team) {
  ...
  int t2 =
      match->GetTeam(abs(team->GetID() - 1))->GetTimeNeededToGetToBall_ms();
  ...
  for (unsigned int i = 0; i < players.size(); i++) {
    if (players.at(i)->IsActive() && players.at(i) != except) {
      ...
      if (distance < closestDistance) {
        if (!onlyAIControlled ||
            !team->IsHumanControlled(players.at(i)->GetID())) {
...
Player *AI_GetBestSwitchTargetPlayer(Match *match, Team *team,
  ...
  Vector3 defensePosition = (actionPosition * Vector3(1.0f, 0.8f, 0.0f)) +
                            Vector3(team->GetSide() * 4.0f, 0.0f, 0.0f);
  Vector3 offensePosition = (actionPosition * Vector3(1.0f, 0.8f, 0.0f)) +
                            Vector3(-team->GetSide() * 8.0f, 0.0f, 0.0f);

  // experiment: also take team possession player into account, try to pick one
  // near
  defensePosition =
      defensePosition * 0.8f +
      team->GetDesignatedTeamPossessionPlayer()->GetPosition() * 0.2f;

  float offenseBias = team->GetFadingTeamPossessionAmount() - 0.5f;
  offenseBias = offenseBias * 0.2f +
                clamp(team->GetTeamPossessionAmount() - 0.5f, 0.0f, 1.0f) *
                    0.8f; // more direct, more urgent
  offenseBias =
      clamp((offenseBias - 0.5f) * 2.0 + 0.5f, 0.0f, 1.0f); // make more binary
  offenseBias =
  ...
  // in case of defending, we ideally need someone who is closer to our goal
  // than the opponent
  ...
  if (designated->GetTeamID() != team->GetID()) {
    ...
    float oppGoalDist =
        (goalPos - (opp->GetPosition() + opp->GetMovement() * 0.5f))
            .GetLength();
    for (unsigned int i = 0; i < teamPlayers.size(); i++) {
      float mateGoalDist = (goalPos - (teamPlayers.at(i)->GetPosition() +
                                       teamPlayers.at(i)->GetMovement() * 0.5f))
                               .GetLength();
      if (mateGoalDist < oppGoalDist + tooLateDistance +
                             clamp(oppGoalDist * 0.1f, 0.0f,
                                   3.0f)) { // doesn't matter much when opp is
...
void AI_GetAutoPass(e_FunctionType passType, const Vector3 &vector,
  ...
  if (passType == e_FunctionType_HighPass) {
    heightOffset = 0.45f - NormalizedClamp(vector.GetLength(), 0.0f, 60.0f) *
                               0.15f; // 0.37f;
    powerFactor = 1.15f;              // 1.75
    distanceExp = 1.4f;               // 1.6
  }
  resultingDirection =
      (vector.GetNormalized(0) + Vector3(0, 0, heightOffset)).GetNormalized(0);
  resultingPower =
...
void AI_GetPass(Player *player, e_FunctionType passType,
  ...
  Vector playerPos =
      player->GetPosition() +
      player->GetMovement().GetNormalized(0) *
          0.2f; // + ffo. don't need movement for future stuff, since getpass is
                // recalled at moment of passing, for refinement
  if (player->TouchAnim()) {
    playerPos = player->GetTouchPos().Get2D();
  }
  Vector3 manualTarget =
      playerPos + inputDirection * clamp(inputPower * 60.0f, 1.0f, 100.0f);
  ...
  if (forcedTargetPlayer) {
    ...
    float passDuration =
        0.3f + (forcedTargetPlayer->GetPosition() - playerPos).GetLength() *
    ...
    autoTarget = forcedTargetPlayer->GetPosition() +
                 forcedTargetPlayer->GetMovement() *
                     passDuration; // correct for pass duration
    ...
    for (int i = 0; i < (signed int)players.size(); i++) {
      if (players.at(i) != player /* && players.at(i)->IsActive()*/) {
        ...
        float passDuration =
            0.3f + (players.at(i)->GetPosition() - playerPos).GetLength() *
                       0.05f; // educated guess
        passDuration = std::pow(clamp(passDuration, 0.0f, 1.0f), 0.7f) *
                       0.7f; // after this time, the player is supposed to have
                             // been able to stop
        ...
        float angleRating = fabs((targetPos - playerPos)
                                     .GetNormalized(0)
                                     .GetAngle2D(inputDirection) /
                                 (1.0f * pi)) *
                            1.0f;
  ...
  if (forcedTargetPlayer || (fullAutoDirection && fullAutoPower)) {
    ...
    float distanceFactor =
        1.0 - std::pow(clamp((autoTargetRel - manualTargetRel).GetLength() /
                             ...
                             0.0f, 1.0f),
                       1.5f);
    // extra help with power on close targets (because people can only physicaly
    // press the pass button so short)
    ...
    if (fullAutoPower) {
      ...
      adaptedAutoPowerBias =
          clamp(adaptedAutoPowerBias * (1.0 + proximityBonus), 0.0f, 1.0f);
    }
  ...
  if (passType == e_FunctionType_LongPass) {
    float targetDistance =
        autoTargetRel.GetLength() * adaptedAutoDirectionBias +
        manualTargetRel.GetLength() * (1.0f - adaptedAutoDirectionBias);
    offset =
        Vector3(-player->GetTeam()->GetSide() * targetDistance * 0.2f, 0, 0);
  ...
  resultingTargetRel *=
      float(autoTargetRel.GetLength() * adaptedAutoPowerBias +
  ...
  AI_GetAutoPass(passType, resultingTargetRel, resultingDirection,
                 resultingPower);
}
...
Vector3 AI_GetShotDirection(Player *player, const Vector3 &inputDirection,
  ...
  Vector3 toGoal =
      (goalPos - (player->GetPosition() + player->GetMovement() * 0.12f))
          .GetNormalized(0);
  // if inputDirection ~== toGoal, it is considered as aiming 'through the
  // middle'. so, get the deviation from inputDirection to toGoal, and make 90
  // degrees the maximum
  ...
  goalPos.coords[1] =
      sideFactor * goalHalfWidth * 0.9f * player->GetTeam()->GetSide();
  Vector3 autoDirection =
      (goalPos - (player->GetPosition() + player->GetMovement() * 0.12f))
          .GetNormalized(0);
  ...
  return (manualDirection * (1.0f - autoDirectionBias) +
          autoDirection * autoDirectionBias)
...
float AI_GetMindSet(e_PlayerRole role) {
  ...
  if (role == e_PlayerRole_LM || role == e_PlayerRole_CM ||
