...
TeamAIController::TeamAIController(Team *team) : team(team) {
  ...
  baseTeamTactics.Set("position_offense_sidefocus_strength",
                      0.1f); // take possession factor more seriously (high) or
                             // ball position (low)
  ...
  // how much [-1 * this value .. +1 * this value] offset we can add to the
  // above tactics.
...
Player *SelectAttackingRunPlayer(Team *team) {
  ...
  Player *attackingRunPlayer =
      AI_GetClosestPlayer(team, offenseFocusPos, true, possessionPlayer);
...
void TeamAIController::Process() {
  ...
  float startDistance =
      30.0f +
      20.0f *
          offensivenessBias; // distance from goal where we start holding the
                             // opponents (but are likely to fallback somewhat)
  float forceDistance =
      6.0f; // minimum distance from goal - try to 'hold' the opp there

  float deepestDanger = (pitchHalfW - startDistance) * team->GetSide();

  // ball as max
  float adaptedBallX = match->GetBall()->Predict(0).coords[0];
  // when far away from our goal (startDistance), drop back more easily. closer
  // to forceDistance, don't buckle.
  adaptedBallX *= team->GetSide(); // > 0 == on our half (easier to work with)
  float offsetX =
      20.0f +
      10.0f *
          (1.0f - offensivenessBias); // when ball is this distance away from
                                      // startDistance, we start falling back
                                      // more towards forceDistance.
  float startToForcedBias = NormalizedClamp(
      adaptedBallX, pitchHalfW - startDistance - offsetX,
      pitchHalfW - forceDistance); // where, between startDistance and
                                   // forceDistance, is the ball? 0 .. 1
  adaptedBallX +=
      offsetX *
      (1.0f -
       startToForcedBias); // the fall back intensity gradually diminishes when
  ...
  // ballfuture as max
  if (match->GetBall()->Predict(700).coords[0] * team->GetSide() >
  ...
  float lineX =
      AI_GetOffsideLine(match, match->GetMentalImage(0), abs(team->GetID()));
  float allowSlackDistance = 4.0f; // despite teammates slacking behind line
                                   // this much, just hold the line
  if (lineX * team->GetSide() - allowSlackDistance >
      deepestDanger * team->GetSide()) {
    // printf("previous: %f\n", deepestDanger);
  ...
  */
  ...
  Vector3 mostDangerousPos =
      Vector3((pitchHalfW - 2.0) * team->GetSide(), 0, 0);
  mostDangerousPos =
      mostDangerousPos * 0.8f + match->GetBall()->Predict(100).Get2D() * 0.2f;
  ...
  for (unsigned int i = 0; i < players.size(); i++) {
    ...
    info.dangerFactor =
        1.0 - NormalizedClamp(
                  (players.at(i)->GetPosition() - mostDangerousPos).GetLength(),
                  0, pitchHalfW * 2);

    // player on ball is most dangerous
    info.dangerFactor *= 0.95f;
    if (players.at(i) == match->GetTeam(abs(team->GetID() - 1))
                             ->GetDesignatedTeamPossessionPlayer())
  ...
  if (match->GetActualTime_ms() % 500 == 0 &&
    ...
    if (team->GetHumanGamerCount() <
      ...
      if (match->GetBestPossessionTeamID() == team->GetID()) {
        ...
        // from a certain distance, running is not very useful (can't pass that
        // far)
        ...
        if (runner) {
          ...
          AI_GetClosestPlayers(
              team->GetMatch()->GetTeam(abs(team->GetID() - 1)), spot, false,
              opponents, 4);
          float oppDensityRating = 1.0f;
          for (unsigned int i = 0; i < opponents.size(); i++) {
            float oppDistance =
                (opponents.at(i)->GetPosition() - spot).GetLength();
...
Vector3 TeamAIController::GetAdaptedFormationPosition(
  ...
  float urgencyBias =
      1.0f - NormalizedClamp((focalPoint - player->GetPosition()).GetLength(),
                             2.0f, 30.0f);
  float ballX = match->GetBall()
                    ->GetAveragePosition(3500 * (1.0f - urgencyBias * 0.7f))
                    .coords[0];
  float ballY = match->GetBall()
                    ->GetAveragePosition(4000 * (1.0f - urgencyBias * 0.5f))
                    .coords[1];

  float offense_depthFactor =
      mixup(liveTeamTactics.GetReal("position_offense_depth_factor"),
            "position_offense_depth_factor", role);
  float defense_depthFactor =
      mixup(liveTeamTactics.GetReal("position_defense_depth_factor"),
            "position_defense_depth_factor", role);
  float offense_widthFactor =
      mixup(liveTeamTactics.GetReal("position_offense_width_factor"),
            "position_offense_width_factor", role);
  float defense_widthFactor =
      mixup(liveTeamTactics.GetReal("position_defense_width_factor"),
            "position_defense_width_factor", role);
  float offense_ownHalfFactor =
      mixup(liveTeamTactics.GetReal("position_offense_ownhalf_factor"),
            "position_offense_ownhalf_factor", role);
  float defense_ownHalfFactor =
      mixup(liveTeamTactics.GetReal("position_defense_ownhalf_factor"),
            "position_defense_ownhalf_factor", role);
  float offense_midfieldFocus =
      mixup(liveTeamTactics.GetReal("position_offense_midfieldfocus"),
            "position_offense_midfieldfocus", role);
  float defense_midfieldFocus =
      mixup(liveTeamTactics.GetReal("position_defense_midfieldfocus"),
  ...
  offense_sideFocusStrength =
      clamp(offense_sideFocusStrength + -0.3f + (offensivenessBias) * 0.3f,
            0.0f, 1.0f);
  defense_sideFocusStrength = clamp(defense_sideFocusStrength + -0.3f +
                                        (1.0f - offensivenessBias) * 0.3f,
                                    0.0f, 1.0f);
  if (player->GetDebug())
    printf("offense: %f, defense: %f\n", offense_sideFocusStrength,
           defense_sideFocusStrength);

  float possessionAmountBias =
      NormalizedClamp(fadingTeamPossessionAmount - 0.5f, 0.3f, 0.7f);
  // also take the ball's position as part of the possessionBias equation
  float ballBias =
      NormalizedClamp(((ballX / pitchHalfW) * -team->GetSide()), -0.7f,
  ...
  float ballBiasBias =
      1.0f - fabs(possessionAmountBias * 2.0f - 1.0f); // biasception
  ballBiasBias *= 0.6f; // don't take ballBias too serious - we need defenders
                        // to somewhat keep watch when possession team is
                        // unclear, even when the ball is forward
  float possessionBias =
      possessionAmountBias * (1.0f - ballBiasBias) + ballBias * ballBiasBias;

  possessionBias =
      clamp(possessionBias + (offensivenessBias - 0.5f) * 0.3f, 0.0f, 1.0f);

  // offense can be a bit more relaxed
  focalPoint = (match->GetBall()->GetAveragePosition(3000).Get2D() * 1.0f +
                focalPoint * 0.0f) *
                   possessionBias +
               (match->GetBall()->GetAveragePosition(2000).Get2D() * 0.5f +
                focalPoint * 0.5f) *
                   (1.0f - possessionBias);
  ...
  float sideFocusStrength =
      (offense_sideFocusStrength * possessionBias +
       defense_sideFocusStrength * (1.0f - possessionBias));

  float sideFocus =
      possessionBias * 2.0f - 1.0f; // -1 == own side, 1 == opp side
  ...
  float centerX =
      clamp((ballX * (1.0f - sideFocusStrength) + sideX * sideFocusStrength) +
                offsetX,
  ...
  float adaptCenterToFitDepthBias =
      0.95f; // the lower this value, the more players will move with the
             // centerX, and be clamped to the edges of the pitch
  float adaptCenterToFitWidthBias =
      0.9f; // the lower this value, the more players will move with the
            // centerY, and be clamped to the edges of the pitch
  ...
  // maybe setting offside trap this way doesn't work too well: after all,
  // defensive positioning stuff is done after this, thereby diminishing trap
  // sometimes. on the other hand, maybe this is a good basic trap setup, and we
  // could apply the trap with the applyoffsidetrap function after the defensive
  // positioning as well. (done from the def/mid strategies code)
  ...
  Vector3 microFocus =
      focalPoint; // * Vector3(1.0f, 0.8f + (1.0f - defendFactor) * 0.2f, 0.0f);
                  // // defenders stay in the middle more
  Vector3 defensiveFocusPos =
      Vector3(clamp(microFocus.coords[0] + team->GetSide() * 2.0f, -pitchHalfW,
                    backXBound * team->GetSide()),
              microFocus.coords[1] * 0.9f, 0);
  microFocus = (Vector3(clamp(microFocus.coords[0] - team->GetSide() * 1.0f,
                              -pitchHalfW, pitchHalfW),
                        microFocus.coords[1] * 0.9f, 0)) *
                   possessionBias +
               (defensiveFocusPos) * (1.0f - possessionBias);
  float microFocusStrength =
      (offense_microFocusStrength * possessionBias +
       defense_microFocusStrength * (1.0f - possessionBias));

  // on own half in possession: little microfocus strength. on own half not in
  // possession: lots of microfocus. other half: the other way around.
  ...
  microFocusStrength =
      microFocusStrength * (0.2f + 0.8f * autoMicroFocusStrength);
  ...
  float autoMidfieldFocus =
      NormalizedClamp((ballX / pitchHalfW) * -team->GetSide(), -0.8f,
  ...
  float midfieldFocusStrength =
      (offense_midfieldFocusStrength * possessionBias +
  ...
  Vector3 desiredPos = AI_GetAdaptedFormationPosition(
      team->GetMatch(), player, backXBound, frontXBound, lowYBound, highYBound,
      xFocus, xFocusStrength, yFocus, yFocusStrength, microFocus,
      microFocusStrength, midfieldFocus, midfieldFocusStrength,
...
void TeamAIController::CalculateDynamicRoles() {
  ...
  for (unsigned int y = 0; y < playerNum; y++) {
    adaptedFormationPositions.push_back(
        GetAdaptedFormationPosition(players.at(y), false));
  }

  // first make a sorted list on all possible distances between players and
  // formation targets
  ...
  for (unsigned int i = playerNum; i < distances.size(); i += 5) {
    ...
    int matrix_size = hungarian_init(&p, m, playerNum, playerNum,
...
float TeamAIController::CalculateMarkingQuality(Player *player, Player *opp) {
  ...
  // todo?: a better method than a virtual line would be a virtual circle,
  // originating from the goalpos with playerpos - goalpos distance as radius.
  // not sure how to calculate 'u' then though
  ...
  // draw virtual line from 'left' to 'right' of player, perpendicular to the
  // goal. anything 'above' this line he can catch up with. well, not if it's
  // too much to the side of this line / closer to goal; there just isn't enough
  // time then to catch up.
  ...
  line.SetVertex(0, playerPosition + safetyVec +
                        toGoalNorm.GetRotated2D(-0.5 * pi) *
                            lineLength); // left of player (seen from goal)
  line.SetVertex(1, playerPosition + safetyVec +
                        toGoalNorm.GetRotated2D(0.5 * pi) *
                            lineLength); // right of player (seen from goal)

  bool oppIsOnRightSideOfLine =
      line.WhatSide(oppPosition); // notice the descriptive variable name
  ...
  if (oppIsOnRightSideOfLine)
    adaptedOppFromLineDistance =
        fabs(oppFromLineDistance -
             2.0f); // we put the 'best spot' a bit further away from the line

  volatile float oppFromLineDistanceFactor =
      pow(NormalizedClamp(adaptedOppFromLineDistance, 0.0f, 60.0f), 0.5f);
  float oppOnLineDistanceFactor =
      pow(clamp(fabs(u * 2.0f - 1.0f), 0.0f, 1.0f), 0.5f);

  float result = 1.0f;

  // opponent further away from the line == bad (well.. not bad.. but not very
  // useful either. probably, another team mate is there, if not, can always
  // catch up when opp gets closer)
  ...
  // now add a bit of good old fashioned distancerating so that there'll still
  // be some definition if result is now 0.0f
  float oppDistance =
      1.0 - NormalizedClamp((playerPosition - oppPosition).GetLength(), 0.0,
                            pitchHalfW * 2.0f);
  result = result * 0.8f + oppDistance * 0.2f;

  /*
  if (player == match->GetDesignatedPossessionPlayer() &&
  opp->GetTimeNeededToGetToBall_ms() < 2000) { printf("u: %f\n", u);
    SetRedDebugPilon(line.GetVertex(0));
    SetGreenDebugPilon(line.GetVertex(1));
    //if (line.WhatSide(opp->GetPosition()) == true)
  SetYellowDebugPilon(opp->GetPosition() + opp->GetMovement() * 0.1);
    //SetYellowDebugPilon(line.GetVertex(0) + (line.GetVertex(1) -
  line.GetVertex(0)) * u);
  }
  */
...
void TeamAIController::CalculateManMarking() {
  ...
  // (oppInfo is already sorted, most dangerous first. this is done in
  // this->process() which is called earlier from team->process())
...
void TeamAIController::ApplyOffsideTrap(Vector3 &position) const {
  ...
  // if (position.coords[0] * team->GetSide() > offsideTrapX * team->GetSide())
  // position.coords[0] = offsideTrapX;

  // smooth version

  // area, centered around offsideTrapX, that will be compressed.
  // so the area from [offsideTrapX - areaLength .. offsideTrapX + areaLength]
  // will be compressed into [offsideTrapX .. offsideTrapX + areaLength]
  ...
  if (absPosX > absOffsideTrapX - areaHalfLength) {
    ...
    posFactor = clamp(posFactor, 0.0f,
                      1.0f); // 0.0f == most forward, 1.0f == deepest players
    // posFactor = pow(posFactor, 0.5f); // don't impact players on the
    // non-offside side of the offsideline as much as the other way around

    float absResultPosX =
        areaFront +
        areaHalfLength * posFactor; // this compresses the 2 * areaHalfLength
                                    // into 1 * areaHalfLength
...
void TeamAIController::PrepareSetPiece(e_SetPiece setPiece, int takerTeamID) {
  ...
  case e_SetPiece_KickOff:
    for (unsigned int i = 0; i < players.size(); i++) {
      Vector3 basePos = players.at(i)->GetFormationEntry().position *
                        Vector3(-team->GetSide() * pitchHalfW * 0.6,
                                -team->GetSide() * pitchHalfH * 0.6, 0);
      basePos.coords[1] += random(
          -2.0f, 2.0f); // to stop people from bumping into each other and such
      basePos.coords[0] *= 0.5;
      basePos.coords[0] += (pitchHalfW * 0.2) * team->GetSide();
      if (basePos.coords[0] * team->GetSide() < 0.5)
        basePos.coords[0] =
            0.5 * team->GetSide();     // not allowed to stand on opp side
      if (basePos.GetLength() < 9.4) { // not allowed to stand in center spot
        basePos.Normalize(Vector3(team->GetSide(), 0, 0));
        basePos *= 9.4;
      }
      players.at(i)->ResetPosition(basePos,
                                   match->GetBall()->Predict(0).Get2D());
  ...
  case e_SetPiece_GoalKick:
    for (unsigned int i = 0; i < players.size(); i++) {
      ...
      Vector3 basePos = AI_GetAdaptedFormationPosition(
          match, players.at(i), backXBound, frontXBound, lowYBound, highYBound,
  ...
  case e_SetPiece_Corner:
    for (unsigned int i = 0; i < players.size(); i++) {
      float backXBound, frontXBound, lowYBound, highYBound, xFocus,
          xFocusStrength, yFocus, yFocusStrength, midfieldFocus,
      ...
      Vector3 basePos = AI_GetAdaptedFormationPosition(
          ...
          Vector3(ballPos.coords[0] * 0.95f, ballPos.coords[1] * 0.1f, 0), 0.9,
          midfieldFocus, midfieldFocusStrength, false);
      players.at(i)->ResetPosition(basePos,
                                   match->GetBall()->Predict(0).Get2D());
    }
    break;
  ...
  case e_SetPiece_ThrowIn:
    for (unsigned int i = 0; i < players.size(); i++) {
      ...
      Vector3 ballPos = match->GetBall()->Predict(0).Get2D();
      if (isTakerTeam) {
        backXBound = clamp(ballPos.coords[0] + 30 * team->GetSide(),
                           -pitchHalfW, pitchHalfW);
        frontXBound = clamp(ballPos.coords[0] + 20 * -team->GetSide(),
                            -pitchHalfW, pitchHalfW);
        xFocus = clamp(ballPos.coords[0] + 4 * -team->GetSide(), -pitchHalfW,
                       pitchHalfW);
        xFocusStrength = 0.4;
        ...
        backXBound = clamp(ballPos.coords[0] + 30 * team->GetSide(),
                           -pitchHalfW, pitchHalfW);
        frontXBound = clamp(ballPos.coords[0] + 15 * -team->GetSide(),
                            -pitchHalfW, pitchHalfW);
        xFocus = clamp(ballPos.coords[0] + 16 * team->GetSide(), -pitchHalfW,
                       pitchHalfW);
        xFocusStrength = 0.2;
      ...
      Vector3 basePos = AI_GetAdaptedFormationPosition(
          match, players.at(i), backXBound, frontXBound, lowYBound, highYBound,
          xFocus, xFocusStrength, yFocus, yFocusStrength, ballPos, 0.7, 0, 0,
          false);
      players.at(i)->ResetPosition(basePos,
                                   match->GetBall()->Predict(0).Get2D());
    }
  ...
  case e_SetPiece_FreeKick:
    for (unsigned int i = 0; i < players.size(); i++) {
      ...
      if (isTakerTeam) {
        float xOffset =
            clamp((ballPos.coords[0] * -team->GetSide()) / pitchHalfW, -1.0,
                  1.0) *
                0.5 +
            0.5; // 0 == close to our goal, 1 == far from our goal
        backXBound = clamp(team->GetSide() * pitchHalfW * (0.7 - xOffset * 0.7),
                           -pitchHalfW, pitchHalfW);
        frontXBound =
            clamp(team->GetSide() * pitchHalfW * (-0.3 - xOffset * 0.7),
                  -pitchHalfW, pitchHalfW);
        // printf("fb: %f %f\n", backXBound, frontXBound);
        xFocus = clamp(ballPos.coords[0] + 10 * -team->GetSide(), -pitchHalfW,
                       pitchHalfW);
        ...
        float xOffset =
            ...
            0.5; // 0 == close to our goal, 1 == far from our goal
        // xOffset *= 40.0;
        backXBound = clamp(team->GetSide() * pitchHalfW * (1.0 - xOffset * 0.6),
                           -pitchHalfW, pitchHalfW);
        frontXBound =
            clamp(team->GetSide() * pitchHalfW * (0.5 - xOffset * 0.8),
                  -pitchHalfW, pitchHalfW);
        xFocus = clamp(ballPos.coords[0] + 20 * team->GetSide(), -pitchHalfW,
                       pitchHalfW);
      ...
      Vector3 basePos = AI_GetAdaptedFormationPosition(
          match, players.at(i), backXBound, frontXBound, lowYBound, highYBound,
          xFocus, xFocusStrength, yFocus, yFocusStrength, ballPos, 0.4, 0, 0,
          false);

      // keep distance
      if (!isTakerTeam) {
        if ((basePos - match->GetBall()->Predict(0).Get2D()).GetLength() <
            9.15) {
          basePos =
              match->GetBall()->Predict(0).Get2D() +
              (basePos - match->GetBall()->Predict(0).Get2D()).GetNormalized() *
                  9.15;
        }
      }
    ...
    if (!isTakerTeam && (match->GetBall()->Predict(0).Get2D() -
                         Vector3(team->GetSide() * pitchHalfW, 0, 0))
                                .GetLength() < 40.0) {
      std::vector<Player *> result;
      AI_GetClosestPlayers(team, match->GetBall()->Predict(0).Get2D(), false,
                           result, 3);
      for (unsigned int i = 0; i < result.size(); i++) {
        Vector3 toGoal = (Vector3(team->GetSide() * pitchHalfW, 0, 0) -
                          match->GetBall()->Predict(0).Get2D())
                             .GetNormalized(0);
        toGoal += Vector3(0, 1.0 - i, 0) * 0.07;
        toGoal.Normalize();
        result.at(i)->ResetPosition(match->GetBall()->Predict(0).Get2D() +
                                        toGoal * 9.15f,
                                    match->GetBall()->Predict(0).Get2D());
      }
    }
  ...
  case e_SetPiece_Penalty:
    for (unsigned int i = 0; i < players.size(); i++) {
      ...
      Vector3 ballPos = match->GetBall()->Predict(0).Get2D();
      if (isTakerTeam) {
        backXBound = clamp(ballPos.coords[0] + 50 * team->GetSide(),
                           -pitchHalfW, pitchHalfW);
        frontXBound = clamp(ballPos.coords[0] + 10 * -team->GetSide(),
                            -pitchHalfW, pitchHalfW);
        ...
        backXBound = clamp(ballPos.coords[0] + 11 * team->GetSide(),
                           -pitchHalfW, pitchHalfW);
        frontXBound = clamp(ballPos.coords[0] + 20 * -team->GetSide(),
                            -pitchHalfW, pitchHalfW);
        xFocus = ballPos.coords[0];
      ...
      Vector3 basePos = AI_GetAdaptedFormationPosition(
          match, players.at(i), backXBound, frontXBound, lowYBound, highYBound,
          xFocus, xFocusStrength, yFocus, yFocusStrength, 0, 0, 0, 0, false);
  ...
  default:
    for (unsigned int i = 0; i < players.size(); i++) {
      Vector3 basePos = players.at(i)->GetFormationEntry().position *
                        Vector3(-team->GetSide() * pitchHalfW * 0.7,
                                -team->GetSide() * pitchHalfH * 0.7, 0);
  ...
  if (isTakerTeam) {
    taker =
        AI_GetClosestPlayer(team, match->GetBall()->Predict(0).Get2D(), false);

    if (setPiece == e_SetPiece_ThrowIn || setPiece == e_SetPiece_KickOff) {
      taker->ResetPosition(
          match->GetBall()->Predict(0).Get2D() +
              match->GetBall()->Predict(0).Get2D().GetNormalized(
                  Vector3(0, -team->GetSide(), 0)) *
                  0.3f,
          match->GetBall()->Predict(0).Get2D());
    } else if (setPiece == e_SetPiece_FreeKick) {
      taker->ResetPosition(match->GetBall()->Predict(0).Get2D() +
                               Vector3(team->GetSide(), 0, 0) * 2.3f,
                           match->GetBall()->Predict(0).Get2D());
    } else {
      taker->ResetPosition(
          match->GetBall()->Predict(0).Get2D() +
  ...
  for (unsigned int i = 0; i < players.size(); i++) {
    if (players.at(i) != taker) {
      Vector3 toBall =
          players.at(i)->GetPosition() - match->GetBall()->Predict(0).Get2D();
      float ballDistance = (toBall).GetLength();
      if (ballDistance < minDistance)
        players.at(i)->ResetPosition(players.at(i)->GetPosition() +
                                         toBall.GetNormalized(0) * minDistance,
                                     match->GetBall()->Predict(0).Get2D());
    }
  }
}
...
void TeamAIController::ApplyAttackingRun(Player *manualPlayer) {
  ...
  attackingRunPlayer =
      manualPlayer ? manualPlayer : SelectAttackingRunPlayer(team);
}
...
void TeamAIController::ApplyTeamPressure() {
  ...
  Player *opp =
      match->GetTeam(abs(team->GetID() - 1))->GetBestPossessionPlayer();
  Vector3 opponentPos = opp->GetPosition() + opp->GetMovement() * 0.24f;

  teamPressurePlayer = AI_GetClosestPlayer(
      team, opponentPos + Vector3(team->GetSide() * 1.0f, 0, 0), true,
      team->GetGoalie());
...
void TeamAIController::CalculateSituation() {
  ...
  oppTeamHasPossession =
      match->GetTeam(abs(team->GetID() - 1))->HasPossession();
  oppTeamHasUniquePossession =
      match->GetTeam(abs(team->GetID() - 1))->HasUniquePossession();
...
void TeamAIController::UpdateTactics() {
  ...
  float timeFactor =
      0.5f + 0.5f * clamp(match->GetMatchTime_ms() / 6300000.0f, 0.0f, 1.0f);
  // printf("timefactor: %f\n", timeFactor);

  float offenseBias =
      clamp(0.5f + (goalFactor - 0.5f) * (timeFactor * 1.0f), 0.0f, 1.0f);
  // todo: add skills/opp skills and difficulty as factors
  // todo: add off/def slider/tactics
  // if (team->GetHumanGamerCount() == 0) offenseBias = offenseBias * 0.8f +
  // 0.2f; // let's assume human gamers want more offensive play
  //                                     offenseBias = offenseBias * 0.8f +
  //                                     0.2f; // let's assume AI gamers want
  //                                     more defensive play

  float possessionFactor =
      match->GetMatchData()->GetPossessionFactor_60seconds();
  float recentPossessionBias = 1.0f - fabs(possessionFactor - team->GetID());
  // if (team->GetID() == 0) printf("team 0 recentPossessionBias: %f
  // (possessionFactor: %f)\n", recentPossessionBias, possessionFactor); if
  // (team->GetID() == 1) printf("team 1 recentPossessionBias: %f
  ...
  // offensivenessBias); if (team->GetID() == 1) printf("T2 offensivenessBias:
  // %f\n", offensivenessBias);

  // autoBias = clamp(fabs(offenseBias - 0.5f) * 2.0f, 0.0f, 1.0f); // don't use
  // too much autobias when things are calm (= offenseBias being around 0.5)
  ...
  while (iter != userMods->end()) {
    float multiplier =
        teamTacticsModMultipliers.GetReal(iter->first.c_str(), 0.0f);

    float userOffset = atof(iter->second.c_str());
    float offset = userOffset;

    // feed back to teamdata (todo: hasn't this system been effectively
    // disabled? if so, delete it to avoid confusion)
    ...
    // printf("value: %s\n", iter->first.c_str());
    if (baseValue >=
        0.0f) { // not all user mods from teamdata are used in this class (for
                // example, individual settings like dribble stuff), ignore them
      liveTeamTactics.Set(iter->first.c_str(),
                          clamp(baseValue + offset, 0.0f, 1.0f));
    }
    iter++;
  }
}
