...
public:
  ...
  unsigned long GetPossessionTime_ms(int teamID) {
    return possessionTime_ms[teamID];
  }
  float GetPossessionFactor_60seconds() {
    ...
    // should be based on irl possession time instead of gametime? not sure yet,
    // think about this)
...
protected:
  ...
  float possession60seconds; // -600 to 600 for possession of team 1 / 2
                             // respectively
  int shots[2];
};
