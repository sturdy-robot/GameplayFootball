...
template <typename T> class Resource : public RefCounted {
...
public:
  ...
  Resource(const Resource &src, const std::string &identString)
      : identString(identString) {
    // src.resourceMutex.lock(); terribly slow, why? (note: moved to
    // resourcemanager's fetchcopy)
