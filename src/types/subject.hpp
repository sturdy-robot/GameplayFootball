...
// this class is not reentrant, derived classes and their users take this
// responsibility (by using Subject::subjectMutex)
template <class T = Observer> class Subject {
...
public:
  ...
  virtual void Detach(boost::intrusive_ptr<T> observer) {
    typename std::vector<boost::intrusive_ptr<T>>::iterator o_iter =
        observers.begin();
