...
template <typename T = boost::intrusive_ptr<Command>> class MessageQueue {
...
public:
  ...
  inline void NotifyWaiting(
      e_NotificationSubject notificationSubject = e_NotificationSubject_All) {
    boost::mutex::scoped_lock lock(queue.mutex);
  ...
  inline T WaitForMessage(bool &isMessage, int timeout_ms = -1) {
    ...
    boost::mutex::scoped_lock lock(queue.mutex);
    while (!isMessage) {
      // cannot use queue.WaitForNotification, because its scoped lock unlocks
      // on return
      if (queue.data.size() == 0) {
