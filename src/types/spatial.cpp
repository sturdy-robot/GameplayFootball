...
bool Spatial::GetLocalMode() { return localMode; }
...
Spatial *Spatial::GetParent() const { return parent; }
  ...
  if (_dirty_DerivedPosition) {
    if (localMode == e_LocalMode_Relative) {
      if (parent) {
        ...
        _cache_DerivedPosition.Set(parentDerivedRotation *
                                   (parentDerivedScale * GetPosition()));
...
Quaternion Spatial::GetDerivedRotation() const {
  ...
  if (_dirty_DerivedRotation) {
    if (localMode == e_LocalMode_Relative) {
      if (parent) {
        _cache_DerivedRotation =
            (parent->GetDerivedRotation() * GetRotation()).GetNormalized();
      } else {
