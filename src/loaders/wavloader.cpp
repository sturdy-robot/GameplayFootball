...
void WAVLoader::Load(std::string filename,
  ...
  if (!f) {
    Log(e_FatalError, "WAVLoader", "Load",
        "Could not load " + filename + ": file not found");
  }
  ...
  if (buf[0] != 1 || buf[1] != 0) {
    fclose(f);
    Log(e_FatalError, "WAVLoader", "Load",
        "Could not load " + filename + ": not PCM");
  }
  ...
  if (buf[0] != 'd' || buf[1] != 'a' || buf[2] != 't' || buf[3] != 'a') {
    fclose(f);
    Log(e_FatalError, "WAVLoader", "Load",
        "Could not load " + filename + ": data chunk not found");
  }
  ...
  // sizeof(unsigned char), channels, bits, frequency);
