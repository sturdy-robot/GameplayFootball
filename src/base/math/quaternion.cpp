...
const Quaternion Quaternion::operator*(float scale) const {
  return Quaternion(elements[0] * scale, elements[1] * scale,
                    elements[2] * scale, elements[3] * scale);
}
...
void Quaternion::operator=(const Vector3 &vec) {
  ...
  Matrix3 mat(x.coords[0], x.coords[1], x.coords[2], y.coords[0], y.coords[1],
              y.coords[2], z.coords[0], z.coords[1], z.coords[2]);
...
Quaternion Quaternion::operator*(const Quaternion &fac) const {
  ...
  result.elements[0] =
      elements[3] * fac.elements[0] + elements[0] * fac.elements[3] +
      elements[1] * fac.elements[2] - elements[2] * fac.elements[1];
  result.elements[1] =
      elements[3] * fac.elements[1] + elements[1] * fac.elements[3] +
      elements[2] * fac.elements[0] - elements[0] * fac.elements[2];
  result.elements[2] =
      elements[3] * fac.elements[2] + elements[2] * fac.elements[3] +
      elements[0] * fac.elements[1] - elements[1] * fac.elements[0];
  result.elements[3] =
      elements[3] * fac.elements[3] - elements[0] * fac.elements[0] -
...
Quaternion Quaternion::GetInverse() const {
  ...
  if (fnorm < 0.000001f) {
    // Log(e_Warning, "Quaternion", "GetInverse", "Unable to normalize
    // quaternion");
    return QUATERNION_IDENTITY;
  } else {
    real finvnorm = 1.0 / fnorm;
    return Quaternion(-elements[0] * finvnorm, -elements[1] * finvnorm,
                      -elements[2] * finvnorm, elements[3] * finvnorm);
  }
}
...
void Quaternion::GetAngles(radian &X, radian &Y, radian &Z) const {
  ...
  if (singularityTest > 0.49999 ||
      singularityTest < -0.49999) { // north and south pole
...
real Quaternion::GetMagnitude() const {
  if (elements[0] == 0.0f && elements[1] == 0.0f && elements[2] == 0.0f &&
      elements[3] == 0.0f)
    return 0.0f;
  real magnitude =
...
void Quaternion::Normalize() {
  ...
  double qmagsq = elements[0] * elements[0] + elements[1] * elements[1] +
                  elements[2] * elements[2] +
                  elements[3] * elements[3]; // squared magnitude
  if (qmagsq < 0.000001f) {
...
float Quaternion::GetDotProduct(const Quaternion &subject) const {
  return (
      elements[0] * subject.elements[0] + elements[1] * subject.elements[1] +
...
Quaternion Quaternion::GetSlerped(float bias, const Quaternion &to) const {
  ...
  double cosHalfTheta =
      elements[3] * qb.elements[3] + elements[0] * qb.elements[0] +
