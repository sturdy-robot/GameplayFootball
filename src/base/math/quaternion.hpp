...
public:
  ...
  Quaternion GetLerped(float bias, const Quaternion &to)
      const; // bias > 1 can extrapolate small angles in a hacky, incorrect way
             // *edit: is that so? i'm not so sure
  ...
  float MakeSameNeighborhood(
      const Quaternion &src); // returns dot product as added bonus! ;)
