...
public:
  ...
  } // gcc bug? this sometimes only sets the last element: memcpy(coords,
    // src.coords, 3 * sizeof(real)); }
  Vector3(real xyz);
  ...
  // this function does not make much sense does it? void NormalizeMax(float
  // length, const Vector3 &ifNull);
  ...
  // this function does not make much sense does it? Vector3
  // GetNormalizedMax(float length, const Vector3 &ifNull) const;
  ...
  Vector3 EnforceMaximumDeviation(const Vector3 &deviant,
                                  float maxDeviation) const;
  Vector3 GetClamped2D(const Vector3 &v1, const Vector3 &v2) const;

  // extrapolates a vector by a direction & a time difference in the future
  // (positive) or in the past (negative) assume direction vector to be units
  // per sec, assume time to be in milliseconds
...
inline void Vector3::operator=(const real src) { Set(src); }
...
inline Vector3 Vector3::operator+(const Vector3 &vec) const {
  return Vector3(coords[0] + vec.coords[0], coords[1] + vec.coords[1],
