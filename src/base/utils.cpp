...
std::string file_to_string(std::string filename) {
  ...
  if (file.fail())
    Log(e_FatalError, "utils", "file_to_vector",
...
void file_to_vector(std::string filename,
  ...
  while (file.getline(line, 32767)) {
    ...
    // remove possible windows CR
    line_str.erase(std::remove(line_str.begin(), line_str.end(), '\r'),
                   line_str.end());
...
int CopyDirectory(boost::filesystem::path const &source,
...
bool CopyFile(boost::filesystem::path const &source,
              boost::filesystem::path const &destinationDir) {
  boost::system::error_code error;
  namespace fs = boost::filesystem;
