...
void sdl_triangle_filled(SDL_Surface *surface, const Triangle &triangle,
                         Uint8 r, Uint8 g, Uint8 b) {
  // uses only the x, y part of the triangle

  // sge: inverse-wounded triangles won't be drawn, so draw twice, both ways
  // round. lame fix :D
  ...
  //                              int(triangle.GetVertex(2).coords[1]), r, g,
  //                              b);
  //
...
void sdl_alphablit(SDL_Surface *src, SDL_Rect *srcrect, SDL_Surface *dst,
                   SDL_Rect *dstrect) {
...
void sdl_setsurfacealpha(SDL_Surface *surface, int alpha) {
  ...
  for (int h = 0; h < height; h++) {
    ...
    for (int w = 0; w < width; w++) {
      ...
      // todo: endiannes!! *update: seems to work as is.. maybe alpha is always
      // in index 3 and only the colors are inverted?
