...
void AABB::Reset() {
  ...
  maxxyz.Set(-std::numeric_limits<real>::max(),
             -std::numeric_limits<real>::max(),
             -std::numeric_limits<real>::max());
  radius_needupdate = true;
  center_needupdate = true;
...
bool AABB::Intersects(const vector_Planes &planes) const {
  ...
  for (int p = 0; p < planes_size; p++) {
    // check if there is any point (that defines this aabb) on the inside of
    // this plane
    ...
    for (int ap = 0; ap < 8; ap++) {
      distance =
          determinant +
          planes.at(p).GetVertex(1).coords[0] * aabb_points[ap].coords[0] +
...
bool AABB::Intersects(const Line &src) const {
  ...
  Vector3 ad =
      d.GetAbsolute(); // returns same vector with all components positive
