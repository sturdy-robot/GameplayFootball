...
inline void Triangle::SetVertex(unsigned char pos, const real x, const real y,
                                const real z) {
  //    assert(pos < 3);
...
inline void Triangle::SetTextureVertex(unsigned char pos, const real x,
                                       const real y, const real z) {
  SetTextureVertex(0, pos, x, y, z);
}

inline void Triangle::SetTextureVertex(unsigned char texture_unit,
                                       unsigned char pos, const Vector3 &xyz) {
  SetTextureVertex(texture_unit, pos, xyz.coords[0], xyz.coords[1],
                   xyz.coords[2]);
}

inline void Triangle::SetTextureVertex(unsigned char texture_unit,
                                       unsigned char pos, const real x,
                                       const real y, const real z) {
  //    assert(pos < 3);
...
inline void Triangle::SetNormal(unsigned char pos, const real x, const real y,
                                const real z) {
  //    assert(pos < 3);
...
AABB Triangle::GetAABB() {
  ...
  aabb.SetMinXYZ(Vector3(vertices[0].coords[0], vertices[0].coords[1],
                         vertices[0].coords[2]));
  aabb.SetMaxXYZ(Vector3(vertices[0].coords[0], vertices[0].coords[1],
                         vertices[0].coords[2]));
...
void Triangle::IntersectsPlane(bool intersects[], const Vector3 &pvector,
  ...
  // pvector.coords[1], pvector.coords[2], pnormal.coords[0], pnormal.coords[1],
  // pnormal.coords[2]);
  ...
  if ((distance[0] == 0 && distance[1] == 0) ||
    ...
    // else, a triangle x(0, -100, 0) would detect intersection; 0 == pos, -100
    // == neg
    return;
  }
...
bool Triangle::IntersectsLine(const Line &u_ray, Vector3 &intersectVec) const {
  ...
  // todo: might be fixable by ensuring correctness of lumel positions in
  // lightmap generation
  ...
  intersectVec =
      u_ray.GetVertex(0) + dir * r; // intersect point of ray and plane
...
bool Triangle::IsCoplanar(const Vector3 &N, const Triangle &triangle) const {
  ...
  EDGE_AGAINST_TRI_EDGES(vertices[0], vertices[1], triangle.GetVertex(0),
                         triangle.GetVertex(1), triangle.GetVertex(2));
  EDGE_AGAINST_TRI_EDGES(vertices[1], vertices[2], triangle.GetVertex(0),
                         triangle.GetVertex(1), triangle.GetVertex(2));
  EDGE_AGAINST_TRI_EDGES(vertices[2], vertices[0], triangle.GetVertex(0),
                         triangle.GetVertex(1), triangle.GetVertex(2));

  /* finally, test if tri1 is totally contained in tri2 or vice versa */
  POINT_IN_TRI(vertices[0], triangle.GetVertex(0), triangle.GetVertex(1),
...
bool Triangle::IntersectsTriangle(const Triangle &triangle) const {
  ...
  COMPUTE_INTERVALS(vp0, vp1, vp2, dv0, dv1, dv2, dv0dv1, dv0dv2, a, b, c, x0,
                    x1);

  // compute interval for triangle 2
  COMPUTE_INTERVALS(up0, up1, up2, du0, du1, du2, du0du1, du0du2, d, e, f, y0,
                    y1);
...
void Triangle::CalculateTangents() {
  ...
  float cp = edge1uv.coords[1] * edge2uv.coords[0] -
             edge1uv.coords[0] * edge2uv.coords[1];

  if (cp != 0.0f) {
    float mul = 1.0f / cp;
    for (int v = 0; v < 3; v++) {
      tangents[v] =
          (edge1 * -edge2uv.coords[1] + edge2 * edge1uv.coords[1]) * mul;
      biTangents[v] =
          (edge1 * -edge2uv.coords[0] + edge2 * edge1uv.coords[0]) * mul;
      ...
      // "To check whether it must be inverted or not, the check is simple : TBN
      // must form a right-handed coordinate system, i.e. cross(n,t) must have
      // the same orientation than b.
      ...
      //  > 0. If it’s false, just invert t : if (glm::dot(glm::cross(n, t), b)
      //  < 0.0f) {
      //    t = t * -1.0f;
      //  }"
      ...
      // printf("dot: %f\n", dotProduct);
      //  check for > 0 and change bitangents, since we want a left handed TBN
      //  (todo: is that so?)
      if (dotProduct > 0.0f) {
