...
inline e_ObjectType Object::GetObjectType() { return objectType; }

const Properties &Object::GetProperties() const { return properties; }
...
void Object::Synchronize() {
  ...
  for (int i = 0; i < observersSize; i++) {
    boost::intrusive_ptr<Interpreter> interpreter =
        static_pointer_cast<Interpreter>(observers.at(i));
...
inline void
Object::RecursiveUpdateSpatialData(e_SpatialDataType spatialDataType,
                                   e_SystemType excludeSystem) {}

boost::intrusive_ptr<Interpreter>
Object::GetInterpreter(e_SystemType targetSystemType) {
  subjectMutex.lock();
...
void Object::LockSubject() { subjectMutex.lock(); }

void Object::UnlockSubject() { subjectMutex.unlock(); }
