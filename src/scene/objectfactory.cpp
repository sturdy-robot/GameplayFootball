...
void ObjectFactory::Exit() {
  std::map<e_ObjectType, boost::intrusive_ptr<Object>>::iterator
      prototype_iter = prototypes.begin();
...
ObjectFactory::CreateObject(const std::string &name, std::string objectTypeStr,
                            std::string postfix) {
...
boost::intrusive_ptr<Object>
ObjectFactory::CreateObject(const std::string &name, e_ObjectType objectType,
                            std::string postfix) {
  std::map<e_ObjectType, boost::intrusive_ptr<Object>>::iterator
      prototype_iter = prototypes.find(objectType);
  if (prototype_iter != prototypes.end()) {
    boost::intrusive_ptr<Object> bla =
        CopyObject((*prototype_iter).second, postfix);
    bla->SetName(name);
    return bla;
  } else {
    Log(e_FatalError, "ObjectFactory", "CreateObject",
        "Object type " + int_to_str(objectType) +
            " not found in prototype registry");
    return 0;
  }
}

boost::intrusive_ptr<Object>
ObjectFactory::CopyObject(boost::intrusive_ptr<Object> source,
                          std::string postfix) {
  boost::intrusive_ptr<Object> bla;
...
void ObjectFactory::RegisterPrototype(e_ObjectType objectType,
  ...
  prototypes.insert(std::pair<e_ObjectType, boost::intrusive_ptr<Object>>(
