...
Scene::Scene(std::string name, e_SceneType sceneType)
    : name(name), sceneType(sceneType) {}

Scene::~Scene() {}

void Scene::Exit() { DetachAll(); }
...
void Scene::CreateSystemObjects(boost::intrusive_ptr<Object> object) {
  ...
  for (int i = 0; i < observersSize; i++) {
    ISceneInterpreter *sceneInterpreter =
        static_cast<ISceneInterpreter *>(observers.at(i).get());
...
e_SceneType Scene::GetSceneType() const { return sceneType; }
