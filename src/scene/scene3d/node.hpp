...
public:
  ...
  Node(const Node &source, const std::string &postfix,
  ...
  void GetSpatials(std::list<boost::intrusive_ptr<Spatial>> &gatherSpatials,
                   bool recurse = true, int depth = 0) const;

  void GetObjects(std::list<boost::intrusive_ptr<Object>> &gatherObjects,
                  bool recurse = true, int depth = 0) const;
  void GetObjects(std::list<boost::intrusive_ptr<Object>> &gatherObjects,
                  const vector_Planes &bounding, bool recurse = true,
                  int depth = 0) const;
  ...
  inline void GetObjects(e_ObjectType targetObjectType,
    ...
    if (recurse) {
      ...
      for (int i = 0; i < nodesSize; i++) {
        nodes.data.at(i)->GetObjects<T>(targetObjectType, gatherObjects,
                                        recurse, depth + 1);
      }
      nodes.Unlock();
    }
