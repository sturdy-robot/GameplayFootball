...
Node::Node(const Node &source, const std::string &postfix,
  ...
  for (int i = 0; i < (signed int)gatherNodes.size(); i++) {
    boost::intrusive_ptr<Node> copy(
        new Node(*gatherNodes.at(i).get(), postfix, scene3D));
    AddNode(copy);
  }
...
void Node::GetNodes(std::vector<boost::intrusive_ptr<Node>> &gatherNodes,
                    bool recurse) const {
  nodes.Lock();
...
void Node::RemoveObject(const std::string &name) { DeleteObject(name, false); }

void Node::RemoveObject(boost::intrusive_ptr<Object> object) {
  DeleteObject(object, false);
}

void Node::RemoveAllObjects() { DeleteAllObjects(false); }

void Node::GetSpatials(std::list<boost::intrusive_ptr<Spatial>> &gatherSpatials,
...
void Node::GetObjects(std::list<boost::intrusive_ptr<Object>> &gatherObjects,
                      const vector_Planes &bounding, bool recurse,
                      int depth) const {
  objects.Lock();
  int objectsSize = objects.data.size();
  for (int i = 0; i < objectsSize; i++) {
  ...
  if (recurse) {
    ...
    for (int i = 0; i < nodesSize; i++) {
      if (nodes.data.at(i)->GetAABB().Intersects(bounding))
        nodes.data.at(i)->GetObjects(gatherObjects, bounding, recurse,
                                     depth + 1);
    }
    nodes.Unlock();
  }
...
void Node::RecursiveUpdateSpatialData(e_SpatialDataType spatialDataType,
  ...
  int nodesSize = nodes.data.size();
  for (int i = 0; i < nodesSize; i++) {
    // todo: not for localmode == absolute? doesn't seem to work, but why not?!
    // commented out for now (maybe because it won't validate
    // spatialdata/bounding volume of parents of these?)
    // if (nodes.data.at(i)->GetLocalMode() != e_LocalMode_Absolute)
    nodes.data.at(i)->RecursiveUpdateSpatialData(spatialDataType,
                                                 excludeSystem);
  }
  nodes.Unlock();
  ...
  for (int i = 0; i < objectsSize; i++) {
    ...
    objects.data.at(i)->RecursiveUpdateSpatialData(spatialDataType,
                                                   excludeSystem);
  }
  objects.Unlock();
}
