...
// ATOMICITY: this class is responsible for doing about everything concurrently
// without crashing. this implicitly accounts for atomicity in observers.
...
public:
  ...
  virtual void
  RecursiveUpdateSpatialData(e_SpatialDataType spatialDataType,
                             e_SystemType excludeSystem = e_SystemType_None);

  Lockable<MustUpdateSpatialData> updateSpatialDataAfterPoke;

  virtual boost::intrusive_ptr<Interpreter>
  GetInterpreter(e_SystemType targetSystemType);
  ...
  // todo: create 2 types of properties: one for pre-creating system objects
  // (for example; what system objects should be created?) and one for Init()
  // stuff
