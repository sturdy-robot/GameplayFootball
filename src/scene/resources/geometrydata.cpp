...
bool SortTriangleMeshes(const MaterializedTriangleMesh &tm1,
                        const MaterializedTriangleMesh &tm2) {
...
void GeometryData::SetTriangleMesh(Material material, float *vertices,
...
AABB GeometryData::GetAABB() const {
  if (aabb.dirty) {
    ...
    for (int i = 0; i < (signed int)triangleMeshes.size(); i++) {
      aabb.aabb += GetTriangleMeshAABB(triangleMeshes.at(i).vertices,
                                       triangleMeshes.at(i).verticesDataSize,
