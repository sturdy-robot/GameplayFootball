...
void Geometry::Poke(e_SystemType targetSystemType) {
  ...
  // todo: does this work when multiple systems want to exclude themselves? i
  // guess not.. make it (excludes) an array?
...
AABB Geometry::GetAABB() const {
  ...
  if (aabb.data.dirty == true) {
    ...
    aabb.data.aabb =
        geometryData->GetResource()->GetAABB() * GetDerivedRotation() +
        GetDerivedPosition();
...
void Geometry::ApplyForceAtRelativePosition(float force,
  ...
  for (int i = 0; i < observersSize; i++) {
    IGeometryInterpreter *geometryInterpreter =
        static_cast<IGeometryInterpreter *>(observers.at(i).get());
    geometryInterpreter->ApplyForceAtRelativePosition(force, direction,
                                                      position);
  }
