...
boost::intrusive_ptr<Resource<Surface>> Image2D::GetImage() {
  // todo: this mutex lock makes little sense or so it seems. will have to force
  // users to lock from outside this function
...
void Image2D::SetPosition(const Vector3 &newPosition, bool updateSpatialData) {
  SetPosition(int(floor(newPosition.coords[0])),
              int(floor(newPosition.coords[1])));
}
...
void Image2D::PutPixel(int x, int y, const Vector3 &color, int alpha) {
  ...
  if (SDL_ISPIXELFORMAT_ALPHA(surface->format->format))
    color32 = SDL_MapRGBA(surface->format, int(floor(color.coords[0])),
                          int(floor(color.coords[1])),
                          int(floor(color.coords[2])), alpha);
  else
    color32 =
        SDL_MapRGB(surface->format, int(floor(color.coords[0])),
    ...
    for (int y = 0; y < surface->h; y++) {
      ...
      for (int xsub = -radius; xsub < radius + 1; xsub++) {
        for (int ysub = -radius; ysub < radius + 1; ysub++) {
          ...
          if (x + xsub >= 0 && x + xsub < surface->w && y + ysub >= 0 &&
              y + ysub < surface->h) {
...
void Image2D::DrawTriangle(const Triangle &triangle, const Vector3 &color,
                           int alpha) {
  subjectMutex.lock();
  image->resourceMutex.lock();
  ...
  sdl_triangle_filled(surface, triangle, int(floor(color.coords[0])),
                      int(floor(color.coords[1])), int(floor(color.coords[2])));
...
void Image2D::DrawSimpleText(
    const std::string &caption, int x, int y, TTF_Font *font,
    const Vector3 &color, int alpha) { // todo: alpha doesn't work properly yet

  SDL_Color sdlColor = {(unsigned char)(color.coords[0]),
                        (unsigned char)(color.coords[1]),
                        (unsigned char)(color.coords[2]), 0};
  SDL_Surface *sdlText =
      TTF_RenderUTF8_Blended(font, caption.c_str(), sdlColor);
  if (alpha != 255)
