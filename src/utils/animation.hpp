...
//
// rules:
//   - first node inserted (by using SetKeyFrame) should be root node of the
//   animated object (for optimization purposes)
...
public:
  Animation();
  Animation(
      const Animation &src); // attention! this does not deep copy extensions!
  virtual ~Animation();
  ...
  bool GetKeyFrame(std::string nodeName, int frame, Quaternion &orientation,
  ...
  void Invert();
  void
  Apply(const std::map<const std::string, boost::intrusive_ptr<Node>> nodeMap,
        int frame, int timeOffset_ms = 0, bool smooth = true,
        float smoothFactor = 1.0f, /*const boost::shared_ptr<Animation>
                                      previousAnimation, int smoothFrames, */
                                   const Vector3 &basePos = Vector3(0),
        radian baseRot = 0,
