...
Animation::Animation(const Animation &src) {
  ...
  std::map<const char *, std::string>::const_iterator varCacheIter =
      src.variableCache.begin();
  while (varCacheIter != src.variableCache.end()) {
    char *varName = new char[256];
    memcpy(varName, varCacheIter->first, 256 * sizeof(char));
    variableCache.insert(
        std::pair<const char *, std::string>(varName, varCacheIter->second));
    varCacheIter++;
  }
...
bool Animation::GetKeyFrame(std::string nodeName, int frame,
  ...
  for (int i = 0; i < animSize; i++) {
    if (nodeAnimations.at(i)->nodeName == nodeName) {
      GetInterpolatedValues(nodeAnimations.at(i)->animation, frame, orientation,
                            position, getOrientation, getPosition);
      return nodeAnimations.at(i)->animation.find(frame) !=
             nodeAnimations.at(i)->animation.end();
    }
  }
  return false;
}

void Animation::SetKeyFrame(std::string nodeName, int frame,
  ...
  // find frame
  const std::map<int, KeyFrame>::iterator animIter =
      nodeAnimation->animation.find(frame);
...
void Animation::GetInterpolatedValues(const std::map<int, KeyFrame> &animation,
                                      int frame, Quaternion &orientation,
                                      Vector3 &position, bool getOrientation,
  ...
  if (frame > 0 && frame < GetFrameCount()) {
    ...
    while (animIter != animation.end()) {
      ...
      // still before current frame and yet encountered keys? clear them, we
      // don't need earlier keys
      if (animIter->first < frame && weighedKeys.size() > 0)
        weighedKeys.clear();

      // add key, hopefully this is the last one before our current frame, or
      // the first after
      WeighedKey key;
      ...
      // if this keyframe came after our current frame, we've got everything we
      // need, so bail out
    ...
    // we've now got either 1 keyframe (before/after/at our current frame) or 2,
    // on both sides of (or 1 at) our keyframe. calculate its/their influence
    float bias = 0;
    ...
    if (getPosition) {
      for (unsigned int v = 0; v < weighedKeys.size(); v++) {
        position +=
            weighedKeys.at(v).keyFrame.position * weighedKeys.at(v).influence;
      }
    }
    ...
    // extrapolation through hax: bias > 1 doesn't really give proper results.
    // however, it works for small angles so FUFUUFUUUU XD (use slerp instead of
    // lerp - lerp can't extrapolate at all)
...
void Animation::ConvertToStartFacingForwardIfIdle() {
  ...
  std::map<std::string, boost::shared_ptr<AnimationExtension>>::iterator
  ...
  Vector3 newBallDirVec =
      GetVariable("balldirection") != ""
          ? GetVectorFromString(GetVariable("balldirection"))
  ...
  Vector3 newBumpDirVec =
      GetVariable("bumpdirection") != ""
          ? GetVectorFromString(GetVariable("bumpdirection"))
                .GetRotated2D(-incomingBodyAngle)
          : 0;
  SetVariable("balldirection", GetStringFromVector(newBallDirVec));
  SetVariable("incomingballdirection",
              GetStringFromVector(newIncomingBallDirVec));
...
void Animation::Invert() {
  ...
  for (int i = 0; i < animSize; i++) {
    ...
    std::map<int, KeyFrame>::iterator keyIter =
        nodeAnimation->animation.begin();
...
void Animation::Apply(
  ...
  for (unsigned int i = 0; i < animSize; i++) {
    ...
    // old: GetInterpolatedValues(nodeAnimation->animation, frame, orientation,
    // position);
    ...
    if (smooth && 1 == 2) {
      ...
      bias /= factor;
      bias += 0.5f - ((1.0f / factor) *
                      0.5f); // center, then subtract half of new size
    }
    GetInterpolatedValues(nodeAnimation->animation, frame - smoothFrames,
                          orientation_pre, position_pre);
    GetInterpolatedValues(nodeAnimation->animation, frame + 1 + smoothFrames,
                          orientation_post, position_post);
    orientation_pre.MakeSameNeighborhood(orientation_post);
    orientation =
        orientation_pre.GetLerped(bias, orientation_post).GetNormalized();
    ...
    // offset
    std::map<std::string, BiasedOffset>::iterator iter =
        offsets.find(nodeAnimation->nodeName);
    if (iter != offsets.end()) {
      iter->second.orientation.MakeSameNeighborhood(orientation);
      if (iter->second.isRelative) {
        orientation = orientation
                          .GetLerped(iter->second.bias,
                                     iter->second.orientation * orientation)
                          .GetNormalized();
      } else {
        orientation =
            orientation.GetLerped(iter->second.bias, iter->second.orientation)
                .GetNormalized();
      }
    }
    ...
    if (smooth) {
      ...
      // needed for smoothing - keep track of old limb positions/movements so we
      // can extrapolate those and use that for rotation change limit
      // calculations
      ...
      for (unsigned int node = 0; node < movementHistory->size(); node++) {
        if (movementHistory->at(node).nodeName.compare(
                nodeAnimation->nodeName) == 0) {
      ...
      float beginBias =
          pow(curve(1.0f - NormalizedClamp(frame, 0, 8), 1.0f), 0.5f);
      float currentBias = 0.0f + beginBias * smoothFactor * 0.5f;

      if (nodeAnimation->nodeName.compare("player") != 0) {

        const Quaternion &previousOrientation =
            movementHistoryEntry->orientation;
        ...
        if (timeDiff_ms > 0) {
          ...
          bool simpleMethod = true; // non-simple-method bug: initial
                                    // orientation seems to be off
          ...
          if (simpleMethod == false) {
            ...
            // * (1.0f / (movementHistoryEntry->timeDiff_ms * 0.001f));
            Quaternion currentRotation_per_ms =
                previousOrientation.GetRotationTo(currentOrientation)
                    .GetNormalized();
            currentRotation_per_ms =
                currentRotation_per_ms.GetRotationMultipliedBy(
                    1.0f / (float)movementHistoryEntry
                               ->timeDiff_ms); // already normalized in function
            ...
            orientation =
                orientation
                    .GetSlerped(
                        pow(1.0f - clamp(timeDiff_ms / 30.0f, 0.0f, 1.0f),
                            0.5f) *
                            (0.5f + smoothFactor * 0.5f * beginBias),
                        currentOrientation)
                    .GetNormalized();
            // orientation = orientation.GetSlerped(pow(1.0f - clamp(timeDiff_ms
            // / 30.0f, 0.0f, 1.0f), 0.5f), currentOrientation).GetNormalized();
            // orientation = orientation.GetSlerped(0.9f + 0.1f * beginBias,
            // currentOrientation).GetNormalized();

            // float currentToDesiredDot =
            // orientation.GetDotProduct(currentOrientation);
            float currentToDesiredDot =
            ...
            radian angleDiff_per_ms =
                2.0f * acos(clamp(currentToDesiredDot, -1.0f, 1.0f)) /
                (float)timeDiff_ms;
            radian maxDiff_per_ms = 5.0f * pi * 0.001f;
            // if (nodeAnimation->nodeName.compare("body") == 0) maxDiff_per_ms
            // = 3.0f * pi * 0.001f;
            if (angleDiff_per_ms > maxDiff_per_ms) {
              ...
              // if (desiredRotation.elements[3] < 0)
              // desiredRotation.elements[3] = -desiredRotation.elements[3];
              orientation =
                  desiredRotation.GetRotationMultipliedBy(allowFraction) *
                  currentOrientation;
              orientation.Normalize();
            }
            ...
            /*
               ...
               http://www.freebasic.net/forum/viewtopic.php?t=9769 ) Quaternion
               ...
               1000.0f; // basically: angle between desired and current rotation
               (per second) if (angle_sec > 0.0f) { radian dampAngle_sec = 0.15f
               * pi; float dampFactor = NormalizedClamp(dampAngle_sec, 0.0f,
               angle_sec);
            ...
            */
            /*
               ...
               currentOrientation; extrapolatedOrientation.Normalize();
                    ...
                    // now we want to go from current to new orientation, but we
               can only differ so much from the rotation from previous to
               ...
               pow(movementInfluence, 0.5f); // influence of old movement wears
               off in exponential fashion (source: laws of nature)
            ...
            */
            ...
            Quaternion desiredRotation_per_ms =
                currentOrientation.GetRotationTo(orientation).GetNormalized();
            desiredRotation_per_ms =
                desiredRotation_per_ms.GetRotationMultipliedBy(
                    1.0f / (float)timeDiff_ms);

            // now get the resulting rotation, which is the current rotation,
            // converged as much towards the desired rotation as allowed, based
            // upon maximum rotational angle change per time unit
            Quaternion resultingRotation_per_ms = desiredRotation_per_ms;
            float currentToDesiredRotationDot =
                desiredRotation_per_ms.MakeSameNeighborhood(
                    currentRotation_per_ms);
            radian angleDiff_per_ms_per_s =
                2.0f * acos(clamp(currentToDesiredRotationDot, -1.0f, 1.0f)) /
            ...
            // identity.MakeSameNeighborhood(currentRotation_per_ms))
            // maxDiff_per_ms_per_s = 0.4f * pi; if
            // (nodeAnimation->nodeName.compare("body") == 0)
            // maxDiff_per_ms_per_s = 0.1f * pi;
            if (angleDiff_per_ms_per_s > maxDiff_per_ms_per_s) {
              // now make desiredRotation_per_ms into limited rotation *
              // currentRotation_per_ms
              float allowFraction =
                  maxDiff_per_ms_per_s / angleDiff_per_ms_per_s;
              Quaternion desiredRotationRotation =
                  currentRotation_per_ms.GetRotationTo(desiredRotation_per_ms)
                      .GetNormalized();
              // if (desiredRotation.elements[3] < 0)
              // desiredRotation.elements[3] = -desiredRotation.elements[3];
              resultingRotation_per_ms =
                  desiredRotationRotation.GetRotationMultipliedBy(
                      allowFraction) *
                  currentRotation_per_ms;
            ...
            // desiredRotation_per_ms.GetLerped(movementInfluence,
            // currentRotation_per_ms);
            Quaternion resultingOrientation =
                resultingRotation_per_ms.GetRotationMultipliedBy(timeDiff_ms) *
                currentOrientation;

            orientation =
                orientation.GetSlerped(beginBias, resultingOrientation)
                    .GetNormalized();
            // orientation = resultingOrientation;

            // add some extra overall smoothness
            // orientation = orientation.GetSlerped(pow(1.0f - clamp(timeDiff_ms
            // / 30.0f, 0.0f, 1.0f), 0.5f) * (0.6f + 0.2f * beginBias),
            // currentOrientation).GetNormalized(); orientation =
            // (identity.GetRotationTo(orientation).GetNormalized().GetRotationMultipliedBy(0.1f)
            // * orientation).GetNormalized(); orientation =
            // orientation.GetSlerped(pow(1.0f - clamp(timeDiff_ms / 30.0f,
            // 0.0f, 1.0f), 0.5f) * (0.2f + 0.1f * beginBias),
            // currentOrientation).GetNormalized(); orientation =
            // orientation.GetSlerped(0.3f + beginBias * 0.2f,
            // currentOrientation).GetNormalized(); // incorrect! incorporate
            // timeDiff_ms into this somehow, whilst still allowing for less
            // influence than the above version
            ...
            orientation =
                orientation.GetSlerped(currentBias, currentOrientation)
                    .GetNormalized();

            // maximum rotational velocity
            float dot = orientation.MakeSameNeighborhood(currentOrientation);
            radian angle_per_second = 2.0f * acos(clamp(dot, -1.0f, 1.0f)) /
                                      ((float)timeDiff_ms * 0.001f);
            radian maxAngle_per_second = 7.5f * pi; // 5.5f * pi;
            // if (nodeAnimation->nodeName.compare("body") == 0)
            // maxAngle_per_second *= 0.8f;
            if (nodeAnimation->nodeName.compare("left_elbow") == 0)
              ...
              maxAngle_per_second *= 1.6f;
            maxAngle_per_second =
                (0.3f + 0.7f * (1.0f - beginBias)) * maxAngle_per_second;
            // if (nodeAnimation->nodeName.compare("body") == 0)
            // maxAngle_per_second = 3.0f * pi;
        ...
        /*
          ...
          float dot = clamp(orientation.GetDotProduct(oldOrientation),
          0.0f, 1.0f);
          //radian diff = pi - acos(change) * 2.0f;
          radian diff = acos(dot);
          radian maxRadPerSec = 3.8f * pi;
          if ((nodeAnimation->nodeName.compare("body") == 0 ||
          nodeAnimation->nodeName.compare("middle") == 0)) maxRadPerSec = 3.2f *
          pi; maxRadPerSec *= 1.0f - beginBias * smoothFactor; radian
          clampedDiff = clamp(diff, 0.0f, (timeDiff_ms * 0.001f) *
          maxRadPerSec); float slerpBias = 1.0f - NormalizedClamp(clampedDiff +
          0.00001f, 0.0f, diff + 0.00001f); // convert from clampedDiff to slerp
          bias. 0 == diff, 1 == 0
          //slerpBias = 0.94f;
          ...
          nodeAnimation->nodeName.compare("body") == 0) printf("%i ms, %f %f %f
          %f\n", timeDiff_ms, dot, diff, clampedDiff, slerpBias);

          // tmp super low tek hax
          //if (dot < 0.95f) orientation = orientation.GetSlerped(0.3f,
          oldOrientation);
        ...
        */
        ...
        Vector3 currentPosition =
            nodeMap.find(nodeAnimation->nodeName)->second->GetPosition();
        ...
        if (timeDiff_ms > 0 && beginBias > 0.01f) {
          ...
          /*
                    // damping (maybe useful:
             http://www.freebasic.net/forum/viewtopic.php?t=9769 ) Vector3
             desiredMovement = ((basePos + position) - currentPosition) /
             (timeDiff_ms * 0.001f); Vector3 dampingMovement = (desiredMovement
             - currentMovement); currentMovement +=
             dampingMovement.GetNormalizedMax(timeDiff_ms * 0.02f);

                    Vector3 extrapolatedPosition = currentPosition +
             currentMovement * (timeDiff_ms * 0.001f);

                    // now we want to go from current to new position, but we
             can only differ so much from the movement from previous to current
             position float movementInfluence = 1.0f - clamp(timeDiff_ms
             / 50.0f, 0.0f, 1.0f); movementInfluence = pow(movementInfluence,
             0.5f); // influence of old movement wears off in exponential
             fashion (source: laws of nature) float beginBias = pow(1.0f -
             NormalizedClamp(frame, 0, 15), 0.5f); movementInfluence *= 0.4f +
             beginBias * 0.6f;

                    //if (desiredMovement.GetLength() <
             currentMovement.GetLength()) movementInfluence = 0.0f;
                    //if (currentMovement.GetLength() >
             desiredMovement.GetLength()) currentMovement =
             currentMovement.GetNormalized() * desiredMovement.GetLength();
                    //if (currentMovement.GetLength() >
             desiredMovement.GetLength()) currentMovement =
             currentMovement.GetNormalized() * (currentMovement.GetLength() *
             movementInfluence + desiredMovement.GetLength() * (1.0f -
             movementInfluence));

                    Vector3 newPosition = (extrapolatedPosition) *
             movementInfluence + // current-movement-implied position (basePos +
             position) * (1.0f - movementInfluence); // desired position
          */

          // smooth
          position.coords[2] = position.coords[2] * (1.0f - currentBias) +
                               currentPosition.coords[2] * currentBias;
          ...
          float allowedDistance =
              maxMetersPerSec * ((float)timeDiff_ms * 0.001f);
          ...
          newZ = (newZ * bias + currentPosition.coords[2] * (1.0f - bias)) *
                     beginBias +
                 newZ * (1.0f - beginBias); // dual biasses ^_^ works like this:
                                            // currentPosition influence only
                                            // effective when beginbias > 0

          // /old version

          // nodeMap.find(nodeAnimation->nodeName)->second->SetPosition(newPosition,
          // false);
          //  only height
          position = position.Get2D() + Vector3(0, 0, newZ);

          // nodeMap.find(nodeAnimation->nodeName)->second->SetPosition((position
          // + basePos).Get2D() + Vector3(0, 0, newZ), false);
        }
    ...
    if (nodeAnimation->nodeName.compare("player") != 0) {
      Quaternion currentOrientation =
          nodeMap.find(nodeAnimation->nodeName)->second->GetRotation();
      orientation.MakeSameNeighborhood(currentOrientation);
      nodeMap.find(nodeAnimation->nodeName)
          ->second->SetRotation(orientation, false);
    } else if (nodeAnimation->nodeName.compare("player") == 0) {
      nodeMap.find(nodeAnimation->nodeName)
          ->second->SetPosition(position + basePos, false);
    }
  }
...
void Animation::Shift(int fromFrame,
  ...
  if (offset == 1) {
    ...
    for (int i = 0; i < animSize; i++) {
      NodeAnimation *nodeAnimation = nodeAnimations.at(i);
      std::map<int, KeyFrame>::iterator animIter =
          nodeAnimation->animation.begin();
...
Vector3 Animation::GetRangedOutgoingMovement() const {
  if (cache_rangedOutgoingMovement_dirty || cache_outgoingVelocity_dirty ||
      cache_angle_dirty) {

    if (nodeAnimations.at(0)->animation.size() > 1) {
      cache_rangedOutgoingMovement = Vector3(0, -GetOutgoingVelocity(), 0)
                                         .GetRotated2D(GetOutgoingAngle());
    } else {
...
Vector3 Animation::GetOutgoingDirection() const {
  if (cache_outgoingDirection_dirty || cache_angle_dirty) {
    cache_outgoingDirection =
        Vector3(0, -1, 0).GetRotated2D(GetOutgoingAngle());
...
Vector3 Animation::GetIncomingBodyDirection() const {
  if (cache_incomingBodyDirection_dirty || cache_incomingBodyAngle_dirty) {
    cache_incomingBodyDirection =
        Vector3(0, -1, 0).GetRotated2D(GetIncomingBodyAngle());
...
Vector3 Animation::GetOutgoingBodyDirection() const {
  if (cache_outgoingBodyDirection_dirty || cache_outgoingBodyAngle_dirty) {
    cache_outgoingBodyDirection =
        Vector3(0, -1, 0).GetRotated2D(GetOutgoingBodyAngle());
...
radian Animation::GetOutgoingAngle() const {
  if (cache_angle_dirty || cache_outgoingVelocity_dirty) {
    if (GetOutgoingVelocity() >= 1.8) {
      ...
      // if angle is close to 180 degrees, we can't be sure if we want 180 or
      // -180 deg. use body angle as hint.
      if (cache_angle < -0.95f * pi || cache_angle > 0.95f * pi) {
        radian x, y, z;
        (--(nodeAnimations.at(1)->animation.end()))
            ->second.orientation.GetAngles(x, y, z);
        if (signSide(cache_angle) != signSide(z)) {
          cache_angle = pi * 0.99f * signSide(z);
        } else {
          cache_angle = clamp(
              cache_angle, -0.99f * pi,
              0.99f * pi); // also do this if the side is already correct: we
                           // want to be a little away from pi, else vectors
                           // based on this don't have a clear sidedness
        }
      }
...
radian Animation::GetIncomingBodyAngle() const {
  if (cache_incomingBodyAngle_dirty) {
    if (nodeAnimations.at(1)->animation.size() > 0) {
      ...
      // body rotation
      radian x, y, z;
      (nodeAnimations.at(1)->animation.begin())
          ->second.orientation.GetAngles(x, y, z);
      ...
      cache_incomingBodyAngle =
          ModulateIntoRange(-pi, pi, cache_incomingBodyAngle);
      // printf("angle: %f\n", cache_incomingBodyAngle);
    ...
    if (GetOutgoingVelocity() >= 1.8) {
      if (nodeAnimations.at(1)->animation.size() > 0) {
        ...
        */
        ...
        cache_outgoingBodyAngle =
            ModulateIntoRange(-pi, pi, cache_outgoingBodyAngle);
...
void Animation::Reset() {
  ...
  std::map<const char *, std::string>::const_iterator iter =
      variableCache.begin();
...
void Animation::Load(const std::string &filename) {
  ...
  std::multimap<std::string, XMLTree>::iterator iter =
      customData->children.find("bumpdirection");
  ...
  while (iter != customData->children.end()) {
    ...
    variableCache.insert(
        std::pair<const char *, std::string>(varName, varData));
...
void Animation::Save(const std::string &filename) {
  ...
  for (int i = 0; i < (signed int)nodeAnimations.size(); i++) {
    ...
    std::map<int, KeyFrame>::iterator animIter =
        nodeAnimations.at(i)->animation.begin();
    while (animIter != nodeAnimations.at(i)->animation.end()) {
      ...
      if (nodeAnimations.at(i)->nodeName != "player") {
        line.append(real_to_str(animIter->second.orientation.elements[0]) +
                    ","); // X element
        line.append(real_to_str(animIter->second.orientation.elements[1]) +
                    ","); // Y element
        line.append(real_to_str(animIter->second.orientation.elements[2]) +
                    ","); // Z element
        line.append(real_to_str(animIter->second.orientation.elements[3]) +
                    ","); // W element
      } else {
        line.append(real_to_str(animIter->second.position.coords[0]) +
                    ","); // X pos
        line.append(real_to_str(animIter->second.position.coords[1]) +
                    ","); // Y pos
        line.append(real_to_str(animIter->second.position.coords[2]) +
                    ","); // Z pos
      }
      animIter++;
    }
...
void Animation::Mirror() {
  name.append("_mirror");
  (currentFoot == e_Foot_Right) ? currentFoot = e_Foot_Left
                                : currentFoot = e_Foot_Right;
  ...
  for (unsigned int i = 0; i < nodeAnimations.size(); i++) {
    ...
    while (keyIter != nodeAnimations.at(i)->animation.end()) {
      if (i == 0) {
        keyIter->second.position.coords[0] =
            -keyIter->second.position.coords[0];
      } else {
        keyIter->second.orientation.elements[1] =
            -keyIter->second.orientation.elements[1];
        keyIter->second.orientation.elements[2] =
            -keyIter->second.orientation.elements[2];
      }
      keyIter++;
    }
...
std::string Animation::GetName() const { return name; }

void Animation::AddExtension(const std::string &name,
                             boost::shared_ptr<AnimationExtension> extension) {
  extensions.insert(
      std::pair<std::string, boost::shared_ptr<AnimationExtension>>(name,
                                                                    extension));
}

boost::shared_ptr<AnimationExtension>
Animation::GetExtension(const std::string &name) {
...
void Animation::SetVariable(const std::string &name, const std::string &value) {
  ...
  std::map<const char *, std::string>::iterator iter2 =
      variableCache.find(name.c_str());
...
boost::shared_ptr<XMLTree> Animation::GetCustomData() { return customData; }
...
void Animation::Hax() {
  /*
      ...
      int frame;
      bool touch =
     boost::static_pointer_cast<FootballAnimationExtension>(GetExtension("football"))->GetFirstTouch(position,
     frame); if (touch) { Vector3 angles, position; float power;
        ...
        //position = Vector3(0.0, -0.1 - 0.012 *
     GetOutgoingMovement().GetLength() * frame, 0);
  ...
  */
  ...
  /*
        XMLTree tree;
        tree.value = "movement";
        customData->children.insert(std::pair<std::string, XMLTree>("type",
     tree));
  */
  ...
  /*
      for (int i = 0; i < nodeAnimations.size(); i++) {
        ...
        if (line == "player node")
     nodeAnimations.at(i)->node->SetName("player");
      }
      printf("\n\n");
  */
  ...
  */
  ...
  /*
          std::map<int, KeyFrame>::iterator animIter =
     nodeAnimations.at(0)->animation.begin(); while (animIter !=
     nodeAnimations.at(0)->animation.end()) { Vector3 angles =
     animIter->second.angles; animIter->second.angles = Vector3(0);
            SetKeyFrame(nodeAnimations.at(1)->node, animIter->first, angles,
     Vector3(0)); animIter++;
          }
  */
  ...
  /*
      for (int i = 0; i < (signed int)nodeAnimations.size(); i++) {
        ...
        if (line == "middle") {
          std::map<int, KeyFrame>::iterator animIter =
     nodeAnimations.at(i)->animation.begin(); while (animIter !=
     nodeAnimations.at(i)->animation.end()) { animIter->second.angles.coords[0]
     += (pi * 2) / 360.0 * 2; animIter++;
          }
        }
      }
  */
  ...
  for (int i = 0; i < (signed int)nodeAnimations.size(); i++) {
    ...
    if (line == "body") {
      std::map<int, KeyFrame>::iterator animIter =
  nodeAnimations.at(i)->animation.begin(); while (animIter !=
  nodeAnimations.at(i)->animation.end()) { radian rotXa, rotYa, rotZa;
...
*/
  ...
  */
  ...
  /*
          std::map<int, KeyFrame>::iterator animIter =
     nodeAnimations.at(1)->animation.begin(); while (animIter !=
     nodeAnimations.at(1)->animation.end()) { Vector3 angles =
     animIter->second.angles; animIter->second.position += Vector3(0, 0, 0.74);
            animIter++;
          }
  */
  ...
  /*
      ...
      float vel = (GetIncomingMovement().GetLength() +
     GetOutgoingMovement().GetLength() * 2) / 3.0; angle *= vel;

          std::map<int, KeyFrame>::iterator animIter =
     nodeAnimations.at(1)->animation.begin(); while (animIter !=
     nodeAnimations.at(1)->animation.end()) { animIter->second.angles.coords[1]
     += angle * 3; animIter++;
          }
  */
  ...
  for (int i = 0; i < (signed int)nodeAnimations.size(); i++) {
    ...
    if (line == "left_shoulder" || line == "right_shoulder") {
      std::map<int, KeyFrame>::iterator animIter =
  nodeAnimations.at(i)->animation.begin(); while (animIter !=
  nodeAnimations.at(i)->animation.end()) { float offset; if (line ==
  "left_shoulder") offset = -0.05 * pi; else offset = 0.05 * pi;
