...
public:
  ...
  // depricated: LoadObject now recurses, this would take away the need for a
  // level->objects hierarchy
  boost::intrusive_ptr<Node> LoadLevel(boost::shared_ptr<Scene3D> scene3D,
                                       const std::string &filename) const;

  boost::intrusive_ptr<Node>
  LoadObject(boost::shared_ptr<Scene3D> scene3D, const std::string &filename,
             const Vector3 &offset = Vector3(0)) const;

protected:
  boost::intrusive_ptr<Node> LoadObjectImpl(boost::shared_ptr<Scene3D> scene3D,
