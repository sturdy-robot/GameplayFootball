...
Text2D::Text2D(boost::shared_ptr<Scene2D> scene2D, const std::string &fontFile,
               int pts)
    : count(0), scene2D(scene2D) {
  assert(1 ==
         2); // todo: error, this file is deprecated, can't do ttf_init for one,
             // already done that. check if this file/this whole old 2d/gui
             // system is used anyway, and if not, remove from project
  TTF_Init();
...
boost::intrusive_ptr<Image2D> Text2D::Create(int width) {
  ...
  SDL_Surface *sdlTextPow2 =
      SDL_CreateRGBSurface(alpha, pot(sdlText->w), pot(sdlText->h), 32, r_mask,
                           g_mask, b_mask, a_mask);

  // printf("address old: %i\n", sdlTextPow2);

  // test
  // sdl_putpixel(sdlTextPow2, 1, 1, SDL_MapRGB(sdlTextPow2->format, 255, 255,
  // 255)); sdl_putpixel(sdlTextPow2, 4, 4, SDL_MapRGB(sdlTextPow2->format, 255,
  // 155, 155));
  ...
  boost::intrusive_ptr<Image2D> image =
      static_pointer_cast<Image2D>(ObjectFactory::GetInstance().CreateObject(
...
void Text2D::SetText(boost::intrusive_ptr<Image2D> image,
                     const std::string &text, const Vector3 &color) const {
...
SDL_Surface *Text2D::RenderTextSurface(const std::string &text,
