...
Gui2WindowManager::Gui2WindowManager(boost::shared_ptr<Scene2D> scene2D,
    ...
    : scene2D(scene2D), aspectRatio(aspectRatio), margin(margin),
      pageFactory(0) {
  ...
  if (contextAspectRatio >
      aspectRatio) { // context width is larger than "virtual context's" width,
                     // so cap by height
  ...
  blackoutBackground = boost::static_pointer_cast<Image2D>(
      ObjectFactory::GetInstance().CreateObject("gui2_blackoutbackground",
...
Gui2View *Gui2WindowManager::GetFocus() { return focus; }
...
void Gui2WindowManager::GetCoordinates(float x_percent, float y_percent,
                                       float width_percent,
                                       float height_percent, int &x, int &y,
...
boost::intrusive_ptr<Image2D>
Gui2WindowManager::CreateImage2D(const std::string &name, int width, int height,
                                 bool sceneRegister) {
