...
Gui2IconSelector::Gui2IconSelector(Gui2WindowManager *windowManager,
  ...
  Gui2Caption *captionView = new Gui2Caption(windowManager, name + "caption", 1,
                                             0.3, width_percent, 2.5, caption);
  this->AddView(captionView);
  captionView->Show();
  selectedCaption =
      new Gui2Caption(windowManager, name + "selectedcaption", 1,
...
void Gui2IconSelector::GetImages(
...
void Gui2IconSelector::Process() {
  ...
  if (fadeOut_ms <= fadeOutTime_ms) {
    ...
    if (!IsFocussed()) { // && fadeOut_ms <= fadeOutTime_ms) { // cool fadeout
                         // effect!
      redraw = true;
    }
  }
  ...
  if (visibleSelectedEntry != (float)selectedEntry)
    redraw = true;
  if (fabs(visibleSelectedEntry - (float)selectedEntry) <=
      0.08f * scrollSpeedFactor) {
...
void Gui2IconSelector::Redraw() {
  ...
  if (IsFocussed()) {
    ...
    alpha = int(floor(200 - (clamp(fadeOut_ms, 0, fadeOutTime_ms) /
  ...
  windowManager->GetCoordinates(x_percent, y_percent, width_percent,
                                height_percent, x, y, w, h);
  image->DrawRectangle(
      0, 0, w, h, windowManager->GetStyle()->GetColor(e_DecorationType_Bright2),
      alpha);
  image->OnChange();
  ...
  for (int i = 0; i < (signed int)entries.size(); i++) {
    ...
    if (position > 0.5 * -pi && position < 0.5 * pi) {
      ...
      entries.at(i).icon->SetPosition(xpos - 8.0 * 0.5,
                                      height_percent * 0.5 - 10.0 * 0.5);
...
void Gui2IconSelector::AddEntry(const std::string &id,
                                const std::string &caption,
...
void Gui2IconSelector::OnGainFocus() { Redraw(); }

void Gui2IconSelector::OnLoseFocus() { fadeOut_ms = 0; }
