...
Gui2Text::Gui2Text(Gui2WindowManager *windowManager, const std::string &name,
                   float x_percent, float y_percent, float width_percent,
                   float height_percent, float fontsize_percent,
                   unsigned int maxHorizChars, const std::string &text)
    : Gui2View(windowManager, name, x_percent, y_percent, width_percent,
               height_percent),
      fontsize_percent(fontsize_percent), maxHorizChars(maxHorizChars) {
  int x, y, w, h;
...
void Gui2Text::AddEmptyLine() {
  ...
  Gui2Caption *caption = new Gui2Caption(
      ...
      (resultText.size() - 1) * fontsize_percent * 1.5f, width_percent,
      fontsize_percent, "");
...
void Gui2Text::AddText(const std::string &newText) {
  ...
  for (unsigned int i = 0; i < resultText.size(); i++) {
    Gui2Caption *caption =
        new Gui2Caption(windowManager, GetName() + int_to_str(i), 0,
                        i * fontsize_percent * 1.5f, width_percent,
                        fontsize_percent, resultText.at(i));
    this->AddView(caption);
    caption->Show();
  }
}
