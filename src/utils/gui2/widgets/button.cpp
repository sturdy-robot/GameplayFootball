...
Gui2Button::Gui2Button(Gui2WindowManager *windowManager,
  ...
  captionView = new Gui2Caption(windowManager, name + "caption", 1.2, 0.3,
                                width_percent, height_percent - 0.6, caption);
...
void Gui2Button::Redraw() {
  ...
  if (IsFocussed()) {
    ...
    if (toggleable && toggled)
      color1 = windowManager->GetStyle()->GetColor(e_DecorationType_Toggled);
    ...
    if (toggleable && toggled)
      selectedColor =
          windowManager->GetStyle()->GetColor(e_DecorationType_Toggled);
