...
Gui2FileBrowser::Gui2FileBrowser(Gui2WindowManager *windowManager,
    ...
    : Gui2View(windowManager, name, x_percent, y_percent, width_percent,
               height_percent),
      directory(startDir), selectType(selectType) {
  clickedID = -1;
...
void Gui2FileBrowser::DisplayDirectory() {
  ...
  for (unsigned int i = 0; i < dirContents.size(); i++) {
    ...
    Gui2Button *button = new Gui2Button(
        windowManager, this->GetName() + "_" + dirContents.at(i).name, 0, 0,
...
void Gui2FileBrowser::GetDirectoryContents() {
  ...
  while (dirIter != endIter) {
    DirEntry entry;
    entry.name = dirIter->path()
                     .filename()
                     .string(); // used to be .native() .. todo, check out why
    entry.type = is_directory(dirIter->status()) ? e_DirEntryType_Directory
                                                 : e_DirEntryType_File;
    dirContents.push_back(entry);
