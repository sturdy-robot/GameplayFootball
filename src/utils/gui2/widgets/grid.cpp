...
Gui2Grid::Gui2Grid(Gui2WindowManager *windowManager, const std::string &name,
...
void Gui2Grid::UpdateLayout(float margin_left_percent,
  ...
  for (unsigned int i = 0; i < container.size(); i++) {
    if (container.at(i).row >= offsetRows && container.at(i).row < rows) {
      container.at(i).view->SetPosition(widths[container.at(i).col],
                                        heights[container.at(i).row]);
    }
    // printf("%f, %f\n", widths[container.at(i).col],
    // heights[container.at(i).row]);
  }
...
void Gui2Grid::UpdateScrolling() {
  ...
  for (unsigned int i = 0; i < container.size(); i++) {
    if (container.at(i).row >= offsetRows &&
        container.at(i).row < offsetRows + maxVisibleRows)
  ...
  if (rows > maxVisibleRows) {
    if (scrollY == 0) {
      scrollY = new Gui2Scrollbar(windowManager, name + "_scroll_vert",
                                  x_percent + width_percent - 1, y_percent, 1,
                                  height_percent);
      Gui2View::AddView(scrollY);
      scrollY->Show();
    }
    scrollY->SetSizePercent((maxVisibleRows / (float)rows) * 100);
    scrollY->SetProgressPercent((offsetRows / (float)(rows - maxVisibleRows)) *
                                100);
    scrollY->Redraw();
  } else {
    if (scrollY != 0) {
...
void Gui2Grid::ProcessWindowingEvent(WindowingEvent *event) {
  ...
  bool movedOutOfGrid = false; // maybe we need to ignore this event so parent
                               // can receive it instead

  if ((xoffset != 0 || yoffset != 0) && switchDelay_ms >= minSwitchDelay_ms &&
      hasSelectables) {
    ...
    while (!selectableFound && !movedOutOfGrid) {
      ...
      provisionalTarget =
          FindView(provisionalSelectedRow, provisionalSelectedCol);
