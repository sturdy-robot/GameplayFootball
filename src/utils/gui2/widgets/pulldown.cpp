...
Gui2Pulldown::Gui2Pulldown(Gui2WindowManager *windowManager,
  ...
  bg = new Gui2Image(windowManager, name + "_bg", 0, 0, width_percent,
                     height_percent);
  bg->LoadImage("media/menu/backgrounds/black.png");
  bg->Show();

  grid = new Gui2Grid(windowManager, name + "_grid", 0, 0, width_percent,
                      height_percent);
  grid->Show();

  selectedEntry = 0;

  pulldownButton = new Gui2Button(windowManager, name + "_pulldownbutton", 0, 0,
                                  width_percent, 3, "tmp");
  pulldownButton->sig_OnClick.connect(
      boost::bind(&Gui2Pulldown::PullDownOrUp, this));
...
void Gui2Pulldown::AddEntry(const std::string &caption,
                            const std::string &name) {

  Gui2Button *button = new Gui2Button(
      windowManager, name + "_entry" + int_to_str(entries.size()), 0, 0,
      width_percent - 1, 3, caption);
  button->sig_OnClick.connect(
      boost::bind(&Gui2Pulldown::Select, this, entries.size()));
