...
Gui2Slider::Gui2Slider(Gui2WindowManager *windowManager,
    ...
    : Gui2View(windowManager, name, x_percent, y_percent, width_percent,
               height_percent),
      quantizationSteps(51), caption(caption) {
  ...
  titleCaption = new Gui2Caption(windowManager, name + "caption", 1.0, 0.2,
                                 width_percent, 2.4, caption);
...
void Gui2Slider::Redraw() {
  ...
  for (unsigned int i = 0; i < helperValues.size(); i++) {
    ...
    image->DrawRectangle(x_margin * 2 + helperValue * (w - x_margin * 6),
                         h * 0.5, x_ratio, h * 0.5, helperColor, alpha);
  }

  // slider groove
  image->DrawRectangle(x_margin * 2, h * 0.7, w - x_margin * 4, h * 0.1, color2,
                       255);

  // the slider
  image->DrawRectangle(x_margin * 2 + quantizedValue * (w - x_margin * 6),
                       h * 0.5, x_ratio, h * 0.5, color2, 160);
...
void Gui2Slider::ProcessWindowingEvent(WindowingEvent *event) {
  ...
  if (direction.coords[0] > 0.99f ||
      direction.coords[0] < 0.01f) { // full effect, maybe digital input, so
                                     // make this 1 quantized step
    fullStep = true;
  }
  ...
  if (xoffset != 0) {
    ...
    if (value < 0.0f)
      value = 0.0f;
    quantizedValue =
        round(value * (quantizationSteps - 1)) / (quantizationSteps - 1.0f);
    sig_OnChange(this);
    Redraw();
  } else {
    event->Ignore();
...
void Gui2Slider::OnGainFocus() { Redraw(); }
...
int Gui2Slider::AddHelperValue(const Vector3 &color,
                               const std::string &description,
                               float initialValue) {
  Gui2Slider_HelperValue helper;
  helper.index = helperValues.empty()
                     ? 1
                     : helperValues.at(helperValues.size() - 1).index + 1;
  helper.color = color;
  helper.descriptionCaption = new Gui2Caption(
      windowManager,
      "gui2sliderhelper_" + int_to_str(helper.index) + "_description_caption",
      1.0, 0.2, width_percent, 2.4, description);
