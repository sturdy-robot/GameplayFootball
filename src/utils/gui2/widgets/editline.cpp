...
Gui2EditLine::Gui2EditLine(Gui2WindowManager *windowManager,
  ...
  caption = new Gui2Caption(windowManager, GetName() + "_caption", 0, 0,
                            width_percent, height_percent, defaultText);
  ...
  cursor = new Gui2Image(windowManager, GetName() + "_cursor", 0, 0, 0.3f,
                         height_percent);

  Vector3 cursorSize = cursor->GetImage2D()->GetSize();
  cursor->GetImage2D()->DrawRectangle(
      0, 0, cursorSize.coords[0], cursorSize.coords[1],
      windowManager->GetStyle()->GetColor(e_DecorationType_Bright1), 127);
...
void Gui2EditLine::ProcessKeyboardEvent(KeyboardEvent *event) {
  ...
  for (auto key : event->GetKeysymRepeated()) {
    ...
    if (event->GetKeyRepeated(key)) {
      ...
      // ascii char
      if ((key & 0xFF80) == 0 && key >= 0x20 &&
          currentText.length() <= maxLength) {
        char ch = key & 0x7F;
        if (allowedChars.length() == 0 ||
            allowedChars.find_first_of(ch) != std::string::npos) {
