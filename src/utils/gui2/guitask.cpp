...
void Gui2Task::ProcessPhase() {
  windowManager->GetRoot()->SetRecursiveZPriority(
      0); // todo: this can probably be optimized (like, doing it every frame
          // seems redundant?)
...
void Gui2Task::ProcessEvents() {
  ...
  // todo: stop making objects every process(); this is terribly inefficient.
  // find out if they are needed in the first place.

  if (!windowManager->GetFocus())
    return;
  Gui2View *currentFocus = windowManager->GetFocus();

  // some buttons need to be sent through a windowing event (escape, enter,
  // directional controls) if a view accepts a keyboard/joystick event, don't
  // send windowing event => *disabled, now uses focus check
  ...
  for (auto keyState : currentKeyState) {
    ...
    if (prevKeyState.count(pressedKey) == 0 ||
        UserEventManager::GetInstance().GetLastKeyPressDiff_ms(pressedKey) >
            250) {
  ...
  for (int j = 0; j < UserEventManager::GetInstance().GetJoystickCount(); j++) {
    ...
    for (int i = 0; i < _JOYSTICK_MAXBUTTONS; i++) {
      if (UserEventManager::GetInstance().GetJoyButtonState(j, i) !=
          prevButtonState[j][i]) {
        buttonChanged = true;
        break;
      }
