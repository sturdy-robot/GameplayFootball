...
Console::Console(boost::shared_ptr<Scene2D> scene2D, int numlines)
...
void Console::PrintLog(e_LogType logType, std::string className,
                       std::string methodName, std::string message) {
  ...
  char bla[2048];
  sprintf(bla, "[%s] in [%s::%s]: %s", logTypeString.c_str(), className.c_str(),
