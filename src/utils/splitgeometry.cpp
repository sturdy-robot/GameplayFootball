...
boost::intrusive_ptr<Resource<GeometryData>>
GetGridGeom(const std::string &name, boost::shared_ptr<Scene3D> scene3D,
  ...
  newIndex.x = x;
  newIndex.y = y;
  newIndex.geomData = ResourceManagerPool::GetInstance()
                          .GetManager<GeometryData>(e_ResourceType_GeometryData)
                          ->Fetch(name + " gridGeomData @ " + int_to_str(x) +
                                      ", " + int_to_str(y),
                                  false, false);
...
boost::intrusive_ptr<Node> SplitGeometry(boost::shared_ptr<Scene3D> scene3D,
  ...
  boost::intrusive_ptr<Resource<GeometryData>> geomData =
      source->GetGeometryData();

  geomData->resourceMutex
      .lock(); // lock all the time, else triangle meshes might get altered

  std::vector<MaterializedTriangleMesh> tmeshes =
      geomData->GetResource()->GetTriangleMeshes();

  // printf("%i subgeoms found\n", tmeshes.size());
  for (int i = 0; i < (signed int)tmeshes.size(); i++) {

    std::vector<unsigned int> indices; // empty == don't use indices
    AABB aabb = GetTriangleMeshAABB(tmeshes.at(i).vertices,
    ...
    memcpy(newTMesh, tmeshes.at(i).vertices,
           tmeshes.at(i).verticesDataSize * sizeof(float));
    subGeomData->resourceMutex.lock();
    subGeomData->GetResource()->AddTriangleMesh(
        tmeshes.at(i).material, newTMesh, tmeshes.at(i).verticesDataSize,
        indices);
