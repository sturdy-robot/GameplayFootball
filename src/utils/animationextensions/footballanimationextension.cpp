...
FootballAnimationExtension::~FootballAnimationExtension() { animation.clear(); }

void FootballAnimationExtension::Shift(
    int fromFrame, int offset) { // todo: offset does not yet work
...
bool FootballAnimationExtension::GetKeyFrame(int frame, Quaternion &orientation,
                                             Vector3 &position,
                                             float &power) const {
  std::map<int, FootballKeyFrame>::const_iterator animIter =
      animation.find(frame);
