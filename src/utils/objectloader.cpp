...
ObjectLoader::LoadLevel(boost::shared_ptr<Scene3D> scene3D,
  ...
  map_XMLTree::iterator iter =
      levelTree.children.begin()->second.children.begin();
  while (iter != levelTree.children.begin()->second.children.end()) {
    if ((*iter).first == "object") {
      ...
      boost::intrusive_ptr<Node> objNode =
          LoadObject(scene3D, "media/" + filename);
...
boost::intrusive_ptr<Node>
ObjectLoader::LoadObject(boost::shared_ptr<Scene3D> scene3D,
  ...
  // printf("\n\n");
  boost::intrusive_ptr<Node> result = LoadObjectImpl(
      scene3D, filename, objectTree.children.begin()->second, offset);
  // printf("\n\n");
  // result->PrintTree();
  // printf("\n\n");
  return result;
...
boost::intrusive_ptr<Node> ObjectLoader::LoadObjectImpl(
    boost::shared_ptr<Scene3D> scene3D, const std::string &nodename,
    const XMLTree &objectTree, const Vector3 &offset) const {
  ...
  while (objectIter != objectTree.children.end()) {
    ...
    if (objectIter->first == "node") {
      objNode->AddNode(
          LoadObjectImpl(scene3D, dirpart, objectIter->second, offset));
    }
    ...
    // JOINT waaaah smoke em
      ...
      if (target_1 != "")
        target_1_object =
            static_pointer_cast<Geometry>(objNode->GetObject(target_1));
      if (target_2 != "")
        target_2_object =
            static_pointer_cast<Geometry>(objNode->GetObject(target_2));
...
void ObjectLoader::InterpretProperties(const map_XMLTree &tree,
  ...
  while (propIter != tree.end()) {
    ...
    // printf("%s %s\n", propIter->first.c_str(),
    // propIter->second.value.c_str());
    propIter++;
  }
}
