...
std::string XMLLoader::GetSource(const XMLTree &source, int depth) const {
  ...
  std::multimap<std::string, XMLTree>::const_iterator iter =
...
void XMLLoader::PrintTree(const XMLTree &source) const {
  /*std::multimap<std::string, XMLTree>::const_iterator iter =
  source.children.begin(); if (iter == source.children.end()) { if (source.value
  != "") { for (int i = 0; i < depth; i++) printf("\t"); printf("'%s'\n",
  source.value.c_str());
    }
  } else {
    while (iter != source.children.end()) {
