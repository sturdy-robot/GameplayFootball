...
WorkerThread::WorkerThread(int affinity)
    : Thread(), affinity(affinity), messagesHandled(0) {
  taskManager = TaskManager::GetInstancePtr();
}

WorkerThread::~WorkerThread() {
  Log(e_Notice, "WorkerThread", "~WorkerThread",
      int_to_str(messagesHandled) + " messages handled");
}
...
void WorkerThread::GetWorkerState(e_ThreadState &state,
