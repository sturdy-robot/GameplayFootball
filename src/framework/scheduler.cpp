...
void Scheduler::UnregisterTaskSequence(
    boost::shared_ptr<TaskSequence> sequence) {
  // todo
}
...
void Scheduler::ResetTaskSequenceTime(const std::string &name) {
  ...
  for (unsigned int i = 0; i < sequences.data.size(); i++) {
    ...
    if (program->taskSequence->GetName() == name) {
      program->startTime =
          EnvironmentManager::GetInstance().GetTime_ms(); // - startTime_ms;
      program->timesRan = 0;
      break;
    }
  }
  ...
  for (unsigned int i = 0; i < sequences.data.size(); i++) {
    ...
    if (program->taskSequence->GetName() == name) {
      ...
      if (program->taskSequence->GetSkippable()) {
        resultTime_ms = program->sequenceStartTime +
...
bool Scheduler::Run() {
  ...
  while (EnvironmentManager::GetInstance().GetQuit() == false ||
         GetSequenceCount() > 0) {
    ...
    for (unsigned int i = 0; i < sequences.data.size(); i++) {
      ...
      boost::shared_ptr<TaskSequenceProgram> program =
          sequences.data.at(programIndex);
          ...
          if (program->dueQuit == true) {
            ...
            if (program->taskSequence->GetSkippable()) {
              ...
              timeUntilDueEntry_ms =
                  (program->sequenceStartTime +
                   program->taskSequence->GetSequenceTime()) -
                  time_ms;
              // printf("wiieee: %i .. %li .. %li\n",
              // program->taskSequence->GetFrameTime(), time_ms,
              // startTimeRel_ms);
            } else {
              // use absolute time: if not enough iterations have been done to
              // get to frametime * timesran, start immediately
              timeUntilDueEntry_ms = (program->startTime +
                                      program->taskSequence->GetSequenceTime() *
                                          program->timesRan) -
                                     time_ms;
              // printf("wiieee: %i .. %li .. %li\n",
              // program->taskSequence->GetSequenceTime(), time_ms,
              // timeUntilDueEntry_ms);
            }
          }
        }
    ...
    if (someSequenceNeedsDeleting) {
      ...
      std::vector<boost::shared_ptr<TaskSequenceProgram>>::iterator quiterator =
      ...
      // todo: this way, longer sequences get relatively less time in total. is
      // this desirable?
      firstSequence++;
      ...
      if (dueEntry.program != boost::shared_ptr<TaskSequenceProgram>()) {
        // wait until time for due sequence entry (even if that is <= 0, because
        // we need to unlock the condition and sequences locks anyway)
      ...
      if (dueEntry.program != boost::shared_ptr<TaskSequenceProgram>()) {
        ...
        if (dueEntry.timeUntilDueEntry_ms <=
            0) { // the first and/or other entries are due
          ...
          dueEntry.program->taskSequence
              ->GetEntry(dueEntry.program->programCounter)
              ->Reset();
          dueEntry.program->taskSequence
              ->GetEntry(dueEntry.program->programCounter)
              ->Execute();

          dueEntry.program->previousProgramCounter =
              dueEntry.program->programCounter;
          dueEntry.program->programCounter++;
        }
      }
