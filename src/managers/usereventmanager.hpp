...
public:
  ...
  void SetJoystickAxisCalibration(int joyID, int axisID, float min, float max,
                                  float rest);

protected:
  // may need to switch to the vectors below in the future. why? SDL_keysym also
  // takes into account unicode and modifier stuff, and contains the
  // SDL_Keycodes. so basically, it's the parent structure, and we are going to
  // need that info at some point, and we can't make an array with the max size
  // (probably) since i guess it has a dynamic size.
  std::map<SDL_Keycode, TimedKeyPress> keyPressed;
  unsigned long lastKeyTime_ms = 0;

  bool mousePressed[8];

  SDL_GameController *joystick[_JOYSTICK_MAX];
  bool joyButtonPressed[_JOYSTICK_MAX][_JOYSTICK_MAXBUTTONS];
  float joyAxis[_JOYSTICK_MAX][_JOYSTICK_MAXAXES];
  float joyAxisCalibration[_JOYSTICK_MAX][_JOYSTICK_MAXAXES]
                          [3]; // min, max, rest
