...
public:
  ...
  void CleanUp() {
    std::map<e_ResourceType, boost::shared_ptr<void>>::iterator resmanIter =
        resourceManagers.begin();
    while (resmanIter != resourceManagers.end()) {
      boost::static_pointer_cast<ResourceManager<void>>((*resmanIter).second)
          ->RemoveUnused();
      resmanIter++;
    }
  }
  ...
  void RegisterManager(e_ResourceType resourceType,
                       boost::shared_ptr<ResourceManager<T>> resourceManager) {
    resourceManagers.insert(std::make_pair(resourceType, resourceManager));
  }

  template <typename T>
  boost::shared_ptr<ResourceManager<T>>
  GetManager(e_ResourceType resourceType) {
    typename std::map<e_ResourceType, boost::shared_ptr<void>>::iterator iter =
