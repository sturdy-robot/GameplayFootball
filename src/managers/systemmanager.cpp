...
void SystemManager::Exit() {
  ...
  while (s_iter != systems.end()) {
    ...
    Log(e_Notice, "SystemManager", "Exit",
        "Deleting system named '" + s_iter->first + "'");
    system->Exit();
    delete system;
    s_iter++;
  }
...
bool SystemManager::RegisterSystem(const std::string systemName,
                                   ISystem *system) {
  ...
  Log(e_Notice, "SystemManager", "RegisterSystem",
      "Added system named '" + systemName + "'");
  return true;
}
...
const map_Systems &SystemManager::GetSystems() const { return systems; }
