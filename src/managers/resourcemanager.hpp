...
public:
  ResourceManager(const std::string &typeDescription)
  ...
  boost::intrusive_ptr<Resource<T>> Fetch(const std::string &filename,
                                          bool load = true,
  ...
  boost::intrusive_ptr<Resource<T>> Fetch(const std::string &filename,
    ...
    if (success) {
      ...
      alreadyThere = false;
      boost::intrusive_ptr<Resource<T>> resource(
          new Resource<T>(adaptedFilename));
      ...
      if (load) {
        ...
        std::transform(extension.begin(), extension.end(), extension.begin(),
                       ::tolower);
        typename std::map<std::string, Loader<T> *>::iterator iter =
            loaders.find(extension);
        if (iter != loaders.end()) {
          (*iter).second->Load(filename, resource);
        } else {
          Log(e_FatalError, "ResourceManager<>", "Load",
              "There is no loader for " + filename);
        }
      }
      Register(resource);
      return resource;
  ...
  boost::intrusive_ptr<Resource<T>> FetchCopy(const std::string &filename,
    ...
    // todo: this lock is probably needed, but slows down everything very much..
    // find out why
    resource->resourceMutex.lock();
    boost::intrusive_ptr<Resource<T>> resourceCopy(
        new Resource<T>(*resource, newName));
  ...
  boost::intrusive_ptr<Resource<T>> FetchCopy(const std::string &filename,
    ...
    if (resources.data.find(newName) != resources.data.end()) {
      // Log(e_Warning, "ResourceManager", "FetchCopy", "Duplicate key '" +
      // newName + "' - returning existing resource instead of copy (maybe just
      // use Fetch() instead?)");
      resourceCopy = Fetch(newName, false, true);
    } else {
      boost::intrusive_ptr<Resource<T>> resource =
          Fetch(filename, true, alreadyThere, true);

      // todo: this lock is probably needed, but slows down everything very
      // much.. find out why
      resource->resourceMutex.lock();
      resourceCopy = boost::intrusive_ptr<Resource<T>>(
          new Resource<T>(*resource, newName));
      resource->resourceMutex.unlock();
  ...
  void RemoveUnused() {
    ...
    typename std::map<std::string, boost::intrusive_ptr<Resource<T>>>::iterator
        resIter = resources.data.begin();
...
protected:
  boost::intrusive_ptr<Resource<T>> Find(const std::string &identString,
                                         bool &success) {
    resources.Lock();
    typename std::map<std::string, boost::intrusive_ptr<Resource<T>>>::iterator
        resIter = resources.data.find(identString);
  ...
  void Register(boost::intrusive_ptr<Resource<T>> resource) {
    ...
    if (resources.data.find(resource->GetIdentString()) !=
        resources.data.end()) {
      resources.Unlock();
      RemoveUnused();
      resources.Lock();
