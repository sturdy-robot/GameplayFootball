...
void TaskManager::Exit() {
  ...
  if (workQueue.GetPending() > 0)
    Log(e_FatalError, "TaskManager", "Exit",
        workQueue.GetPending() + " messages left on quit!");
}

int TaskManager::GetWorkerThreadCount() { return pool.size(); }

e_ThreadState TaskManager::GetWorkerThreadState(int workerThreadIndex) {
  return pool.at(workerThreadIndex)->GetState();
}

void TaskManager::GetFullWorkerThreadState(int workerThreadIndex,
                                           e_ThreadState &state,
                                           std::string &commandName) {
  pool.at(workerThreadIndex)->GetWorkerState(state, commandName);
}

void TaskManager::EnqueueWork(boost::intrusive_ptr<Command> message,
                              bool notify) { // ATOMIC
  if (pool.size() > 0) {

    if (workQueue.GetPending() > 50) {
      Log(e_Warning, "TaskManager", "EnqueueWork",
          "Too much work to do! Blocking for 1 ms!");
      while (workQueue.GetPending() > 0) {
