...
IntroPage::IntroPage(Gui2WindowManager *windowManager,
...
IntroPage::~IntroPage() { windowManager->BlackoutBackground(false); }
...
OutroPage::OutroPage(Gui2WindowManager *windowManager,
                     const Gui2PageData &pageData)
    : Gui2Page(windowManager, pageData) {
...
OutroPage::~OutroPage() { windowManager->BlackoutBackground(false); }
...
MainMenuPage::MainMenuPage(Gui2WindowManager *windowManager,
    ...
    : Gui2Page(windowManager, pageData) {
  Gui2Image *title =
      new Gui2Image(windowManager, "image_main_title", 28, 32, 44, 20);
  ...
  buttons.push_back(
      new Gui2Button(windowManager, "button_main_start", 0, 0, 20, 3, "Match"));
  buttons.push_back(
      new Gui2Button(windowManager, "button_main_cup", 0, 0, 20, 3, "Cup"));
  buttons.push_back(new Gui2Button(windowManager, "button_main_league", 0, 0,
                                   20, 3, "League"));
  buttons.push_back(
      new Gui2Button(windowManager, "button_main_edit", 0, 0, 20, 3, "Editor"));
  buttons.push_back(new Gui2Button(windowManager, "button_main_settings", 0, 0,
                                   20, 3, "Settings"));
  buttons.push_back(new Gui2Button(windowManager, "button_main_credits", 0, 0,
                                   20, 3, "Credits"));
  buttons.push_back(
      new Gui2Button(windowManager, "button_main_quit", 0, 0, 20, 3, "Exit"));
  if (!IsReleaseVersion()) {
    buttons.push_back(new Gui2Button(windowManager, "button_main_import", 0, 0,
                                     20, 3, "Import FM"));
  }

  buttons.at(0)->sig_OnClick.connect(
      boost::bind(&MainMenuPage::GoControllerSelect, this));
  buttons.at(2)->sig_OnClick.connect(
      boost::bind(&MainMenuPage::GoLeague, this));
  buttons.at(4)->sig_OnClick.connect(
      boost::bind(&MainMenuPage::GoSettings, this));
  buttons.at(5)->sig_OnClick.connect(
      boost::bind(&MainMenuPage::GoCredits, this));
  if (!IsReleaseVersion()) {
    buttons.at(6)->sig_OnClick.connect(
        boost::bind(&MenuTask::QuitGame, GetMenuTask()));
  } else {
    buttons.at(6)->sig_OnClick.connect(
        boost::bind(&MainMenuPage::GoOutro, this));
  }
  if (!IsReleaseVersion()) {
    buttons.at(7)->sig_OnClick.connect(
        boost::bind(&MainMenuPage::GoImportDB, this));
  }
...
void MainMenuPage::GoControllerSelect() {
  ...
  windowManager->GetPageFactory()->CreatePage((int)e_PageID_ControllerSelect,
                                              properties, 0);
...
void MainMenuPage::GoLeague() {
  ...
  Properties properties;
  windowManager->GetPageFactory()->CreatePage((int)e_PageID_League_Start,
                                              properties, 0);
...
void MainMenuPage::GoSettings() {
  ...
  Properties properties;
  windowManager->GetPageFactory()->CreatePage((int)e_PageID_Settings,
                                              properties, 0);
...
void MainMenuPage::GoCredits() {
  ...
  Properties properties;
  windowManager->GetPageFactory()->CreatePage((int)e_PageID_Credits, properties,
                                              0);
...
void MainMenuPage::GoOutro() {
  ...
  Properties properties;
  windowManager->GetPageFactory()->CreatePage((int)e_PageID_Outro, properties,
                                              0);

  delete this;
}

bool SortClubPlayersByAverageStat(const PlayerImport &a,
                                  const PlayerImport &b) {
...
bool SortClubPlayersByRole(const PlayerImport &a, const PlayerImport &b) {
  ...
  for (unsigned int statPos = 0; statPos < a.profileStats.size(); statPos++) {
    if (a.profileStats.at(statPos).name.compare(
            "mental_defensivepositioning") == 0) {
      aDef = a.profileStats.at(statPos).value;
      defPos = statPos;
    }
    if (a.profileStats.at(statPos).name.compare(
            "mental_offensivepositioning") == 0) {
...
std::string RenameLeague(Database *namedb, const std::string &name,
                         const std::string &srcfield) {
  std::string query = "select " + srcfield +
                      " from leaguenames where name = \"" + name +
                      "\" limit 1;";
...
std::string RenameClub(Database *namedb, const std::string &name,
                       const std::string &srcfield) {
  std::string query = "select " + srcfield + " from clubnames where name = \"" +
                      name + "\" limit 1;";
  DatabaseResult *result = namedb->Query(query);
  std::string targetName;
  if (result->data.size() > 0) {
...
ClubData GetClubData(Database *namedb, const std::string &name) {
  std::string query = "select formation_xml, tactics_xml, shortname, color1, "
                      "color2 from clubnames where name = \"" +
                      name + "\" limit 1;";
  DatabaseResult *result = namedb->Query(query);
  ClubData data;
  if (result->data.size() > 0) {
    ...
    data.tactics_xml = "";
    data.shortName = name;
    data.shortName.erase(
        remove_if(data.shortName.begin(), data.shortName.end(), isspace),
        data.shortName.end());
...
void GetPlayerData(Database *namedb, const std::string &firstname,
                   const std::string &lastname, std::string &fakefirstname_ret,
                   std::string &fakelastname_ret, int &skincolor_ret,
                   std::string &hairstyle_ret, std::string &haircolor_ret,
                   float &height_ret, float &weight_ret,
                   signed int &formationorder_ret, float &baseStatOffset_ret,
                   std::string &customProfile_ret) {
  ...
  std::string query =
      "select fakefirstname, fakelastname, skincolor, hairstyle, haircolor, "
      "height, weight, formationorder, base_stat_offset, profile_xml from "
      "playernames where firstname = \"" +
...
/* THIS CODE IMPORTS FM DATABASES, exported with some FM management tool, of
 * which I forgot the name. maybe it's useful for somebody. */
...
bool MainMenuPage::GoImportDB() {
  ...
  result =
      GetDB()->Query("CREATE TABLE teams(id INTEGER PRIMARY KEY AUTOINCREMENT, "
                     "league_id INTEGER, "
                     "name VARCHAR(64), "
                     "logo_url VARCHAR(512), "
                     "kit_url VARCHAR(512), "
  ...
  // 0.0</position><role>CF</role></p10><p11><position>0.7,
  ...
  */
  ...
  /*
  // 4-4-2
  std::string defaultFormation = "<p1><position>-1.0,
  0.0</position><role>GK</role></p1>"

                                 "<p2><position>-0.7,
  0.6</position><role>LB</role></p2>"
                                 "<p3><position>-0.8,
  0.2</position><role>CB</role></p3>"
                                 "<p4><position>-0.8,
  -0.2</position><role>CB</role></p4>"
                                 "<p5><position>-0.7,
  -0.6</position><role>RB</role></p5>"

                                 "<p6><position> 0.0,
  0.9</position><role>LM</role></p6>"
                                 "<p7><position>-0.3,
  0.0</position><role>DM</role></p7>"
                                 "<p8><position> 0.0,
  -0.9</position><role>RM</role></p8>"
                                 "<p9><position> 0.3,
  0.0</position><role>AM</role></p9>"

                                 "<p10><position>0.8,
  0.3</position><role>CF</role></p10>"
                                 "<p11><position>0.8,
  -0.3</position><role>CF</role></p11>";
  ...
  0.3</position><role>CF</role></p10><p11><position>0.8,
  -0.3</position><role>CF</role></p11>
  */

  /*
  // 4-3-3
  std::string defaultFormation = "<p1><position> -1.0,  0.0
  </position><role>GK</role></p1>"

                                 "<p2><position> -0.7,
  0.75</position><role>LB</role></p2>"
                                 "<p3><position> -1.0,
  0.25</position><role>CB</role></p3>"
                                 "<p4><position> -1.0,
  -0.25</position><role>CB</role></p4>"
                                 "<p5><position> -0.7,
  -0.75</position><role>RB</role></p5>"

                                 "<p6><position>  0.0,  0.5
  </position><role>CM</role></p6>"
                                 "<p7><position> -0.2,  0.0
  </position><role>CM</role></p7>"
                                 "<p8><position>  0.0, -0.5
  </position><role>CM</role></p8>"

                                 "<p9><position>  0.6,  0.75
  </position><role>LM</role></p9>"
                                 "<p10><position> 1.0,  0.0
  </position><role>CF</role></p10>"
                                 "<p11><position> 0.6, -0.75
  </position><role>RM</role></p11>";

  //<p1><position> -1.0,  0.0 </position><role>GK</role></p1><p2><position>
  -0.7,  0.75</position><role>LB</role></p2><p3><position> -1.0,
  0.25</position><role>CB</role></p3><p4><position> -1.0,
  -0.25</position><role>CB</role></p4><p5><position> -0.7,
  ...
  */
  ...
  std::string defaultTactics =
      ...
      "<position_offense_sidefocus_strength>0.5</"
      "position_offense_sidefocus_strength>"
      "<position_defense_sidefocus_strength>0.5</"
      "position_defense_sidefocus_strength>"

      "<position_offense_microfocus_strength>0.5</"
      "position_offense_microfocus_strength>"
      "<position_defense_microfocus_strength>0.5</"
  ...
  // put these in a map with country + competition (as string) as key to get
  ...
  for (unsigned int i = 1; i < data.size(); i++) { // (skip header)
    ...
    for (unsigned int t = 0; t < tokens.size(); t++) {
      tokens.at(t).erase(
          std::remove(tokens.at(t).begin(), tokens.at(t).end(), '\"'),
          tokens.at(t).end());
    ...
    /* public beta 1
    if ((club.country.compare("England") == 0 &&
    club.competition.compare("Premier Division") == 0) ||
        (club.country.compare("Germany") == 0 && club.competition.compare("First
    Division") == 0) || (club.country.compare("Holland") == 0 &&
    club.competition.compare("Eredivisie") == 0) ||
        (club.country.compare("Spain")   == 0 && club.competition.compare("LIGA
    BBVA") == 0)) { addClub = true;
    }
    */
    ...
    if (addClub && club.status.compare("Professional") == 0) {
      countries.insert(tokens.at(
          1)); // sets only support unique elements so there'll be no dupes
  ...
  query = "begin transaction; delete from regions; delete from sqlite_sequence "
          "where name=\"regions\";";
  ...
  query = "begin transaction; delete from countries; delete from "
          "sqlite_sequence where name=\"countries\";";
  std::set<std::string>::iterator iter = countries.begin();
  for (unsigned int i = 0; i < countries.size(); i++) {
    query += "insert into countries (id, region_id, name) values (" +
             int_to_str(i + 1) + ", 1, \"" + *iter + "\");";
  ...
  query = "begin transaction; delete from leagues; delete from sqlite_sequence "
          "where name=\"leagues\";";
  std::map<std::string, CountryCompetition>::iterator compIter =
      uniqueCompetitions.begin();
  id = 1;
  while (compIter != uniqueCompetitions.end()) {
    std::map<std::string, int>::iterator countryIter =
        countryIDs.find(compIter->second.country);
    if (countryIter != countryIDs.end()) {
      ...
      std::string strippedCompetitionFilename =
          StripString(competitionFilename);
      std::transform(strippedCompetitionFilename.begin(),
                     strippedCompetitionFilename.end(),
      ...
      query += "insert into leagues (id, name, logo_url, country_id) values (" +
               int_to_str(id) + ", \"" + competitionName +
               "\", \"images_competitions/" + strippedCompetitionFilename +
               ".png\", " + int_to_str(countryID) + ");";
      competitionIDs.insert(std::pair<std::string, int>(
          compIter->second.country + compIter->second.competition, id));
      id++;
    }
    compIter++;
  }
  ...
  query = "begin transaction; delete from teams; delete from sqlite_sequence "
          "where name=\"teams\";";
  for (unsigned int i = 0; i < clubs.size(); i++) {
    int competitionID =
        competitionIDs.find(clubs.at(i).country + clubs.at(i).competition)
            ->second;
    std::string competitionFilename =
        RenameLeague(namedb, clubs.at(i).competition, "targetname");
    std::string strippedCompetitionFilename = StripString(competitionFilename);
    std::transform(strippedCompetitionFilename.begin(),
                   strippedCompetitionFilename.end(),
                   strippedCompetitionFilename.begin(), ::tolower);
    std::string clubName =
        RenameClub(namedb, clubs.at(i).name, "faketargetname");
    std::string strippedClubName = StripString(clubName);
    std::string clubFilename =
        RenameClub(namedb, clubs.at(i).name, "targetname");
    ...
    printf("adding team: orig %s - renamed %s - stripped %s\n",
           clubs.at(i).name.c_str(), clubName.c_str(),
           strippedClubName.c_str());
    ...
    // query += "insert into teams (id, league_id, name, logo_url, kit_url,
    // formation) values (" + int_to_str(clubs.at(i).id) + ", " +
    // int_to_str(competitionID) + ", \"" + clubs.at(i).name + "\",
    // \"images_teams/" + strippedCompetition + "/" + strippedTeam +
    // "_logo.png\", \"images_teams/" + strippedCompetition + "/" + strippedTeam
    // + "_kit.png\", \"" + defaultFormation + "\");";
    query += "insert into teams (id, league_id, name, logo_url, kit_url, "
             "formation_xml, formation_factory_xml, tactics_xml, "
             "tactics_factory_xml, shortname, color1, color2) values (" +
             int_to_str(clubs.at(i).id) + ", " + int_to_str(competitionID) +
             ", \"" + clubName + "\", \"images_teams/" +
             strippedCompetitionFilename + "/" + strippedClubFilename +
             "_logo.png\", \"images_teams/" + strippedCompetitionFilename +
             "/" + strippedClubFilename + "\", \"" + clubData.formation_xml +
             "\", \"" + clubData.formation_xml + "\", \"" +
             clubData.tactics_xml + "\", \"" + clubData.tactics_xml + "\", \"" +
             clubData.shortName + "\", \"" + clubData.color1 + "\", \"" +
             clubData.color2 + "\");";
    clubIDs.insert(
        std::pair<std::string, int>(clubs.at(i).name, clubs.at(i).id));
  }
  query += "commit;";
  result = GetDB()->Query(query);
  delete result;
  ...
  for (unsigned int i = 1; i < players.size(); i++) { // (skip header)
    ...
    tokens.at(11).erase(
        std::remove(tokens.at(11).begin(), tokens.at(11).end(), ','),
        tokens.at(11).end());
    ...
    if (club != clubIDs.end() && player.value > 0 && player.age >= 15 &&
        player.age <=
            40) { // only add player if he's with a club that's in the database
      ...
      // XX float averageStat = GetAverageStatFromValue(player.age,
      // player.value);
      player.averageStat = NormalizedClamp(
          atof(tokens.at(13).substr(0, 5).c_str()) * 1.0f - 10, 0,
          100); // don't go full range - the hypothetical best player ever
                // doesn't exist in real life (yet? ;))
      player.averageStartStat = NormalizedClamp(
          atof(tokens.at(14).substr(0, 5).c_str()) * 1.0f - 10, 0, 100);
      // youngsters are underrated in FM db
      player.averageStat *=
          1.0f -
          curve(NormalizedClamp(abs(player.age - 18), 0, 10), 1.0f) * 0.1f;
  ...
  query = "begin transaction; delete from players; delete from sqlite_sequence "
          "where name=\"players\";";
  for (unsigned int i = 0; i < importedPlayers.size(); i++) {

    PlayerImport &player = importedPlayers.at(i);

    // XX float averageStatAtCurrentAge =
    // averageStatPerAge.find(player.age)->second; XX player.averageStartStat =
    // (player.averageStat / averageStatAtCurrentAge) * averageStatAt15; float
    // averageStartStat = ((player.averageStat / averageStatAtCurrentAge * 0.7)
    // + player.averageStat * 0.3) * averageStatAt15; float averageStartStat =
    // player.averageStat * averageStatAt15; printf("stat: %f (%f), age: %i
    // (%s)\n", player.averageStartStat, player.averageStat, player.age,
    // player.lastName.c_str());
    ...
    for (unsigned int j = 0; j < player.profileStats.size(); j++) {
      ...
      //player.profileStats.at(j).value =
    GetIndividualStat(player.profileStats.at(j).value)
    ...
    */
    ...
    std::string lastName;
    // deprecated RenamePlayer(namedb, player.firstName, player.lastName,
    // firstName, lastName);
    int skinColor;
    ...
    GetPlayerData(namedb, player.firstName, player.lastName, firstName,
                  lastName, skinColor, hairStyle, hairColor, height, weight,
                  formationOrder, baseStatOffset, customProfileXML);
    if (customProfileXML.size() > 0)
      statsString = customProfileXML;
    player.averageStartStat =
        clamp(player.averageStartStat + baseStatOffset, 0.01f, 1.0f);
    player.averageStat =
        clamp(player.averageStat + baseStatOffset, 0.01f, 1.0f);

    bestYoungPlayersMap.insert(
        std::pair<float, int>(player.averageStartStat, i));
    ...
    query +=
        (std::string)("insert into players (id, firstname, lastname, team_id, "
                      "role, formationorder, age, base_stat, profile_xml, "
                      "skincolor, hairstyle, haircolor, height, weight, "
                      "nationalteam_id, nationalteamformationorder) values ") +
        (std::string)(
            "(" + int_to_str(player.id) + ", \"" + firstName + "\", \"" +
            lastName + "\", " + int_to_str(player.clubID) + ", \"" +
            player.position + "\", " + int_to_str(formationOrder) + ", " +
            int_to_str(player.age) + ", " +
            real_to_str(player.averageStartStat) + ", \"" + statsString +
            "\", " + int_to_str(skinColor) + ", \"" + hairStyle + "\", \"" +
            hairColor + "\", " + real_to_str(height) + ", " +
            real_to_str(weight) + ", -1, -1);");
  }
  query += "commit;";
  result = GetDB()->Query(query);
  delete result;
  ...
  std::map<float, int>::iterator bestYoungPlayersIter =
      bestYoungPlayersMap.begin();
  for (int i = 0; i < 30; i++) {
  ...
  // calculate order of players per club - to get players on their right
  // positions, and the best players in the base eleven
  ...
  for (unsigned int c = 0; c < clubs.size(); c++) {
    ...
    if (keepers.size() > 0) {
      ...
      // Log(e_Warning, "MainMenuPage", "GoImportDB", "Team " + clubs.at(c).name
      // + " has no keeper!");
    }

    // bestestest players on top
    std::vector<PlayerImport>::reverse_iterator rend =
        clubs.at(c).players.rend();
    std::vector<PlayerImport>::reverse_iterator rbegin =
        clubs.at(c).players.rbegin();
    if (keepers.size() > 0)
    ...
    */
    ...
    for (unsigned int p = 0; p < keepPlayers; p++) {
      ...
      query += (std::string)(
          "update players set formationorder = " + int_to_str(p) +
      ...
      // fakefirstname, fakelastname, formationorder) values (\"" + firstname +
      // "\", \"" + lastname + "\", \"" + firstname + "\", \"" + lastname + "\",
      // " + int_to_str(p) + ");";
      std::string namequery =
          "insert into playernames (firstname, lastname, fakefirstname, "
          "fakelastname, formationorder) select \"" +
          firstname + "\", \"" + lastname + "\", \"" + firstname + "\", \"" +
          lastname + "\", " + int_to_str(p) +
          " WHERE NOT EXISTS(SELECT 1 FROM playernames WHERE (firstname = \"" +
          firstname + "\" AND lastname = \"" + lastname +
          "\") OR (fakefirstname = \"" + firstname +
...
void MainMenuPage::Import_ProcessAgeValueStats() {
  ...
  if (cachedVersion) {
    ...
    std::map<int, float> tempAverageStatPerAge; // needs to be smoothed later on
into the non-temp version int lowestOverallValue = 10000000; int
highestOverallValue = 0; for (int age = 15; age <= 40; age++) { std::map < int,
std::vector<int> >::iterator iter = ageValues.find(age); if (iter !=
ageValues.end()) { int totalValues = 0; int cumulativeValue = 0; int lowestValue
= 10000000; int highestValue = 0; std::vector<int> &values = iter->second; for
(unsigned int i = 0; i < values.size(); i++) { cumulativeValue += values.at(i);
          totalValues++;
          if (values.at(i) < lowestValue) lowestValue = values.at(i);
          if (values.at(i) > highestValue) highestValue = values.at(i);
          if (values.at(i) < lowestOverallValue) lowestOverallValue =
values.at(i); if (values.at(i) > highestOverallValue) highestOverallValue =
values.at(i);
        }
        ...
        printf("age: %i, players: %i, average value: %i, median value: %i,
lowest/highest: %i/%i\n", age, totalValues, average, median, lowestValue,
highestValue);

        // save
        //float averageStat = GetAverageStatFromValue(age, average);
        float averageStat = GetAverageStatFromValue(age, median * 0.9 + average
* 0.1); tempAverageStatPerAge.insert(std::pair<int, float>(age, averageStat));
      }
    }

    // average stats per age over multiple ages to smooth out the graph and to
fix ages where few players are available (>~40 year old) averageStatPerAge =
tempAverageStatPerAge; std::map<int, float>::iterator tmpIter =
tempAverageStatPerAge.begin(); std::map<int, float>::iterator iter =
averageStatPerAge.begin(); while (tmpIter != tempAverageStatPerAge.end()) { int
amount = 1; // divide by how many? (for average) float average =
tmpIter->second; if (tmpIter != tempAverageStatPerAge.begin()) { // add previous
        tmpIter--;
