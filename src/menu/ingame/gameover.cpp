...
GameOverPage::GameOverPage(Gui2WindowManager *windowManager,
  ...
  Gui2Image *bg1 =
      new Gui2Image(windowManager, "image_gameover_bg", 10, 10, 80, 80);
  this->AddView(bg1);
  ...
  Gui2Caption *header = new Gui2Caption(
      windowManager, "caption_gameover_header", 0, 15, 80, 4, scoreStr);
  header->SetPosition(50 - header->GetTextWidthPercent() / 2, 15);
  this->AddView(header);
  header->Show();

  buttonOkay = new Gui2Button(windowManager, "button_gameover_ok", 40, 82, 20,
                              3, "well then");
  ...
  Gui2Grid *grid =
      new Gui2Grid(windowManager, "grid_gameover_stats", 15, 25, 70, 50);

  grid->AddView(
      new Gui2Caption(
          windowManager, "caption_possession_t1", 0, 0, 25, 3,
          int_to_str(round(possession1 / (possession1 + possession2) * 100)) +
              "%"),
      0, 0);
  grid->AddView(new Gui2Caption(windowManager, "caption_possession_header", 0,
                                0, 35, 3, "possession"),
                0, 1);
  grid->AddView(
      new Gui2Caption(
          windowManager, "caption_possession_t2", 0, 0, 10, 3,
          int_to_str(round(possession2 / (possession1 + possession2) * 100)) +
              "%"),
      0, 2);

  grid->AddView(new Gui2Caption(windowManager, "caption_shots_t1", 0, 0, 25, 3,
                                int_to_str(shots1)),
                1, 0);
  grid->AddView(new Gui2Caption(windowManager, "caption_shots_header", 0, 0, 35,
                                3, "shots"),
                1, 1);
  grid->AddView(new Gui2Caption(windowManager, "caption_shots_t2", 0, 0, 10, 3,
                                int_to_str(shots2)),
                1, 2);
...
void GameOverPage::GoRematch() {
  ...
  windowManager->GetPageFactory()->CreatePage((int)e_PageID_Game, properties,
                                              0);
  delete this;
}
