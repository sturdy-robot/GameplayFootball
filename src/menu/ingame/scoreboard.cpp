...
Gui2ScoreBoard::Gui2ScoreBoard(Gui2WindowManager *windowManager, Match *match)
  ...
  Gui2Image *bg = new Gui2Image(windowManager, "image_scoreboard_bg", 0, 0,
                                xOffset[8] + 1, height_percent);
  bg->LoadImage("media/menu/scoreboard_bg.png");
  this->AddView(bg);
  bg->Show();

  leagueLogo = new Gui2Image(
      windowManager, "game_scoreboard_leaguelogo", xOffset[0], 0,
  ...
  timeCaption = new Gui2Caption(windowManager, "game_scoreboard_timecaption",
                                xOffset[1] + content_xOffset, 0, 5,
                                height_percent * 0.9f, "0:00");
  teamNameCaption[0] =
      new Gui2Caption(windowManager, "game_scoreboard_team1name",
                      xOffset[3] + content_xOffset, 0, 5, height_percent * 0.9f,
                      match->GetTeam(0)->GetTeamData()->GetShortName());
  teamNameCaption[1] =
      new Gui2Caption(windowManager, "game_scoreboard_team2name",
                      xOffset[6] + content_xOffset, 0, 5, height_percent * 0.9f,
                      match->GetTeam(1)->GetTeamData()->GetShortName());
  goalCountCaption[0] = new Gui2Caption(
      windowManager, "game_scoreboard_team1goals", xOffset[4] + content_xOffset,
      0, 5, height_percent * 0.9f, "0");
  goalCountCaption[1] = new Gui2Caption(
      windowManager, "game_scoreboard_team2goals", xOffset[7] + content_xOffset,
      0, 5, height_percent * 0.9f, "0");
  ...
  tvLogo = new Gui2Image(
      windowManager, "game_scoreboard_tvlogo",
      width_percent - (height_percent * 2.0f) / windowManager->GetAspectRatio(),
      0, (height_percent * 2.0f) / windowManager->GetAspectRatio(),
      height_percent);
  this->AddView(tvLogo);
  ...
  teamLogo[0] = new Gui2Image(
      windowManager, "game_scoreboard_team1logo", xOffset[2], 0,
      height_percent / windowManager->GetAspectRatio(), height_percent);
  this->AddView(teamLogo[0]);
  teamLogo[0]->LoadImage(match->GetTeam(0)->GetTeamData()->GetLogoUrl());
  teamLogo[0]->Show();

  teamLogo[1] = new Gui2Image(
      windowManager, "game_scoreboard_team2logo", xOffset[5], 0,
      height_percent / windowManager->GetAspectRatio(), height_percent);
