...
GamePage::GamePage(Gui2WindowManager *windowManager,
                   const Gui2PageData &pageData)
    : Gui2Page(windowManager, pageData), match(0) {

  Gui2Caption *betaSign =
      new Gui2Caption(windowManager, "caption_betasign", 0, 0, 0, 2,
                      "gameplay football public beta 2 v0.2");
...
GamePage::~GamePage() {
  ...
  // problem is, this function may be called outside of gametask's or match's
  // lifetime.
  ...
  if (match) {
    ...
    match->sig_OnMatchPhaseChange.disconnect(
        boost::bind(&GamePage::GoMatchPhasePage, this));
    match->sig_OnShortReplayMoment.disconnect(
        boost::bind(&GamePage::GoShortReplayPage, this));
    match->sig_OnExtendedReplayMoment.disconnect(
        boost::bind(&GamePage::GoExtendedReplayPage, this));
    match->sig_OnGameOver.disconnect(
        boost::bind(&GamePage::GoGameOverPage, this));
...
void GamePage::Process() {
  ...
  if (!match) {
    ...
    if (GetGameTask()->GetMatch() != 0) {
      ...
      match->sig_OnMatchPhaseChange.connect(
          boost::bind(&GamePage::GoMatchPhasePage, this));
      match->sig_OnShortReplayMoment.connect(
          boost::bind(&GamePage::GoShortReplayPage, this));
      match->sig_OnExtendedReplayMoment.connect(
          boost::bind(&GamePage::GoExtendedReplayPage, this));
      match->sig_OnGameOver.connect(
          boost::bind(&GamePage::GoGameOverPage, this));
    }
    GetGameTask()->matchLifetimeMutex.unlock();
  }
...
void GamePage::GoExtendedReplayPage() {
  ...
  ReplayPage *replayPage =
      static_cast<ReplayPage *>(windowManager->GetPageFactory()->CreatePage(
          (int)e_PageID_Replay, properties, 0));
...
void GamePage::GoGameOverPage() { CreatePage((int)e_PageID_GameOver); }
...
void GamePage::ProcessKeyboardEvent(KeyboardEvent *event) {
  ...
  if (event->GetKeyOnce(SDLK_ESCAPE)) {
    ...
    int teamID = 0;
    const std::vector<SideSelection> sides =
        GetMenuTask()->GetControllerSetup();
...
void GamePage::ProcessJoystickEvent(JoystickEvent *event) {
  ...
  // oof, the problem is that we are using a GUI joystick event to find out what
  // ingame HID controller pressed <start>. I think we need a new system in
  // which the game doesn't use its own input system, but uses the GUI one. for
  // now, this hax will have to do
  ...
  for (unsigned int c = 0; c < controllers.size(); c++) {
    ...
    if (controllers.at(c)->GetDeviceType() == e_HIDeviceType_Gamepad) {
      ...
      int joyID = gamepad->GetGamepadID(); // these should be the same IDs the
                                           // GUI system uses as joyID

      if (event->GetButton(
              joyID, gamepad->GetControllerMapping(gamepad->GetFunctionMapping(
