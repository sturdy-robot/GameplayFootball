...
public:
  Gui2TacticsDebug(Gui2WindowManager *windowManager, const std::string &name,
  ...
  virtual unsigned int AddEntry(const std::string &caption,
                                const Vector3 &color1, const Vector3 &color2,
                                const Vector3 &color3);
  virtual void SetValue(unsigned int entryID, int typeID, int teamID,
                        float value);
