...
ReplayPage::ReplayPage(Gui2WindowManager *windowManager,
  ...
  signed long tmp =
      ...
      match->GetReplaySize_ms(); // must be signed for negative numbers
...
void ReplayPage::OnClose() {
  ...
  if (stayInReplay)
    match->Pause(
        false); // todo: handle gracefully instead of using stayInReplay :p only
                // unpause when started from gamepage instead of ingame page
}
...
void ReplayPage::ProcessKeyboardEvent(KeyboardEvent *event) {
  ...
  bool button2 = false;
  if (event->GetKeyOnce(
          keyboard->GetFunctionMapping(e_ButtonFunction_ShortPass)))
    button1 = true;
  if (event->GetKeyOnce(
          keyboard->GetFunctionMapping(e_ButtonFunction_HighPass)))
    button2 = true;

  Vector3 direction;
  if (event->GetKeyContinuous(
          keyboard->GetFunctionMapping(e_ButtonFunction_Left)))
    direction.coords[0] += -0.5f;
  if (event->GetKeyContinuous(
          keyboard->GetFunctionMapping(e_ButtonFunction_Right)))
    direction.coords[0] += 0.5f;
  if (event->GetKeyContinuous(
          keyboard->GetFunctionMapping(e_ButtonFunction_Up)))
    direction.coords[1] += -0.5f;
  if (event->GetKeyContinuous(
          keyboard->GetFunctionMapping(e_ButtonFunction_Down)))
    direction.coords[1] += 0.5f;
...
void ReplayPage::ProcessJoystickEvent(JoystickEvent *event) {
  ...
  HIDGamepad *gamepad = static_cast<HIDGamepad *>(controllers.at(
      controllerID +
      1)); // todo: check if we can be sure this is actually a joystick/gamepad
  bool button1 =
      event->GetButton(
          0, gamepad->GetControllerMapping(
                 gamepad->GetFunctionMapping(e_ButtonFunction_LongPass))) ||
      event->GetButton(
          0, gamepad->GetControllerMapping(gamepad->GetFunctionMapping(
                 e_ButtonFunction_ShortPass))); // need 2 options because maybe
                                                // the first is set to gui's
                                                // 'escape' function
  bool button2 =
      event->GetButton(
          0, gamepad->GetControllerMapping(
      ...
      event->GetButton(
          0, gamepad->GetControllerMapping(gamepad->GetFunctionMapping(
                 e_ButtonFunction_Shot))); // need 2 options because maybe the
                                           // first is set to gui's 'escape'
                                           // function
  ...
  if (fabs(direction.coords[0]) < deadzone) {
    ...
    direction.coords[0] =
        pow((fabs(direction.coords[0]) - deadzone) * (1.0f / (1.0f - deadzone)),
            2.0f) *
  ...
  if (fabs(direction.coords[1]) < deadzone) {
    ...
    direction.coords[1] =
        pow((fabs(direction.coords[1]) - deadzone) * (1.0f / (1.0f - deadzone)),
            4.0f) *
...
void ReplayPage::ProcessInput(const Vector3 &direction, bool button1,
                              bool button2) {
