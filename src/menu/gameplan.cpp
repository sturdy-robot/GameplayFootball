...
GamePlanPage::GamePlanPage(Gui2WindowManager *windowManager,
  ...
  Gui2Image *bg1 =
      new Gui2Image(windowManager, "gameplan_image_bg", xOffset, 15, 35, 72);
  this->AddView(bg1);
  bg1->LoadImage("media/menu/backgrounds/black.png");
  bg1->Show();

  Gui2Caption *header =
      new Gui2Caption(windowManager, "gameplan_header", xOffset, 11, 35, 3,
                      "Team " + int_to_str(teamID + 1) + " game plan");
  grid = new Gui2Grid(windowManager, "gameplan_grid", xOffset, 15, 0, 0);
  gridNav =
      new Gui2Grid(windowManager, "gameplan_grid_navigation", xOffset, 0, 0, 0);

  map = new Gui2PlanMap(windowManager, "gameplan_planmap", 0, 0, 35, 28,
                        teamData);
  buttonLineup = new Gui2Button(windowManager, "gameplan_button_lineup", 0, 0,
                                34, 3, "Line-up");
  buttonTactics = new Gui2Button(windowManager, "gameplan_button_tactics", 0, 0,
                                 34, 3, "Tactics");
  Gui2Button *buttonFormation = new Gui2Button(
      windowManager, "gameplan_button_formation", 0, 0, 34, 3, "Formation");

  buttonLineup->sig_OnClick.connect(
      boost::bind(&GamePlanPage::GoLineupMenu, this));
  buttonTactics->sig_OnClick.connect(
      boost::bind(&GamePlanPage::GoTacticsMenu, this));
  ...
  if (UpdateNonImportableDB()) {
    ...
    if (!dbSuccess)
      Log(e_FatalError, "MainMenuPage", "GoImportDB",
...
void GamePlanPage::Deactivate() { grid->RemoveView(1, 0); }
...
void GamePlanPage::GoLineupMenu() {
  ...
  lineupMenu =
      new GamePlanSubMenu(windowManager, buttonLineup, grid, "lineup_submenu");
  ...
  for (unsigned int i = 0; i < playerData.size(); i++) {
    ...
    Gui2Button *button = lineupMenu->AddButton(
        "playerbutton_id" + int_to_str(playerData.at(i)->GetDatabaseID()),
        playerData.at(i)->GetLastName(), i, 0, color);
    button->sig_OnClick.connect(
        boost::bind(&GamePlanPage::LineupMenuOnClick, this, _1));
  ...
  if (selected) {
    ...
    int id1 =
        atoi(selected->GetName()
                 .substr(selected->GetName().rfind("id") + 2, std::string::npos)
                 .c_str());
    int id2 =
        atoi(button->GetName()
                 .substr(button->GetName().rfind("id") + 2, std::string::npos)
                 .c_str());
  ...
  if (UpdateNonImportableDB()) {
    ...
    for (unsigned int i = 0; i < allButtons.size(); i++) {
      ...
      DatabaseResult *result = namedb->Query(
          "select id from playernames where fakefirstname = \"" +
      ...
      if (result->data.size() > 0) {
        ...
        delete result;
        result = namedb->Query("update playernames set formationorder = " +
                               int_to_str(formationorder) + " where id = " +
                               int_to_str(playerDatabaseID) + ";");
        delete result;
      } else { // player does not yet exist in namedb
        if (Verbose())
          printf("WARNING: player does not exist in namedb: %s %s\n",
                 playerData->GetFirstName().c_str(),
                 playerData->GetLastName().c_str());
        delete result;
      }
    }
  }
...
void GamePlanPage::GoTacticsMenu() {
  ...
  tacticsSliders
      .clear(); // could still be here from previous tactics subpage visit.
                // should clear this on leaving page but we have no mechanism to
                // do so (<- todo, could use onclose probably)

  tacticsMenu = new GamePlanSubMenu(windowManager, buttonTactics, grid,
                                    "tactics_submenu");
  tacticsMenu->sig_OnClose.connect(
      boost::bind(&GamePlanPage::SaveTactics, this));
  tacticsMenu->sig_OnClose.connect(
      boost::bind(&GamePlanPage::Reactivate, this));
  ...
  while (iter != userPropMap->end()) {
    ...
    slider.widget = tacticsMenu->AddSlider(
        ...
        teamData->GetTactics().humanReadableNames.Get(slider.tacticName.c_str(),
                                                      slider.tacticName),
        i, 0);
    slider.widget->AddHelperValue(
        Vector3(80, 80, 250), "factory default for this team",
    ...
    slider.widget->sig_OnChange.connect(
        boost::bind(&GamePlanPage::TacticsMenuOnChange, this, _1, slider.id));
    if (i == 0)
...
void GamePlanPage::SaveTactics() {
  ...
  if (UpdateNonImportableDB()) {
    ...
    DatabaseResult *result =
        namedb->Query("select id from clubnames where faketargetname = \"" +
...
void GamePlanPage::TacticsMenuOnChange(Gui2Slider *slider, int id) {
  ...
  userProps.Set(tacticsSliders.at(id).tacticName.c_str(),
                tacticsSliders.at(id).widget->GetValue());
}
