...
public:
  GamePlanSubMenu(Gui2WindowManager *windowManager, Gui2View *parentFocus,
                  Gui2Grid *mainGrid, const std::string &name);
  virtual ~GamePlanSubMenu();

  void OnClose();

  Gui2Button *AddButton(const std::string &buttonName,
                        const std::string &buttonCaption, int row, int column,
                        Vector3 color);
  Gui2Slider *AddSlider(const std::string &sliderName,
