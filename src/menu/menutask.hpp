...
// this requires some bigger changes, so stick with this imperfect system for
// the time being
...
public:
  MenuTask(float aspectRatio, float margin, TTF_Font *defaultFont,
  ...
  const std::vector<SideSelection> GetControllerSetup() {
    return queuedFixture.GetData().sides;
  }
  ...
  } // hint: this lock is useless, since we are returning the pointer and not a
    // copy
...
protected:
  ...
  Lockable<QueuedFixture>
      queuedFixture; // todo: we can probably unlock this stuff
};
