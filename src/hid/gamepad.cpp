...
HIDGamepad::HIDGamepad(int gamepadID) : gamepadID(gamepadID) {
  ...
  identifier = std::string(SDL_JoystickNameForIndex(gamepadID)) + " #" +
               int_to_str(gamepadID);
...
void HIDGamepad::LoadConfig() {
  ...
  for (int i = 0; i < _JOYSTICK_MAXAXES; i++) {
    float min =
        GetConfiguration()->GetReal(("input_gamepad_" + GetIdentifier() +
  ...
  for (int i = 0; i < e_ControllerButton_Size; i++) {
    ...
    controllerMapping[i] = GetConfiguration()->GetInt(
        ("input_gamepad_" + GetIdentifier() + "_" + int_to_str(i)).c_str(),
        defaultButton);
  }
  ...
  for (int i = 0; i < e_ButtonFunction_Size; i++) {
    ...
    functionMapping[i] = (e_ControllerButton)GetConfiguration()->GetInt(
        ("input_gamepad_" + GetIdentifier() + "_mapping_" + int_to_str(i))
            .c_str(),
        defaultMapping);
  }
}
  ...
  for (int i = 0; i < e_ControllerButton_Size; i++) {
    ...
    if (buttonID >= 0) { // button
      controllerButtonState[i] =
          UserEventManager::GetInstance().GetJoyButtonState(gamepadID, buttonID)
              ? 1.0
              : 0.0;
    } else { // axis
      // decode
      ...
      if (axisID < 2)
        deadzone = false;
      float value = UserEventManager::GetInstance().GetJoystickAxis(
          gamepadID, axisID, deadzone);
