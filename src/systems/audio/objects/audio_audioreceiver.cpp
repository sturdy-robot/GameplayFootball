...
AudioAudioReceiver::AudioAudioReceiver(AudioScene *audioScene)
    : AudioObject(audioScene) {}

AudioAudioReceiver::~AudioAudioReceiver() {}

boost::intrusive_ptr<Interpreter>
AudioAudioReceiver::GetInterpreter(e_ObjectType objectType) {
  if (objectType == e_ObjectType_AudioReceiver) {
    boost::intrusive_ptr<AudioAudioReceiver_AudioReceiverInterpreter>
        audioReceiverInterpreter(
...
Vector3 AudioAudioReceiver::GetPosition() const { return position; }

void AudioAudioReceiver::SetRotation(const Quaternion &newRotation) {
  rotation = newRotation;
}

Quaternion AudioAudioReceiver::GetRotation() const { return rotation; }

AudioAudioReceiver_AudioReceiverInterpreter::
...
void AudioAudioReceiver_AudioReceiverInterpreter::OnSpatialChange(
