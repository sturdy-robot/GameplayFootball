...
AudioSound::GetInterpreter(e_ObjectType objectType) {
...
Vector3 AudioSound::GetPosition() const { return position; }

void AudioSound::SetRadius(float radius) { this->radius = radius; }

float AudioSound::GetRadius() const { return radius; }

AudioSound_SoundInterpreter::AudioSound_SoundInterpreter(AudioSound *caller)
    : caller(caller) {}

void AudioSound_SoundInterpreter::OnLoad(
  ...
  AudioRenderer *renderer =
      caller->GetAudioScene()->GetAudioSystem()->GetAudioRenderer();
...
void AudioSound_SoundInterpreter::UpdateParams() {
  ...
  boost::intrusive_ptr<AudioRendererMessage_ConfigAudioSoundBuffer> updateMsg(
      new AudioRendererMessage_ConfigAudioSoundBuffer(
          caller->audioSoundBuffer->GetResource()->GetID(), caller->GetGain(),
          caller->GetPitch(), caller->GetLoop()));
  caller->audioSoundBuffer->resourceMutex.unlock();
