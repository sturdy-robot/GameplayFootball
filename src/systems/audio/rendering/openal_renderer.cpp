...
bool OpenALRenderer::CreateContext() {
  ...
  SetListenerParameters(Vector3(0), Vector3(0),
...
int OpenALRenderer::CreateAudioSoundBuffer(const WavData *wavData) {
  ...
  if (!format) {
    Log(e_FatalError, "OpenALRenderer", "CreateAudioSoundBuffer",
        "Incompatible format");
    return -1;
  }
  ...
  alBufferData(buffer_id, format, wavData->data, wavData->size,
               wavData->frequency);
  ...
  Vector3 velocity;
  velocity.Set(0, 0, 0);
  ALfloat sourcePos[] = {position.coords[0], position.coords[1],
                         position.coords[2]};
  ALfloat sourceVel[] = {velocity.coords[0], velocity.coords[1],
                         velocity.coords[2]};
...
void OpenALRenderer::DeleteAudioSoundBuffer(int audioSoundBufferID) {
  ...
  std::map<ALuint, ALuint>::iterator iter =
      bufferSourceMapping.find(audioSoundBufferID);
  if (iter == bufferSourceMapping.end())
    Log(e_FatalError, "OpenALRenderer", "DeleteAudioSoundBuffer",
        "Source not found for buffer");
...
void OpenALRenderer::SetListenerParameters(const Vector3 &position,
                                           const Vector3 &velocity,
                                           const Quaternion &orientation) {
  ALfloat listenerPos[] = {position.coords[0], position.coords[1],
                           position.coords[2]};
  ALfloat listenerVel[] = {velocity.coords[0], velocity.coords[1],
                           velocity.coords[2]};
  Matrix3 ori;
...
void OpenALRenderer::SetSourceParameter(
    int audioSoundBufferID, e_AudioRenderer_SourceParameter parameter,
    float value) {
...
void OpenALRenderer::operator()() {
  Log(e_Notice, "OpenALRenderer", "operator()()",
      "Starting OpenALRenderer thread");
  ...
  Log(e_Notice, "OpenALRenderer", "operator()()",
      "Shutting down OpenALRenderer thread");
