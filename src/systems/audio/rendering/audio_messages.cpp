...
bool AudioRendererMessage_DeleteAudioSoundBuffer::Execute(void *caller) {
  ...
  AudioRenderer *renderer = static_cast<AudioRenderer *>(caller);
  renderer->SetSourceParameter(audioSoundBufferID,
                               e_AudioRenderer_SourceParameter_Gain, 0);
...
bool AudioRendererMessage_ConfigAudioSoundBuffer::Execute(void *caller) {
  ...
  renderer->SetSourceParameter(audioSoundBufferID,
                               e_AudioRenderer_SourceParameter_Gain, gain);
  renderer->SetSourceParameter(audioSoundBufferID,
                               e_AudioRenderer_SourceParameter_Pitch, pitch);
  renderer->SetSourceParameter(
      audioSoundBufferID, e_AudioRenderer_SourceParameter_Loop, loop ? 1 : 0);
