...
VertexBuffer::VertexBuffer()
    : vertices(0), verticesDataSize(0), vertexCount(0), dynamicBuffer(false),
      sizeChanged(false) {
...
VertexBuffer::~VertexBuffer() {
  ...
  if (vertexBufferID.bufferID != -1) {
    boost::intrusive_ptr<Renderer3DMessage_DeleteVertexBuffer>
        deleteVertexBuffer(
...
void VertexBuffer::SetTriangleMesh(float *vertices,
...
VertexBufferID VertexBuffer::CreateOrUpdateVertexBuffer(Renderer3D *renderer3D,
  ...
  if (vertexBufferID.bufferID == -1) {
    ...
    assert(vertices != 0);
    boost::intrusive_ptr<Renderer3DMessage_CreateVertexBuffer>
        createVertexBuffer(new Renderer3DMessage_CreateVertexBuffer(
    ...
    assert(vertices != 0);
    boost::intrusive_ptr<Renderer3DMessage_UpdateVertexBuffer>
        updateVertexBuffer(new Renderer3DMessage_UpdateVertexBuffer(
            vertexBufferID, vertices, verticesDataSize));
    renderer3D->messageQueue.PushMessage(updateVertexBuffer);
    updateVertexBuffer->Wait(); // todo: make the vertices* stuff lockable so we
...
float *VertexBuffer::GetTriangleMesh() { return vertices; }

int VertexBuffer::GetID() { return vertexBufferID.bufferID; }

int VertexBuffer::GetVaoID() { return vertexBufferID.vertexArrayID; }

int VertexBuffer::GetElementID() { return vertexBufferID.elementArrayID; }

int VertexBuffer::GetVertexCount() { return vertexCount; }

int VertexBuffer::GetVerticesDataSize() { return verticesDataSize; }
