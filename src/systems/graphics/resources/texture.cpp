...
int Texture::CreateTexture(e_InternalPixelFormat internalPixelFormat,
  ...
  boost::intrusive_ptr<Renderer3DMessage_CreateTexture> createTexture(
      new Renderer3DMessage_CreateTexture(
          internalPixelFormat, pixelFormat, width, height, alpha, repeat,
          mipmaps, filter, compareDepth)); // false == multisample
...
void Texture::ResizeTexture(SDL_Surface *image,
  ...
  boost::intrusive_ptr<Renderer3DMessage_ResizeTexture> resizeTexture(
      new Renderer3DMessage_ResizeTexture(textureID, image, internalPixelFormat,
                                          pixelFormat, _alpha, mipmaps));
...
void Texture::SetID(int value) { textureID = value; }

int Texture::GetID() { return textureID; }
