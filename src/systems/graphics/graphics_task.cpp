...
GraphicsTask::GraphicsTask(GraphicsSystem *system)
    : ISystemTask(), graphicsSystem(system) {
...
void GraphicsTask::operator()() {
  Log(e_Notice, "GraphicsTask", "operator()()",
      "Starting GraphicsSystemTask thread");
  ...
  Log(e_Notice, "GraphicsTask", "operator()()",
      "Shutting down GraphicsSystemTask thread");
...
void GraphicsTask::GetPhase() {
  ...
  boost::shared_ptr<IScene> scene =
      SceneManager::GetInstance().GetScene("scene3D", success);
  if (success) {
    std::list<boost::intrusive_ptr<Camera>> cameras;
    boost::static_pointer_cast<Scene3D>(scene)->GetObjects<Camera>(
        e_ObjectType_Camera, cameras);
    ...
    while (cameraIter != cameras.end()) {
      ...
      if ((*cameraIter)->IsEnabled()) {
        enqueueView.push_back(
            boost::intrusive_ptr<GraphicsTaskCommand_EnqueueView>(
                new GraphicsTaskCommand_EnqueueView((*cameraIter),
...
void GraphicsTask::ProcessPhase() {
  ...
  boost::intrusive_ptr<Renderer3DMessage_RenderOverlay2D> renderOverlay2D(
      new Renderer3DMessage_RenderOverlay2D(
          graphicsSystem->GetOverlay2DQueue()));
...
void GraphicsTask::PutPhase() {
  ...
  // opengl thread has only one message processor, so don't wait for swap (might
  // take long to vsync). messages will be processed serially anyway todo: maybe
  // still should be on. have to check. seems that the lazy wait causes time
  // problems: the next frame is drawn for situation time = x, but since the gfx
  // thread has to wait for the vsync, the situation drawn should be that of
  // time = x + vsync_delay. this makes for choppy gfx. update: with high enough
  // fps, this might be ok update: last frame time thing obviously doesn't work
  // when only setting lastswaptime at next frame time, so waiting here for
  // retrace again.
...
bool GraphicsTaskCommand_EnqueueView::Execute(void *caller) {
  ...
  if (success) {
    // test
    // camera->SetPosition(Vector3(sin((float)EnvironmentManager::GetInstance().GetTime_ms()
    // * 0.001f) * 60, 0, 0), true);
    ...
    plane.Set(cameraPos,
              cameraRot *
                  Vector3(0, (3.6f * wideScreenMultiplier) / (fov / 24.0f), -1)
                      .GetNormalized());
    bounding.push_back(plane);
    plane.Set(cameraPos,
              cameraRot *
    ...
    plane.Set(cameraPos,
              cameraRot *
                  Vector3(-2.4f / (fov / 24.0f), 0, -1).GetNormalized());
    bounding.push_back(plane);
    ...
    boost::static_pointer_cast<Scene3D>(scene)->GetObjects<Light>(e_ObjectType_Light,
    visibleLights, bounding);
    //printf("lights.size: %i\n", visibleLights.size());

    std::list < boost::intrusive_ptr<Skybox> > skyboxes;
    boost::static_pointer_cast<Scene3D>(scene)->GetObjects<Skybox>(e_ObjectType_Skybox,
    skyboxes);
    ...
    */
    ...
    boost::static_pointer_cast<Scene3D>(scene)->GetObjects(visibleObjects,
                                                           bounding);
    ...
    while (objectIter != visibleObjects.end()) {
      if ((*objectIter)->IsEnabled()) {
        ...
        if (objectType == e_ObjectType_Geometry)
          ...
          visibleLights.push_back(
              boost::static_pointer_cast<Light>(*objectIter));
    ...
    if (shadowSkipFrameCounter == 0) {
      std::deque<boost::intrusive_ptr<Light>>::iterator lightIter =
          visibleLights.begin();
...
void GraphicsTaskCommand_EnqueueView::EnqueueShadowMap(
  ...
  if (success) {
    ...
    plane.Set(Vector3(60, 0, 0), Vector3(-1, 0, 0.3).GetNormalized());
    bounding.push_back(plane);
    boost::static_pointer_cast<Scene3D>(scene)->GetObjects<Geometry>(
        e_ObjectType_Geometry, visibleGeometry, bounding);
    // boost::static_pointer_cast<Scene3D>(scene)->GetObjects<Geometry>(e_ObjectType_Geometry,
    // visibleGeometry);
...
bool GraphicsTaskCommand_RenderImage2D::Execute(void *caller) {
  bool success;
  boost::shared_ptr<IScene> scene =
      SceneManager::GetInstance().GetScene("scene2D", success);
  if (success) {
