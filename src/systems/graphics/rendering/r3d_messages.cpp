...
bool Renderer3DMessage_RenderView::Execute(void *caller) {
  ...
  Matrix4 projectionMatrix = renderer->CreatePerspectiveMatrix(
      view.width / (view.height * 1.0f), buffer.cameraNearCap,
      buffer.cameraFarCap);
  Matrix4 viewMatrix = buffer.cameraMatrix;

  // not sorting actually seems to be fastest atm. (the sorting being slower
  // than the performance win by reducing state changes)
  ...
  renderer->SetViewport(view.x, height - (view.y + view.height), view.width,
                        view.height);

  renderer->SetFOV(buffer.cameraFOV);

  float depthParamNear =
      buffer.cameraFarCap / (buffer.cameraFarCap - buffer.cameraNearCap);
  float depthParamFar = (buffer.cameraFarCap * buffer.cameraNearCap) /
  ...
  // enabled again: the geometry phase writes to the gbuffer, this writes to the
  // accumbuffer that the lighting uses disabled again: seems to work without
  // z-phase, even with ambient phase on z equal. i guess the depth buffer is
  // kept indepedently of the bound frame buffer. disabled again: alpha pass
  // (lame transparency) won't work with this
  ...
  renderer->SetUniformFloat2("ambient", "cameraClip", depthParamNear,
                             depthParamFar);
  Matrix4 inverseProjectionViewMatrix =
      (projectionMatrix * viewMatrix).GetInverse();
  ...
  // renderer->SetUniformFloat2("lighting", "cameraClip", depthParamNear,
  // depthParamFar);
  ...
  renderer->SetUniformFloat("postprocess", "contextY",
                            (float)(height - (view.y + view.height)));
  renderer->SetUniformFloat2("postprocess", "cameraClip", depthParamNear,
                             depthParamFar);
  renderer->SetUniformFloat("postprocess", "fogScale",
                            0.8f - NormalizedClamp(buffer.cameraFOV, 20, 100) *
                                       0.6f);
...
bool Renderer3DMessage_RenderShadowMap::Execute(void *caller) {
  ...
  // SORTING update: because we pass a const reference, we need to sort
  // vertexbuffers in graphics_light (or wherever we invoke this)
  ...
  // sorting a list however might be ok. needs to be tested in a situation with
  // lots of different id's
