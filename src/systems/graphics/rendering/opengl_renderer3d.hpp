...
public:
  ...
  virtual void
  RenderOverlay2D(const std::vector<Overlay2DQueueEntry> &overlay2DQueue);
  ...
  // render targets
  virtual void
  SetRenderTargets(std::vector<e_TargetAttachment> targetAttachments);
...
protected:
  ...
  // members and functions for rendering overlay with shaders instead of
  // deprecated methods
  VertexBufferID overlayBuffer; // buffer for drawing textures such as player's
                                // names and game score
