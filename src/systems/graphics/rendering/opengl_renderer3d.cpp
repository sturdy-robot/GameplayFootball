...
void OpenGLRenderer3D::SwapBuffers() { SDL_GL_SwapWindow(window); }
...
void OpenGLRenderer3D::RenderOverlay2D(
  ...
  Matrix4 orthoMatrix =
      CreateOrthoMatrix(0, context_width, context_height, 0, 0.1, 10);
  ...
  for (unsigned int i = 0; i < overlay2DQueue.size(); i++) {
    ...
    modelMatrix.SetTranslation(
        Vector3(queueEntry.position[0], queueEntry.position[1], -1.0f));
...
void drawSphere(float r, int lats, int longs) {
  // Never called (because light.type is always set to 0) and uses deprecated
  // methods. Possible implementation, that uses modern OpenGl can be found
  // here: https://gist.github.com/zwzmzd/0195733fa1210346b00d
  /*
    int i, j;
    for (i = 0; i <= lats; i++) {
...
void OpenGLRenderer3D::RenderLights(std::deque<LightQueueEntry> &lightQueue,
  ...
  while (lightIter != lightQueue.end()) {
    ...
    // todo: add light types SetUniformInt("lighting", "lightType",
    // (int)light.type);
...
VertexBufferID OpenGLRenderer3D::CreateSimpleVertexBuffer(GLfloat *vertices,
                                                          unsigned int size) {
  GLuint VBO, VAO;
  ...
  mapping.glVertexAttribPointer(0, 4, GL_FLOAT, GL_FALSE, 4 * sizeof(GLfloat),
                                (GLvoid *)0);
...
void OpenGLRenderer3D::InitializeOverlayAndQuadBuffers() {
  ...
  overlayBuffer =
      CreateSimpleVertexBuffer(overlayVertices, sizeof(overlayVertices));
...
bool OpenGLRenderer3D::CreateContext(int width, int height, int bpp,
  ...
  // todo: remember to enable this later on, after migrating to sdl 2 (though it
  // is on by default with most drivers, or so it seems)
...
#undef SDL_PROC
  ...
  Log(e_Notice, "OpenGLRenderer3D", "CreateContext",
      "Using OpenGL version " + glVersionString);
  ...
  Log(e_Notice, "OpenGLRenderer3D", "CreateContext",
      "OpenGL major/minor " + int_to_str(glVersion[0]) + "." +
          int_to_str(glVersion[1]));
  if (!context) {
  ...
  if (!higherThan32)
    Log(e_Warning, "OpenGLRenderer3D", "CreateContext",
        "OpenGL version not equal to or higher than 3.2 (or not reported as "
        "such)");
  ...
  if (SDL_GetWMInfo(&wmInfo)) {
    ...
    if (!fullscreen) {
      ...
      if (getMonInfo != 0) {
        ...
        xWin = std::max(xWin, 0); // super annoying to have a window's controls
  being unreachable yWin = std::max(yWin, 0); xWin -= 3; // window border size
  (educated guess - is a user setting, after all) yWin -= 12; // window border
  size + taskbar (educated guess - is a user setting, after all)
  ...
  // largest_supported_anisotropy = clamp(largest_supported_anisotropy, 0, 8);
  // // don't overdo it
  ...
  mapping.glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT |
...
void OpenGLRenderer3D::Exit() {
  ...
  // assert(views.size() == 0);
  // todo: make views erase-able, as for now a views vector index is used when
  // requesting views, which prohibits erasion (actually, erasion 'works', but
  // it does leave 'empty' records in the views vector, which is ugly and
  // memory-leaky)
}
...
int OpenGLRenderer3D::CreateView(float x_percent, float y_percent,
  ...
  // view.gBuffer_AlbedoTexID = CreateTexture(e_InternalPixelFormat_RGBA8,
  // e_PixelFormat_RGBA, gBufWidth, gBufHeight, false, false, false, false,
  // false);
  view.gBuffer_AlbedoTexID =
      CreateTexture(e_InternalPixelFormat_RGBA16F, e_PixelFormat_RGBA,
                    gBufWidth, gBufHeight, false, false, false, false, false);
  // view.gBuffer_AlbedoTexID = CreateTexture(e_InternalPixelFormat_RGBA32F,
  // e_PixelFormat_RGBA, gBufWidth, gBufHeight, false, false, false, false,
  // false);
  SetFrameBufferTexture2D(e_TargetAttachment_Color0, view.gBuffer_AlbedoTexID);

  // view.gBuffer_NormalTexID = CreateTexture(e_InternalPixelFormat_RGBA8,
  // e_PixelFormat_RGBA, gBufWidth, gBufHeight, false, false, false, false,
  // false);
  view.gBuffer_NormalTexID =
      CreateTexture(e_InternalPixelFormat_RGBA16F, e_PixelFormat_RGBA,
                    gBufWidth, gBufHeight, false, false, false, false, false);
  // view.gBuffer_NormalTexID = CreateTexture(e_InternalPixelFormat_RGBA32F,
  // e_PixelFormat_RGBA, gBufWidth, gBufHeight, false, false, false, false);
  SetFrameBufferTexture2D(e_TargetAttachment_Color1, view.gBuffer_NormalTexID);

  // view.gBuffer_AuxTexID = CreateTexture(e_InternalPixelFormat_RGBA8,
  // e_PixelFormat_RGBA, gBufWidth, gBufHeight, false, false, false, false,
  // false);
  view.gBuffer_AuxTexID =
      CreateTexture(e_InternalPixelFormat_RGBA16F, e_PixelFormat_RGBA,
                    gBufWidth, gBufHeight, false, false, false, false, false);
  // view.gBuffer_AuxTexID = CreateTexture(e_InternalPixelFormat_RGBA32F,
  // e_PixelFormat_RGBA, gBufWidth, gBufHeight, false, false, false, false,
  // false);
  ...
  // view.accumBuffer_DepthTexID =
  // CreateTexture(e_InternalPixelFormat_DepthComponent32F,
  // e_PixelFormat_DepthComponent, accumBufWidth, accumBufHeight, false, false,
  // false, false); SetFrameBufferTexture2D(e_TargetAttachment_Depth,
  // view.accumBuffer_DepthTexID);

  // view.accumBuffer_AccumTexID = CreateTexture(e_InternalPixelFormat_RGB8,
  // e_PixelFormat_RGB, accumBufWidth, accumBufHeight, false, false, false,
  // false, false);
  ...
  // view.accumBuffer_AccumTexID = CreateTexture(e_InternalPixelFormat_RGBA32F,
  // e_PixelFormat_RGBA, accumBufWidth, accumBufHeight, false, false, false,
  // false, false);
  SetFrameBufferTexture2D(e_TargetAttachment_Color0,
                          view.accumBuffer_AccumTexID);

  // Geforce GT 230 doesn't seem to be too happy about
  // e_InternalPixelFormat_RGBA16 for some reason. Very slow framerates ensue
  // view.accumBuffer_ModifierTexID = CreateTexture(e_InternalPixelFormat_RGBA8,
  // e_PixelFormat_RGB, accumBufWidth, accumBufHeight, false, false, false,
  // false, false);
  ...
  // CreateTexture(e_InternalPixelFormat_RGBA32F, e_PixelFormat_RGBA,
...
View &OpenGLRenderer3D::GetView(int viewID) { return views.at(viewID); }

void OpenGLRenderer3D::DeleteView(int viewID) {

  Log(e_Notice, "OpenGLRenderer3D", "DeleteView",
      "Deleting view, id " + int_to_str(viewID));
...
void OpenGLRenderer3D::UpdateVertexBuffer(VertexBufferID vertexBufferID,
  ...
  mapping.glBindBuffer(
      GL_ARRAY_BUFFER,
      writeVertexBufferID); // todo: why is this necessary? shouldn't this be
                            // part of the vao?
...
void OpenGLRenderer3D::DeleteVertexBuffer(VertexBufferID vertexBufferID) {
  ...
  /*fenceoff
      // delete sync object
      std::map<int, GLsync>::iterator syncIter =
     VAOfence.find(vertexBufferID.vertexArrayID); if (syncIter !=
     VAOfence.end()) { glDeleteSync(syncIter->second); VAOfence.erase(syncIter);
      }
  */
}
  ...
  while (vertexBufferQueueIter != vertexBufferQueue.end()) {
    ...
    if (currentBoundBuffer != vertexBuffer->GetVaoID()) {
      ...
      /*
         ...
         GL_CONDITION_SATISFIED) { returnVal =
    ...
    bool sequential = true; // buffer vertexbuffer chunks until a change happens
                            // (in texture or index, for example)
    struct BufferChunk {
      BufferChunk() {
        startIndex = 0;
        count = 0;
    ...
    while (vertexBufferIter != queueEntry->vertexBufferIndices.end()) {
      ...
      // if (sequential) if (vertexBufferIter + 1 ==
      // queueEntry->vertexBufferIndices.end()) sequential = false; // data end
      if (sequential) {
        // add to buffer
...
void OpenGLRenderer3D::RenderAABB(
    std::list<VertexBufferQueueEntry> &vertexBufferQueue) {
  ...
  std::list<VertexBufferQueueEntry>::iterator vertexBufferQueueIter =
  vertexBufferQueue.begin(); while (vertexBufferQueueIter !=
  vertexBufferQueue.end()) { VertexBufferQueueEntry *queueEntry =
  &(*vertexBufferQueueIter);

    mapping.glVertex3f(queueEntry->aabb.minxyz.coords[0],
  queueEntry->aabb.minxyz.coords[1], queueEntry->aabb.minxyz.coords[2]);
    mapping.glVertex3f(queueEntry->aabb.maxxyz.coords[0],
  queueEntry->aabb.minxyz.coords[1], queueEntry->aabb.minxyz.coords[2]);
...
void OpenGLRenderer3D::SetLight(const Vector3 &position, const Vector3 &color,
  ...
  radius }; mapping.glLightfv(GL_LIGHT0, GL_DIFFUSE, diffuseF);
...
int OpenGLRenderer3D::CreateTexture(e_InternalPixelFormat internalPixelFormat,
  ...
  /*
      if (pot(source->w) != source->w || pot(source->h) != source->h) {
        Log(e_Warning, "OpenGLRenderer3D", "CreateTexture", "Non-power-of-2
     texture!");
      }
  */
...
void OpenGLRenderer3D::ResizeTexture(int textureID, SDL_Surface *source,
  ...
  if (internalPixelFormat == e_InternalPixelFormat_DepthComponent16 ||
    ...
    // Post-GLSL 1.3, DEPTH_TEXTURE_MODE is deprecated and GLSL behaves as if
    // its always set to LUMINANCE
    // mapping.glTexParameteri(GL_TEXTURE_2D, GL_DEPTH_TEXTURE_MODE,
    // GL_INTENSITY);
  }
...
void OpenGLRenderer3D::SetMaxAnisotropy() {
  // #ifndef __gl_glcorearb_h_  // Can be used to check for Core Profile Mode on
  // Linux
...
void OpenGLRenderer3D::SetRenderBufferStorage(
    e_InternalPixelFormat internalPixelFormat, int width, int height) {
  mapping.glRenderbufferStorage(GL_RENDERBUFFER,
                                GetGLInternalPixelFormat(internalPixelFormat),
                                width, height);
}
...
void OpenGLRenderer3D::SetFOV(float angle) { FOV = angle; }

void OpenGLRenderer3D::PushAttribute(int attr) {
  mapping.glPushAttrib((GLbitfield)attr);
}

void OpenGLRenderer3D::PopAttribute() { mapping.glPopAttrib(); }
...
void OpenGLRenderer3D::SetPolygonOffset(float scale, float bias) {
  mapping.glPolygonOffset(
      scale,
      bias); // hint: doesn't work with glsl since i'm using my own calculations
}
  ...
  if (!usePrecalculatedSet || kernelSize != 32) {
    ...
    for (unsigned int i = 0; i < kernelSize; i++) {
      ...
      for (unsigned int candidate = 0; candidate < candidateSize; candidate++) {
        // candidates[candidate] = Vector3(random(-1.0f, 1.0f),
        // random(-1.0f, 1.0f), random(0.001f, 1.0f)); candidates[candidate] =
        // candidates[candidate].GetNormalized(0) * random(0.001f, 1.0f);
        ...
        candidates[candidate] =
            Vector3(sin(s) * m, cos(s) * m, fabs(t)); // fabs for hemi
      }
...
void OpenGLRenderer3D::HDRCaptureOverallBrightness() {
  ...
  int count = 0;
  for (int i = 0; i < 2; i++) {
    //    for (int x = context_width * 0.2; x <= context_width * 0.8; x +=
    //    context_width * 0.01) {
    //      for (int y = context_height * 0.2; y <= context_height * 0.8; y +=
    //      context_height * 0.01) {
...
float OpenGLRenderer3D::HDRGetOverallBrightness() { return overallBrightness; }

// thread main loop

void OpenGLRenderer3D::operator()() {
  Log(e_Notice, "OpenGLRenderer3D", "operator()()",
      "Starting OpenGLRenderer3D thread");
  ...
  while (!quit) {
    ...
    bool isMessage;
    boost::intrusive_ptr<Command> message =
        messageQueue.WaitForMessage(isMessage, 1);
    if (isMessage) {
  ...
  Log(e_Notice, "OpenGLRenderer3D", "operator()()",
      "Shutting down OpenGLRenderer3D thread");
