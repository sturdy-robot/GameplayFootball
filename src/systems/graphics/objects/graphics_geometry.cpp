...
GraphicsGeometry::GetInterpreter(e_ObjectType objectType) {
...
Vector3 GraphicsGeometry::GetPosition() const { return position; }

void GraphicsGeometry::SetRotation(const Quaternion &newRotation) {
  rotation = newRotation;
}

Quaternion GraphicsGeometry::GetRotation() const { return rotation; }
...
void LoadMaterials(
    Renderer3D *renderer3D, const Material *material,
  ...
  if (material->normalTexture) {
    ...
    if (!texAlreadyThere) {
      ...
      normalTexture->GetResource()->CreateTexture(
          e_InternalPixelFormat_RGB8, e_PixelFormat_RGB, image->w, image->h,
  ...
  if (material->specularTexture) {
    ...
    if (!texAlreadyThere) {
      ...
      specularTexture->GetResource()->CreateTexture(
          e_InternalPixelFormat_RGB8, e_PixelFormat_RGB, image->w, image->h,
          false, true, true, true);
  ...
  if (material->illuminationTexture) {
    ...
    if (!texAlreadyThere) {
      ...
      illuminationTexture->GetResource()->CreateTexture(
          e_InternalPixelFormat_RGB8, e_PixelFormat_RGB, image->w, image->h,
          false, true, true, true);
...
void GraphicsGeometry_GeometryInterpreter::OnLoad(
  ...
  // todo: rewrite that indices/usesIndices thing, it's very unclear what's
  // going on
  ...
  for (unsigned int i = 0; i < triangleMeshes.size(); i++) {
    ...
    Renderer3DMaterial r3dMaterial;
    LoadMaterials(renderer3D, material, r3dMaterial, diffuseTexture,
                  normalTexture, specularTexture, illuminationTexture);
    ...
    if (!alreadyThere) {
      ...
      */
      ...
      for (int e = 0; e < GetTriangleMeshElementCount(); e++) {
        ...
        // geometry->GetName().c_str(), e, verticesDataSize, startIndex,
        // triangleMeshes[i].verticesDataSize);
        memcpy(
            &vertices[e * (verticesDataSize / GetTriangleMeshElementCount()) +
                      startIndex],
            &triangleMeshes[i]
                 .vertices[e * (triangleMeshes[i].verticesDataSize /
                                GetTriangleMeshElementCount())],
            triangleMeshes[i].verticesDataSize / GetTriangleMeshElementCount() *
                sizeof(float));
      }
    ...
    if (indices.size() > 0) {
      vbIndex.startIndex =
          (indices.size() - triangleMeshes[i].indices.size()); // start index id
  ...
  if (!alreadyThere) {
    caller->vertexBuffer->GetResource()->SetTriangleMesh(
        vertices, verticesDataSize, indices);
    caller->vertexBuffer->GetResource()->CreateOrUpdateVertexBuffer(
        renderer3D, dynamicBuffer);
...
void GraphicsGeometry_GeometryInterpreter::OnUpdateGeometry(
  ...
  // todo: right now, you can only go from using no indices to using indices.
  // you cannot go back to not using indices anymore. fix this. todo: rewrite
  // that indices/usesIndices thing either way, it's very unclear what's going
  // on
  ...
  caller->vertexBuffer->resourceMutex.lock();
  float *vertices = 0;
  int currentVerticesDataSize =
      caller->vertexBuffer->GetResource()->GetVerticesDataSize();
  ...
  if (verticesDataSize == currentVerticesDataSize) {
    ...
    newFloatData = false;
    updateIndices =
        updateMaterials; // todo: can we decouple indices and materials in this
                         // function? now we need to update indices on material
                         // update and the other way around
  } else {
    vertices = new float[verticesDataSize];
    newFloatData = true;
    updateIndices = true;
  ...
  for (unsigned int i = 0; i < triangleMeshes.size(); i++) {
    ...
    for (int e = 0; e < GetTriangleMeshElementCount(); e++) {
      // printf("%s: e: %i, verticesDataSize: %i, startIndex: %i,
      // triangleMeshes[i].verticesDataSize: %i\n", geometry->GetName().c_str(),
      // e, verticesDataSize, startIndex, triangleMeshes[i].verticesDataSize);
      memcpy(
          &vertices[e * (verticesDataSize / GetTriangleMeshElementCount()) +
                    startIndex],
    ...
    if (!usesIndices) { // can only set indices once (todo: make OnUpdateIndices
    ...
    if (updateIndices) {
      ...
      // this code makes this version only work on meshes that are the same
      // size/materialorder as their previous version
      vbIndex.material = r3dMaterial;
      ...
      // printf("st %i VS %i\n", (indices.size() -
      // triangleMeshes[i].indices.size()), startIndex / 3); printf("si %i VS
  ...
  if (newFloatData) {
    ...
    caller->vertexBuffer->GetResource()->TriangleMeshWasUpdatedExternally(
        verticesDataSize, indices);
  }
  caller->vertexBuffer->GetResource()->CreateOrUpdateVertexBuffer(
      renderer3D, dynamicBuffer);
  caller->vertexBuffer->resourceMutex.unlock();
