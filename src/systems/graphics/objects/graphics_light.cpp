...
GraphicsLight::GetInterpreter(e_ObjectType objectType) {
...
Vector3 GraphicsLight::GetPosition() const { return position; }

void GraphicsLight::SetColor(const Vector3 &newColor) { color = newColor; }

Vector3 GraphicsLight::GetColor() const { return color; }

void GraphicsLight::SetRadius(float radius) { this->radius = radius; }

float GraphicsLight::GetRadius() const { return radius; }

void GraphicsLight::SetType(e_LightType lightType) {
  this->lightType = lightType;
}

e_LightType GraphicsLight::GetType() const { return lightType; }

void GraphicsLight::SetShadow(bool shadow) { this->shadow = shadow; }

bool GraphicsLight::GetShadow() const { return shadow; }
...
void GraphicsLight_LightInterpreter::OnUnload() {
  ...
  // todo IMPORTANT: find a way to dynamically unload shadow maps if a camera is
  // removed maybe a timer? if not used for x seconds, unload? would take a away
  // the need to let camera keep record of lights
  ...
  while (iter != caller->shadowMaps.end()) {
    ...
    boost::intrusive_ptr<Renderer3DMessage_DeleteFrameBuffer> deleteFrameBuffer(
        new Renderer3DMessage_DeleteFrameBuffer((*iter).frameBufferID,
                                                e_TargetAttachment_Depth));
    renderer3D->messageQueue.PushMessage(deleteFrameBuffer);
    deleteFrameBuffer->Wait();

    // happens automagically when noone links to texture anymore.. disabled here
    // so we don't delete the texture twice
    /*
...
bool GraphicsLight_LightInterpreter::GetShadow() { return caller->GetShadow(); }
...
bool GLLI_SortVertexBufferQueueEntries(const VertexBufferQueueEntry &vb1,
  ...
  return vb1.vertexBuffer->GetResource()->GetID() <
         vb2.vertexBuffer->GetResource()->GetID();
}
...
void GraphicsLight_LightInterpreter::EnqueueShadowMap(
  ...
  for (int i = 0; i < (signed int)caller->shadowMaps.size(); i++) {
    if (caller->shadowMaps.at(i).cameraName ==
        camera->GetName()) { // todo: what if cam name changes? this is hardly
                             // ideal
      index = i;
    }
  }
  ...
  if (index == -1) {
    ...
    map.texture =
        ResourceManagerPool::GetInstance()
            ...
            ->Fetch(std::string(camera->GetName() + int_to_str(intptr_t(this))),
                    false, false); // false == don't try to use loader
    if (map.texture->GetResource()->GetID() == -1) {
      ...
      // e_PixelFormat_DepthComponent, 1024, 1024, false, false, false, true,
      // true);
      map.texture->GetResource()->CreateTexture(
          e_InternalPixelFormat_DepthComponent16, e_PixelFormat_DepthComponent,
          2048, 2048, false, false, false, true, true);
      // map.texture->GetResource()->CreateTexture(e_InternalPixelFormat_DepthComponent16,
      // e_PixelFormat_DepthComponent, 4096, 4096, false, false, false, true,
      // true); // filter on to get hardware shadowmap AA in shader:
      // http://stackoverflow.com/questions/22419682/glsl-sampler2dshadow-and-shadow2d-clarification
      // map.texture->GetResource()->CreateTexture(e_InternalPixelFormat_DepthComponent32,
      // e_PixelFormat_DepthComponent, 4096, 4096, false, false, false, true,
      // true);
      // map.texture->GetResource()->CreateTexture(e_InternalPixelFormat_DepthComponent16,
      // e_PixelFormat_DepthComponent, 8192, 8192, false, false, false, true,
      // true);

      // create framebuffer for shadowmap
      boost::intrusive_ptr<Renderer3DMessage_CreateFrameBuffer>
          createFrameBuffer(new Renderer3DMessage_CreateFrameBuffer(
              e_TargetAttachment_Depth, map.texture->GetResource()->GetID()));
  ...
  while (visibleGeometryIter != visibleGeometry.end()) {
    boost::intrusive_ptr<GraphicsGeometry_GeometryInterpreter> interpreter =
        static_pointer_cast<GraphicsGeometry_GeometryInterpreter>(
    ...
    interpreter->GetVertexBufferQueue(
        caller->shadowMaps.at(index).visibleGeometry);
  ...
  // caller->shadowMaps.at(index).visibleGeometry.end(),
  // GLLI_SortVertexBufferQueueEntries);

  // all this is way too hardcoded and should somehow heed the camera bounding
  // box better
  ...
  caller->shadowMaps.at(index).lightViewMatrix.ConstructInverse(
      pos, Vector3(1, 1, 1), rot);
}

ShadowMap
GraphicsLight_LightInterpreter::GetShadowMap(const std::string &camName) {
...
void GraphicsLight_LightInterpreter::OnPoke() {
  ...
  while (iter != caller->shadowMaps.end()) {
    ...
    caller->GetGraphicsScene()
        ->GetGraphicsSystem()
        ->GetRenderer3D()
        ->messageQueue.PushMessage(renderShadowMap);
