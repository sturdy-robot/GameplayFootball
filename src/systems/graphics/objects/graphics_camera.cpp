...
GraphicsCamera::GetInterpreter(e_ObjectType objectType) {
...
Vector3 GraphicsCamera::GetPosition() const { return position; }

void GraphicsCamera::SetRotation(const Quaternion &newRotation) {
  rotation = newRotation;
}

Quaternion GraphicsCamera::GetRotation() const { return rotation; }
...
void GraphicsCamera_CameraInterpreter::OnLoad(const Properties &properties) {
  caller->SetSize(properties.GetReal("x_percent", 0),
                  properties.GetReal("y_percent", 0),
  ...
  boost::intrusive_ptr<Renderer3DMessage_CreateView> createView(
      new Renderer3DMessage_CreateView(caller->x_percent, caller->y_percent,
                                       caller->width_percent,
                                       caller->height_percent));
...
void GraphicsCamera_CameraInterpreter::SetFOV(float fov) { caller->fov = fov; }
...
void GraphicsCamera_CameraInterpreter::EnqueueView(
  ...
  std::deque<boost::intrusive_ptr<Light>>::iterator visibleLightIter =
      visibleLights.begin();
  while (visibleLightIter != visibleLights.end()) {
    ...
    boost::intrusive_ptr<GraphicsLight_LightInterpreter> interpreter =
        static_pointer_cast<GraphicsLight_LightInterpreter>(
    ...
    entry.type =
        (*visibleLightIter)->GetType() == e_LightType_Directional ? 0 : 1;
  ...
  std::deque<boost::intrusive_ptr<Skybox>>::iterator skyboxIter =
      skyboxes.begin();
  while (skyboxIter != skyboxes.end()) {
    boost::intrusive_ptr<GraphicsGeometry_SkyboxInterpreter> interpreter =
        static_pointer_cast<GraphicsGeometry_SkyboxInterpreter>(
  ...
  buffer->cameraMatrix.ConstructInverse(caller->GetPosition(), Vector3(1, 1, 1),
                                        caller->GetRotation());
  // buffer->projectionMatrix.ConstructProjection(caller->fov, aspect,
  // caller->nearCap, caller->farCap);
...
void GraphicsCamera_CameraInterpreter::OnPoke() {
  ...
  caller->GetGraphicsScene()
      ->GetGraphicsSystem()
      ->GetRenderer3D()
      ->messageQueue.PushMessage(renderView, true);
