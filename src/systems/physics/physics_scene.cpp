...
ISystemObject *
PhysicsScene::CreateSystemObject(boost::intrusive_ptr<Object> object) {
  assert(object.get());

  // todo: system scene shouldn't know about object types, i guess? maybe via a
  // Command
  if (object->GetObjectType() == e_ObjectType_Geometry) {
    if (boost::static_pointer_cast<Geometry>(object)->GetProperties().GetBool(
            "physicable") == true) {
      PhysicsGeometry *physicsObject = new PhysicsGeometry(this);
      // physics system objects need to feedback to objects, so pass it as user
      // data
      object->Attach(physicsObject->GetInterpreter(e_ObjectType_Geometry),
                     object.get());
    ...
    // physics system objects need to feedback to objects, so pass it as user
    // data
    object->Attach(physicsObject->GetInterpreter(e_ObjectType_Joint),
                   object.get());
    return physicsObject;
  }
...
boost::intrusive_ptr<ISceneInterpreter>
PhysicsScene::GetInterpreter(e_SceneType sceneType) {
...
PhysicsSystem *PhysicsScene::GetPhysicsSystem() { return physicsSystem; }

IPhysicsWrapper *PhysicsScene::GetPhysicsWrapper() { return physicsWrapper; }

// Scene3D interpreter

PhysicsScene_Scene3DInterpreter::PhysicsScene_Scene3DInterpreter(
    PhysicsScene *caller)
    : caller(caller) {}
...
void PhysicsScene_Scene3DInterpreter::OnLoad() {
  ...
  caller->GetPhysicsWrapper()->SetGravity(caller->worldID,
                                          Vector3(0, 0, -9.81));
