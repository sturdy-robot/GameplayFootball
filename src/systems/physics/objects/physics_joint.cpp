...
PhysicsJoint::PhysicsJoint(PhysicsScene *physicsScene)
    : PhysicsObject(physicsScene), joint(0) {}

PhysicsJoint::~PhysicsJoint() {}

boost::intrusive_ptr<Interpreter>
PhysicsJoint::GetInterpreter(e_ObjectType objectType) {
...
Vector3 PhysicsJoint::GetPosition() const { return position; }

void PhysicsJoint::SetRotation(const Quaternion &newRotation) {
  rotation = newRotation;
}

Quaternion PhysicsJoint::GetRotation() const { return rotation; }

// joint interpreter

PhysicsJoint_JointInterpreter::PhysicsJoint_JointInterpreter(
    PhysicsJoint *caller)
    : caller(caller) {
  physics = caller->GetPhysicsScene()->GetPhysicsSystem()->GetPhysicsWrapper();
}

void PhysicsJoint_JointInterpreter::OnLoad(
    boost::intrusive_ptr<Joint> joint, boost::intrusive_ptr<Geometry> object1,
  ...
  boost::intrusive_ptr<PhysicsGeometry_GeometryInterpreter> interpreter =
      static_pointer_cast<PhysicsGeometry_GeometryInterpreter>(
  ...
  // hmm... well probably the game programmer is responsible for not deleting
  // his objects while joint'ing them

  interpreter = static_pointer_cast<PhysicsGeometry_GeometryInterpreter>(
  ...
  caller->joint = physics->CreateJoint(caller->GetPhysicsScene()->worldID,
                                       caller->jointType, actor1, actor2,
                                       anchor, axis1, axis2);
}
...
void PhysicsJoint_JointInterpreter::SetStops(radian lowStop, radian highStop,
                                             int paramNum) {
...
void PhysicsJoint_JointInterpreter::SetConstraintForceMixing(float value,
                                                             int paramNum) {
  physics->JointSetConstraintForceMixing(caller->joint, value, paramNum);
}

void PhysicsJoint_JointInterpreter::SetErrorCorrection(float value,
                                                       int paramNum) {
  physics->JointSetErrorCorrection(caller->joint, value, paramNum);
}

// only on hinge2 joints
void PhysicsJoint_JointInterpreter::SetSuspensionConstraintForceMixing(
    float value) {
  ...
  if (caller->joint) {
    ...
    if (subject->RequestPropertyExists("angle1_rate")) {
      subject->SetRequestProperty(
          "angle1_rate",
          real_to_str(physics->JointGetAngleRate(caller->joint, 1)).c_str());
    }
    if (subject->RequestPropertyExists("angle2_rate")) {
      subject->SetRequestProperty(
          "angle2_rate",
          real_to_str(physics->JointGetAngleRate(caller->joint, 2)).c_str());
    }
  }
}
