...
PhysicsTask::PhysicsTask(PhysicsSystem *system)
    : ISystemTask(), physicsSystem(system) {
  remainder_ms = 0;
  timeStep_ms = 2;
  previousTime_ms = EnvironmentManager::GetInstance().GetTime_ms();
}

PhysicsTask::~PhysicsTask() { physicsSystem = NULL; }

void PhysicsTask::operator()() {
  Log(e_Notice, "PhysicsTask", "operator()()",
      "Starting PhysicsSystemTask thread");
  ...
  Log(e_Notice, "PhysicsTask", "operator()()",
      "Shutting down PhysicsSystemTask thread");
...
void PhysicsTask::ProcessPhase() {
  ...
  // run timestep loop
  // printf("%i %i %i\n", GetClocksPerFrame(),
  // GetScheduler()->GetMasterClock_ms(), remainder_ms);
  ...
  boost::intrusive_ptr<PhysicsTaskCommand_StepTime> stepTime(
      new PhysicsTaskCommand_StepTime(physicsSystem->GetPhysicsWrapper(),
                                      timeDiff_ms + remainder_ms, timeStep_ms));
...
bool PhysicsTaskCommand_UpdateGeometry::Execute(void *caller) {
  ...
  boost::shared_ptr<IScene> scene =
      SceneManager::GetInstance().GetScene(i, success);
  while (success) {
