...
OdePhysics::OdePhysics() { dInitODE(); };

OdePhysics::~OdePhysics() { dCloseODE(); };
  ...
  if (dGeomIsSpace(o1) || dGeomIsSpace(o2)) {
    ...
    if (bodies[0] != NULL || bodies[1] != NULL) {
      ...
      if (bodies[0] && bodies[1] &&
          dAreConnectedExcluding(bodies[0], bodies[1], dJointTypeContact))
        return;
      ...
      // if both of the colliding objects are colliding, take the first (kinda
      // hacky ;)) example problem is, if both are dynamic, what fdir1 to use?
      ...
      // when larger, collisions near seams between trimesh triangles causes 2
      // contacts (with both triangles) to be created. this causes bumpy
      // movement on seams. max 1 contact probably won't work with non-convex
      // objects, so they are not supported (yet)
      int MAX_CONTACTS = 12;
      dContact contactsArray[MAX_CONTACTS];
      std::vector<dContact> contacts;

      // colliding two non-space geoms, so generate contact points between o1
      // and o2
      int num_contact = dCollide(o1, o2, MAX_CONTACTS, &contactsArray[0].geom,
                                 sizeof(dContact));
      ...
      float mergeThreshold =
          0.4; // contact points less than 40 centimeters apart? merge em
      bool firstTime = true;
      ...
      while (possibleNeighbours || firstTime) {
        ...
        while (subjectIter != contacts.end()) {
          ...
          Vector3 normal((*subjectIter).geom.normal[0],
                         (*subjectIter).geom.normal[1],
          ...
          while (checkIter != contacts.end()) {
            ...
            Vector3 normal2((*checkIter).geom.normal[0],
                            (*checkIter).geom.normal[1],
            ...
            if (pos.GetDistance(pos2) < mergeThreshold) {
              ...
              // edit: just ignoring the second contact seems to give better
              // results
              /*
              ...
              */
              ...
              // iterate the 'main loop' once more after this, there might be
              // more neighbours
      ...
      std::vector<dContact>::iterator subjectIter = contacts.begin();
      // printf("%f - %f\n", properties1->GetReal("friction"),
      // properties2->GetReal("friction"));
      while (subjectIter != contacts.end()) {
        (*subjectIter).surface.mode =
            dContactMu2 | dContactSoftCFM | dContactSoftERP | dContactApprox1;
        (*subjectIter).surface.mu = properties[0]->GetReal("friction_1", 1) *
                                    properties[1]->GetReal("friction_1", 1);
        (*subjectIter).surface.mu2 = properties[0]->GetReal("friction_2", 1) *
                                     properties[1]->GetReal("friction_2", 1);
        ...
        if (properties[dynamicBody]->Exists("bounce")) {
          ...
          (*subjectIter).surface.bounce =
              properties[dynamicBody]->GetReal("bounce", 0.1);
        ...
        if (properties[dynamicBody]->Exists("friction_direction_1")) {
          ...
          // get user fdir1
          Vector3 fdir1 =
          properties[dynamicBody]->GetVector3("friction_direction_1");
          */
          ...
          if (properties[dynamicBody]->Exists("slip")) {
            ...
            (*subjectIter).surface.slip2 =
                properties[dynamicBody]->GetReal("slip") * velocity;
          }
        }

        dJointID c = dJointCreateContact(static_cast<dWorldID>(data),
                                         collisionGroup, &*subjectIter);
...
int OdePhysics::StepTime(int timediff_ms, int resolution_ms) {
  ...
  // printf("%i %i %i %i\n", numSteps, timediff_ms, resolution_ms,
  // remainder_ms);
  ...
  while (worldIter != worldMap.end()) {
    ...
    for (int i = 0; i < numSteps; i++) {
      dSpaceCollide((*spaceIter).second, (*worldIter).second,
                    &OdePhysicsNearCallback);
...
int OdePhysics::CreateWorld() {
  ...
  if (result.second == false) {
    Log(e_FatalError, "OdePhysics", "CreateWorld",
        "Could not insert worldMap entry");
    return 0;
  }
...
int OdePhysics::CreateSpace() {
  ...
  if (result.second == false) {
    Log(e_FatalError, "OdePhysics", "CreateSpace",
        "Could not insert spaceMap entry");
    return 0;
  }
...
void OdePhysics::SetGravity(int worldID, const Vector3 &gravity) {
  mutex.lock();
  dWorldSetGravity(GetOdeWorldID(worldID), gravity.coords[0], gravity.coords[1],
                   gravity.coords[2]);
  mutex.unlock();
}
...
void OdePhysics::ActorSetMassSphere(IPhysicsActor *actor, float radius,
                                    float weight) {
  mutex.lock();
...
void OdePhysics::ActorSetMassBox(IPhysicsActor *actor, const Vector3 &sides,
                                 float weight) {
  mutex.lock();
  ...
  dMassSetBoxTotal(&mass, weight, sides.coords[0], sides.coords[1],
                   sides.coords[2]);
  dBodySetMass(static_cast<OdePhysicsActor *>(actor)->id, &mass);
...
void OdePhysics::ActorSetMassPosition(IPhysicsActor *actor,
                                      const Vector3 &pos) {
  mutex.lock();

  const dReal *bla =
      dBodyGetPosition(static_cast<OdePhysicsActor *>(actor)->id);

  dGeomID geom = dBodyGetFirstGeom(static_cast<OdePhysicsActor *>(actor)->id);

  dBodySetPosition(static_cast<OdePhysicsActor *>(actor)->id,
                   bla[0] + pos.coords[0], bla[1] + pos.coords[1],
                   bla[2] + pos.coords[2]);
...
void OdePhysics::ActorSetPosition(IPhysicsActor *actor, const Vector3 &pos) {
  mutex.lock();
  dBodySetPosition(static_cast<OdePhysicsActor *>(actor)->id, pos.coords[0],
...
void OdePhysics::ActorSetRotation(IPhysicsActor *actor, const Quaternion &rot) {
  mutex.lock();
  dReal R[4] = {rot.elements[3], rot.elements[0], rot.elements[1],
                rot.elements[2]};
...
void OdePhysics::ActorApplyForceAtRelativePosition(IPhysicsActor *actor,
                                                   float force,
                                                   const Vector3 &direction,
                                                   const Vector3 &position) {
  mutex.lock();
  dBodyAddForceAtRelPos(
      static_cast<OdePhysicsActor *>(actor)->id, direction.coords[0] * force,
...
IPhysicsCollisionMesh *
OdePhysics::CreateCollisionPlane(int spaceID, const Vector3 &normal, float d) {
  mutex.lock();

  dGeomID id = dCreatePlane(GetOdeSpaceID(spaceID), normal.coords[0],
                            normal.coords[1], normal.coords[2], d);
...
IPhysicsCollisionMesh *OdePhysics::CreateCollisionSphere(int spaceID,
                                                         float radius) {
  mutex.lock();
...
IPhysicsCollisionMesh *OdePhysics::CreateCollisionBox(int spaceID,
                                                      const Vector3 &sides) {
  mutex.lock();

  dGeomID id = dCreateBox(GetOdeSpaceID(spaceID), sides.coords[0],
                          sides.coords[1], sides.coords[2]);
...
IPhysicsCollisionMesh *OdePhysics::CreateCollisionTriMesh(int spaceID,
  ...
  dGeomTriMeshDataBuildSingle(trimesh_id, dvertices, sizeof(dReal) * 3,
                              vertexCount, indices, vertexCount,
  ...
  for (int i = 0; i < vertexCount; i++) {
    ...
    std::vector<Vector3>::iterator found = std::find(uniqueVertices.begin(),
  uniqueVertices.end(), pos); if (found == uniqueVertices.end()) {
  ...
  dGeomTriMeshDataBuildSingle(trimesh_id, dvertices, sizeof(dReal) * 3,
  uniqueVertices.size(), indices, vertexCount, sizeof(unsigned int) * 3);
  */

  dGeomID id =
      dCreateTriMesh(GetOdeSpaceID(spaceID), trimesh_id, NULL, NULL, NULL);
...
void OdePhysics::CollisionMeshSetActor(IPhysicsCollisionMesh *mesh,
                                       IPhysicsActor *actor) {
  mutex.lock();
...
void OdePhysics::CollisionMeshSetPosition(IPhysicsCollisionMesh *mesh,
                                          const Vector3 &pos) {
  mutex.lock();
  dGeomSetPosition(static_cast<OdePhysicsCollisionMesh *>(mesh)->id,
                   pos.coords[0], pos.coords[1], pos.coords[2]);
  mutex.unlock();
}

void OdePhysics::CollisionMeshSetRotation(IPhysicsCollisionMesh *mesh,
...
void OdePhysics::CollisionMeshSetData(IPhysicsCollisionMesh *mesh,
                                      const Properties *data) {
  mutex.lock();
  // need to const_cast, cause ODE needs a non-const.
  // just don't alter it in the collision callback function!
  dGeomSetData(static_cast<OdePhysicsCollisionMesh *>(mesh)->id,
               const_cast<Properties *>(data));
  mutex.unlock();
}

// joint functions

IPhysicsJoint *OdePhysics::CreateJoint(int worldID, e_JointType jointType,
                                       IPhysicsActor *actor1,
                                       IPhysicsActor *actor2,
  ...
  case e_JointType_Hinge:
    ...
    dJointSetHingeAnchor(joint->id, anchor.coords[0], anchor.coords[1],
                         anchor.coords[2]);
    dJointSetHingeAxis(joint->id, axis1.coords[0], axis1.coords[1],
                       axis1.coords[2]);
    break;
  ...
  case e_JointType_Hinge2:
    ...
    dJointAttach(joint->id, static_cast<OdePhysicsActor *>(actor1)->id,
                 static_cast<OdePhysicsActor *>(actor2)->id);
    dJointSetHinge2Anchor(joint->id, anchor.coords[0], anchor.coords[1],
                          anchor.coords[2]);
    dJointSetHinge2Axis1(joint->id, axis1.coords[0], axis1.coords[1],
                         axis1.coords[2]);
    dJointSetHinge2Axis2(joint->id, axis2.coords[0], axis2.coords[1],
                         axis2.coords[2]);
    break;
...
void OdePhysics::JointSetStops(IPhysicsJoint *joint, radian lowStop,
                               radian highStop, int paramNum) {
  mutex.lock();
  // hack for ode 'flaw':
...
void OdePhysics::JointSetMaxForce(
...
void OdePhysics::JointSetConstraintForceMixing(IPhysicsJoint *joint,
                                               float value, int paramNum) {
  mutex.lock();
  JointSetParameter(joint, dParamCFM, value, paramNum);
  mutex.unlock();
}

void OdePhysics::JointSetErrorCorrection(IPhysicsJoint *joint, float value,
                                         int paramNum) {
  mutex.lock();
...
void OdePhysics::JointSetSuspensionConstraintForceMixing(
    IPhysicsJoint *joint, float value) { // only on hinge2 joints
  mutex.lock();
  JointSetParameter(joint, dParamSuspensionCFM, value);
  mutex.unlock();
}

void OdePhysics::JointSetSuspensionErrorReduction(
    IPhysicsJoint *joint, float value) { // only on hinge2 joints
  mutex.lock();
...
void OdePhysics::JointSetParameter(IPhysicsJoint *joint, int parameter,
                                   float value, int paramNum) {
  ...
  */
  ...
  case e_JointType_Hinge2:
    dJointSetHinge2Param(odeJoint->id, parameter + dParamGroup * (paramNum - 1),
                         value);
    break;
