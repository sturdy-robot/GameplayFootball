...
void Initialize(Properties &config) {
  ...
  boost::shared_ptr<ResourceManager<GeometryData>> geometryDataResourceManager(
      new ResourceManager<GeometryData>("geometrydata"));
  boost::shared_ptr<ResourceManager<Surface>> surfaceResourceManager(
      new ResourceManager<Surface>("surface"));
  boost::shared_ptr<ResourceManager<SoundBuffer>> soundBufferResourceManager(
      new ResourceManager<SoundBuffer>("soundbuffer"));

  ResourceManagerPool::GetInstance().RegisterManager(
      e_ResourceType_GeometryData, geometryDataResourceManager);
  ResourceManagerPool::GetInstance().RegisterManager(e_ResourceType_Surface,
                                                     surfaceResourceManager);
  ResourceManagerPool::GetInstance().RegisterManager(
      e_ResourceType_SoundBuffer, soundBufferResourceManager);
...
void RegisterObjectTypes(ObjectFactory *objectFactory) {
  ...
  boost::intrusive_ptr<AudioReceiver> audioReceiver(
      new AudioReceiver("AudioReceiver prototype"));
